# Extended Pro Review Prompt: After Coarse ETC Suffix Regret Bound

ABRL Lean 4 status:

- `ETC-ACTION-WITH-COMMIT-SUFFIX-COUNT-BUDGET-REGRET` compiles locally.
- Extended Pro then chose Candidate C.
- `ETC-ACTION-WITH-COMMIT-COARSE-SUFFIX-REGRET-BOUND` now compiles locally.
...
- Do not prove the generic constant-arm suffix equality, the ETC phase split,
  RHS simplification of the unsimplified budget, empirical means, commit
  argmax, probability, concentration, filtration, conditional expectation, or
  final ETC theorem without a new single-leaf decision.

Please choose the next exact single leaf.
...
## Candidate C: Simplify Unsimplified Suffix Budget RHS
