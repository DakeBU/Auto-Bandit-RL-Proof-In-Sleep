# Extended Pro Review Response: After Oracle Wrong-Event Measurability

- Date: 2026-06-30
- Tool/model: ChatGPT Extended Pro
- URL: https://chatgpt.com/c/6a43200e-b72c-83e8-8f5f-cff9ddc0a06c
- Prompt file: `reports/extended_pro_after_commit_oracle_event_meas_candidate_prompt_2026-06-30.md`
...
- Recorded from raw response:
  `reports/extended_pro_after_commit_oracle_event_meas_raw_2026-06-30.md`

## Reviewer Decision

- Chosen next leaf: ETC-COMMIT-ORACLE-CHOICE-MEASURABILITY-ROUTE-CARD
- Classification: theorem-card-only route; project-local compiled candidate confirmed
- Status: reviewer-approved

Local follow-up: the reviewer-named compiled candidate
`ETC.measurable_commitOracle_choose_of_measurable_empMeanVector` was checked
against the current Lean environment and implemented as the corresponding
compiled bridge leaf.
...
- `ETC.CommitOracle`
- Mathlib `measurable_of_countable`

## Intended Proof Route

1. Prove `Measurable (fun score : Fin K -> Rat => oracle.choose score)` by
   `measurable_of_countable _`.
2. Compose that selector measurability with the empirical-mean vector
   measurability assumption `hmeas_emp`.
...
- `[MeasurableSingletonClass (Fin K -> Rat)]`
- `[Countable (Fin K -> Rat)]`
- `Measurable (fun omega => (empMean omega : Fin K -> Rat))`

## Retrieval Evidence

- Extended Pro selected Candidate C and identified `measurable_of_countable`
  as the Mathlib route.
- Local Lean check confirmed the proof skeleton compiles in
  `BanditRLProof.Algorithms.ETCMeasurability`.
- `ETC.measurableSet_commitOracle_ne_bestArm` consumes the resulting
  `hmeas_choose` contract.

## Failure Policy

- Do not weaken back to assuming `hmeas_choose`; that duplicates the previous
  boundary.
- Do not attempt concrete argmax construction, pairwise-tail import work,
  concentration, filtration, or final ETC theorem in this batch.
- If future vector measurability is missing, make it a separate
  coordinate-to-vector empirical-mean measurability leaf.
