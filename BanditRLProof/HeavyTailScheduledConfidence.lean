...
theorem scheduled_mean_tail {Ω : Type*} [MeasurableSpace Ω]
  ...
  have htail := truncated_mean_tail μ X (sampleThreshold ε u t) ε u b (confidenceLog t) mean n hn
...
theorem scheduled_adaptive_mean_tail {Ω : Type*} [MeasurableSpace Ω]
  ...
  let E := fun k => {ω | confidenceRadius ε u t (k+1) ≤
    |(∑ s ∈ Finset.range (k+1), truncate (sampleThreshold ε u t s) (X s ω)) / ((k+1 : ℕ) : ℝ) - mean|}
  ...
  calc
    ...
    _ ≤ ∑ _k ∈ Finset.range t, 2 * Real.exp (-confidenceLog t) := by
      ...
      intro k _
      simpa only [E, Nat.cast_add, Nat.cast_one] using scheduled_mean_tail μ X ε u mean t (k+1)
