...
/-!
# Time-varying loss evaluators on IID states for scheduled half-Tsallis

Fresh states remain IID, but the deterministic evaluator may depend on the
round.  This is the process-law layer for an oblivious predictable corruption
schedule.  The generated trajectory still factors through each finite state
prefix, so the current coordinate is independent of the pre-action trace.
-/
...
def iidTimeVaryingLossStatePredictableLossVector
    {LossState : Type u} {Action : Type v}
    [MeasurableSpace LossState] [MeasurableSpace Action]
    (value : Nat -> LossState -> Action -> Real)
    (hvalue : ∀ t, Measurable (fun input : LossState × Action =>
      value t input.1 input.2))
    (hvalue_nonneg : ∀ t state action, 0 <= value t state action)
    (hvalue_le_one : ∀ t state action, value t state action <= 1) :
    Exp3.PredictableLossVector (Nat -> LossState) Action where
  initial environment action := value 0 (environment 0) action
  successor n environment _history action :=
    value (n + 1) (environment (n + 1)) action
  measurable_initial := by
    ...
    exact (hvalue 0).comp (hstate.prodMk measurable_snd)
  measurable_successor n := by
    ...
    exact (hvalue (n + 1)).comp (hstate.prodMk haction)
  initial_nonneg environment action :=
    hvalue_nonneg 0 (environment 0) action
  initial_le_one environment action :=
    hvalue_le_one 0 (environment 0) action
  successor_nonneg n environment _history action :=
    hvalue_nonneg (n + 1) (environment (n + 1)) action
  successor_le_one n environment _history action :=
    hvalue_le_one (n + 1) (environment (n + 1)) action
...
theorem predictableLossAt_iidTimeVaryingLossStatePredictableLossVector
    ...
    Exp3.predictableLossAt
        (iidTimeVaryingLossStatePredictableLossVector value hvalue
          hvalue_nonneg hvalue_le_one) t sample action =
      value t (sample.1 t) action := by
  cases t <;> simp [Exp3.predictableLossAt,
    iidTimeVaryingLossStatePredictableLossVector]

/-- Equal state prefixes generate equal visible trajectory prefixes for a
time-varying fresh-state evaluator. -/
theorem sampledScheduledHalfTsallisTrajectoryKernel_map_frestrictLe_eq_of_iidTimeVaryingLossState_prefix_eq
    {LossState : Type u} {Action : Type v}
...
noncomputable def sampledScheduledHalfTsallisIIDTimeVaryingPrefixKernel
    {LossState : Type u} {Action : Type v}
...
instance instSampledScheduledHalfTsallisIIDTimeVaryingPrefixKernelIsMarkov
    {LossState : Type u} {Action : Type v}
    [MeasurableSpace LossState]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
    [Nonempty Action] [DecidableEq Action]
    ...
    (selector : HalfTsallisScheduleFiniteHistorySelectorMeasurability
      arms harms eta) (n : Nat) :
    IsMarkovKernel
      (sampledScheduledHalfTsallisIIDTimeVaryingPrefixKernel fallback value
        hvalue hvalue_nonneg hvalue_le_one arms harms eta selector n) := by
  unfold sampledScheduledHalfTsallisIIDTimeVaryingPrefixKernel
  letI : IsMarkovKernel
      ((sampledScheduledHalfTsallisTrajectoryKernel
          arms harms eta selector
            (iidTimeVaryingLossStatePredictableLossVector value hvalue
...
/-- The canonical trajectory factors through every finite IID state prefix
even when the deterministic evaluator varies with time. -/
theorem hasScheduledIIDPrefixKernelFactorization_sampledScheduledHalfTsallisTimeVaryingTrajectoryKernel
    ...
    HasScheduledIIDPrefixKernelFactorization
      ...
      horizon := by
  intro n _hn
  let prefixKernel := sampledScheduledHalfTsallisIIDTimeVaryingPrefixKernel
    fallback value hvalue hvalue_nonneg hvalue_le_one
      arms harms eta selector n
  refine ⟨prefixKernel, ?_, ?_⟩
  · exact instSampledScheduledHalfTsallisIIDTimeVaryingPrefixKernelIsMarkov
      fallback value hvalue hvalue_nonneg hvalue_le_one
        arms harms eta selector n
...
noncomputable def independentLossStateTimeVaryingMeanGap
    {LossState : Type u} {Action : Type v}
    [MeasurableSpace LossState]
    (law : Nat -> Measure LossState)
    (value : Nat -> LossState -> Action -> Real)
    (t : Nat) (best action : Action) : Real :=
  ∫ state, value t state action - value t state best ∂law t

/-- Mean loss gap of the round-`t` evaluator under the common coordinate law. -/
noncomputable def iidLossStateTimeVaryingMeanGap
    {LossState : Type u} {Action : Type v}
    [MeasurableSpace LossState]
    (law : Measure LossState) (value : Nat -> LossState -> Action -> Real)
    (t : Nat) (best action : Action) : Real :=
  ∫ state, value t state action - value t state best ∂law

/-- Independent, potentially nonidentically distributed coordinates plus
prefix factorization produce the time-varying independence and global-mean
contract. -/
theorem hasScheduledTimeVaryingIndependentMeanGapLaw_of_independentLossState
    {LossState : Type u} {Action : Type v}
    [MeasurableSpace LossState] [StandardBorelSpace LossState]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
    [StandardBorelSpace Action] [Nonempty Action] [DecidableEq Action]
    (law : Nat -> Measure LossState)
    [∀ t, IsProbabilityMeasure (law t)]
    (value : Nat -> LossState -> Action -> Real)
    (hvalue : ∀ t, Measurable (fun input : LossState × Action =>
      value t input.1 input.2))
    (hvalue_nonneg : ∀ t state action, 0 <= value t state action)
    ...
    HasScheduledTimeVaryingIndependentMeanGapLaw mu arms
      (iidTimeVaryingLossStatePredictableLossVector value hvalue
        hvalue_nonneg hvalue_le_one)
      best (fun t action =>
        independentLossStateTimeVaryingMeanGap law value t best action)
      horizon := by
  dsimp only
  let prior : Measure (Nat -> LossState) :=
    Measure.infinitePi law
  ...
  · cases t with
    ...
    | succ n =>
        ...
        let currentDiff := fun environment : Nat -> LossState =>
          value (n + 1) (environment (n + 1)) action -
            value (n + 1) (environment (n + 1)) best
        ...
        have hcurrentDiff : Measurable currentDiff := by
          ...
          exact
            ((hvalue (n + 1)).comp
              (hstate.prodMk measurable_const)).sub
            ((hvalue (n + 1)).comp
              (hstate.prodMk measurable_const))
        ...
        have hcoordPast : IndepFun currentDiff pastEnvironment prior := by
          ...
          let readCurrent := fun states : (i : currentSet) -> LossState =>
            value (n + 1) (states currentIndex) action -
              value (n + 1) (states currentIndex) best
  ...
  · have hstateDiff : Measurable (fun state : LossState =>
        value t state action - value t state best) :=
      ((hvalue t).comp (measurable_id.prodMk measurable_const)).sub
        ((hvalue t).comp (measurable_id.prodMk measurable_const))
    calc
      integral mu lossDiff =
          integral mu (fun sample =>
            value t (sample.1 t) action - value t (sample.1 t) best) := by
        congr 1
        funext sample
        simp only [lossDiff, loss,
          predictableLossAt_iidTimeVaryingLossStatePredictableLossVector]
      _ = integral (mu.map Prod.fst) (fun environment : Nat -> LossState =>
            value t (environment t) action - value t (environment t) best) := by
        exact (integral_map
          (μ := mu) (φ := Prod.fst)
          (f := fun environment : Nat -> LossState =>
            value t (environment t) action - value t (environment t) best)
      ...
      _ = integral (prior.map (fun environment : Nat -> LossState =>
            environment t)) (fun state : LossState =>
              value t state action - value t state best) := by
        exact (integral_map
      ...
      _ = integral (law t) (fun state : LossState =>
            value t state action - value t state best) := by
        rw [show prior = Measure.infinitePi law from rfl,
          Measure.infinitePi_map_eval]
      _ = independentLossStateTimeVaryingMeanGap
          law value t best action := rfl

/-- IID coordinates are the constant-law specialization of the independent
coordinate producer. -/
theorem hasScheduledTimeVaryingIndependentMeanGapLaw_of_iidLossState
    {LossState : Type u} {Action : Type v}
    [MeasurableSpace LossState] [StandardBorelSpace LossState]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
    [StandardBorelSpace Action] [Nonempty Action] [DecidableEq Action]
    ...
    HasScheduledTimeVaryingIndependentMeanGapLaw mu arms
      (iidTimeVaryingLossStatePredictableLossVector value hvalue
        hvalue_nonneg hvalue_le_one)
      best (fun t action =>
        iidLossStateTimeVaryingMeanGap law value t best action) horizon := by
  simpa [iidLossStateTimeVaryingMeanGap,
    independentLossStateTimeVaryingMeanGap] using
    (hasScheduledTimeVaryingIndependentMeanGapLaw_of_independentLossState
      (law := fun _ : Nat => law) value hvalue hvalue_nonneg hvalue_le_one
      arms best horizon trajectoryKernel hfactor)
