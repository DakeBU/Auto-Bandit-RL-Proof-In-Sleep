...
/--
Build the definitional generated-policy random next-pair source from a
policy-selected reward-coordinate law.

The policy-facing reward law is first rewritten to the generated successor
action.  The existing generated-action reward-map route then supplies the
frozen-prefix extension-map `partialTraj` law consumed by the bare
`GeneratedActionRandomPairDefinitionalMapSource` constructor.
-/
def generatedActionRandomPairDefinitionalMapSource_of_reward_map_eq_selected_policy
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
          ...
          (ae
            (mu.trim
              ((History.historyFiltrationSucc
                ...
                hreward).le i)))) :
    GeneratedActionRandomPairDefinitionalMapSource mu rewardKernel policy
      context state defaultAction reward hreward := by
  have haction :
      forall t : Nat,
        Measurable
          (fun omega : Omega =>
...
/--
Upgrade a generated-action actual reward-coordinate source to the random
next-pair map source.

The source already fixes the action trace to the shifted generated policy and
identifies the conditional reward-coordinate law at the actual successor
action.  The split-product condExpKernel law then recovers the full random
`(action, reward)` next-pair pushforward.
-/
...
/--
Practical definitional raw-range source with a deterministic variance-proxy
ceiling.

The base source carries the generated-action random next-pair law, raw reward
range, selected-mean range, and centered kernel law.  This wrapper adds the
uniform model-side upper bound needed to choose a deterministic
`HasCondSubgaussianMGF` variance proxy.
-/
structure GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
...
/--
Practical definitional raw-range source with time-indexed selected-history
variance-proxy ceilings.

This is weaker than a global context/action ceiling: the bound only needs to
hold on histories reachable by the reward-history context/state surface at the
time being consumed.
-/
structure GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
    ...
    (hreward : forall t : Nat,
      Measurable (fun omega : Omega => reward omega t))
    (rewardLo rewardHi meanLo meanHi : Nat -> Real)
    (varianceCeiling : Nat -> NNReal) where
  base_source :
    GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource
      mu rewardKernel policy context state mean varianceProxy defaultAction
      reward hreward rewardLo rewardHi meanLo meanHi
...
/--
Project a definitional raw-range/measurable-mean-range uniform-variance source
to its packaged base raw-range/measurable-mean-range bounded source.

The uniform source adds a global variance-proxy ceiling for MGF consumers.
This wrapper records the weaker base-source interface for downstream consumers
that only need the generated random-pair law and raw/mean range regularity.
-/
...
/--
Project a definitional raw-range/measurable-mean-range history-variance source
to its packaged base raw-range/measurable-mean-range bounded source.

The history-variance source adds selected-history variance-proxy ceilings for
MGF consumers.  This wrapper records the weaker base-source interface for
downstream consumers that only need the generated random-pair law and raw/mean
range regularity.
-/
...
/--
A uniform variance-proxy ceiling is a constant time-indexed selected-history
variance ceiling.

This adapter lets downstream consumers that are stated against the weaker
history-specific source interface reuse a stronger global context/action source.
-/
def generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource_of_uniformVarianceBoundedSource
    ...
    GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource
      mu rewardKernel policy context state mean varianceProxy defaultAction
      reward hreward rewardLo rewardHi meanLo meanHi
      (fun _ : Nat => varianceCeiling) where
  base_source := source.base_source
  variance_history_le := by
    intro i history
    exact
      source.variance_le (context i history)
        ((policy i).action (state i history))
...
/--
Build the practical definitional generated-policy raw-range source with a
packaged selected-history variance ceiling from a full finite-pair-trace
`partialTraj` law.

This is the history-variance companion to
`generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource_of_actionRewardPartialTrajectoryKernel_map_eq`.
-/
...
/--
Build the practical definitional generated-policy raw-range source with a
packaged uniform variance ceiling from the narrower frozen-prefix extension-map
`partialTraj` law.
...
/--
Build the practical definitional generated-policy raw-range source with a
packaged selected-history variance ceiling from the narrower frozen-prefix
extension-map `partialTraj` law.
...
/--
Build the practical definitional generated-policy raw-range source with a
packaged uniform variance ceiling from the canonical history-step next-pair
law.
...
/--
Build the practical definitional generated-policy raw-range source with a
packaged selected-history variance ceiling from the canonical history-step
next-pair law.
...
/--
A real random variable with an a.e. interval bound has every exponential tilt
integrable on a finite measure space.
-/
theorem integrable_exp_mul_of_mem_Icc
    {Omega : Type u} [MeasurableSpace Omega]
    (mu : MeasureTheory.Measure Omega) [MeasureTheory.IsFiniteMeasure mu]
    (X : Omega -> Real)
    (lo hi : Real)
    (hX : AEMeasurable X mu)
    (hbound : Filter.Eventually (fun omega : Omega => Set.Icc lo hi (X omega)) (ae mu))
    (t : Real) :
    MeasureTheory.Integrable (fun omega : Omega => Real.exp (t * X omega)) mu := by
  let B : Real := max |lo| |hi|
  let C : Real := Real.exp (|t| * B)
  have h_exp_aemeas :
      AEMeasurable (fun omega : Omega => Real.exp (t * X omega)) mu := by
    simpa using (hX.const_mul t).exp
  have h_exp_aestr :
      AEStronglyMeasurable (fun omega : Omega => Real.exp (t * X omega)) mu :=
    h_exp_aemeas.aestronglyMeasurable
  refine MeasureTheory.Integrable.of_bound h_exp_aestr C ?_
  filter_upwards [hbound] with omega h_omega
  have habsX : |X omega| <= B := by
    exact abs_le_max_abs_abs h_omega.1 h_omega.2
  have hmul_abs : |t * X omega| <= |t| * B := by
    rw [abs_mul]
    exact mul_le_mul_of_nonneg_left habsX (abs_nonneg t)
  have harg : t * X omega <= |t| * B := by
    exact (le_abs_self (t * X omega)).trans hmul_abs
  have hexp : Real.exp (t * X omega) <= C := by
    dsimp [C]
    exact Real.exp_le_exp.mpr harg
  have hnorm : norm (Real.exp (t * X omega)) = Real.exp (t * X omega) := by
            ...
            mean
              ...
              ((policy i).action
                (state i
                  (History.finiteRewardHistoryOfTrace (reward omega) i))) :
              Rat) : Real)))) mu := by
  let centered : Omega -> Real := fun omega : Omega =>
    ((reward omega (i + 1) -
      mean
        (context i
          (History.finiteRewardHistoryOfTrace (reward omega) i))
...
/--
Project a raw-reward/selected-mean bounded generated random next-pair source
directly to its packaged random-pair map source.

This source already contains the map source used by its history-step and
actual-reward projections.  The wrapper records that weaker interface under a
stable name for downstream law consumers.
-/
def generatedActionRandomPairMapSource_of_randomPairRawMeanBoundedSource
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
...
/--
Project a raw-reward-bound/selected-mean bounded generated random next-pair
source directly to its packaged random-pair map source.
...
/--
Project a raw-reward-bound/measurable-selected-mean generated random next-pair
source directly to its packaged random-pair map source.
...
/--
Project a raw-reward-range/measurable-mean-range bounded generated random
next-pair source directly to its packaged random-pair map source.

This is the explicit-action counterpart of
`generatedActionRandomPairMapSource_of_definitionalRawRangeMeasurableMeanRangeBoundedSource`:
the top practical regularity source already contains the map source, and this
wrapper gives downstream law consumers a stable named entry point.
-/
...
/--
Consume a policy-selected reward-coordinate law plus raw reward and
selected-mean range regularity through the bare definitional random-pair map
source.

This is the source-route companion to
`centeredReward_succ_condExp_eq_zero_of_reward_map_eq_selected_policy_definitionalRawRangeMeasurableMeanRangeBounded`:
the reward law first builds `GeneratedActionRandomPairDefinitionalMapSource`,
then the existing source-level raw/mean range consumer supplies ordinary
succ-indexed conditional mean-zero.
-/
...
the succ-indexed conditional sub-Gaussian MGF witness for the centered reward.

This is the bounded-source wrapper around the centered-source MGF consumer.  It
uses the bounded source only to lower into `GeneratedActionRandomPairCenteredSource`;
Mathlib's MGF predicate still receives the ambient centered-reward
measurability, exponential integrability, and variance-proxy domination
...
/--
Consume a definitional generated-action raw-range source to obtain exponential
integrability for the generated centered successor reward.
-/
        ...
        Real.exp (t *
          (((reward omega (i + 1) -
            mean
              ...
              Rat) : Real)))) mu := by
  exact
    centeredReward_succ_integrable_exp_of_generatedActionRandomPairBoundedCenteredSource
      (mu := mu)
      (action :=
        generatedActionFromRewardHistory policy state defaultAction reward)
      (rewardKernel := rewardKernel)
...
/--
Lower a definitional generated-action raw-range/history-variance source into
the bounded centered-source interface.

This keeps the deterministic centered reward bounds available for downstream
integrability and tail consumers while hiding the selected-history variance
wrapper when those consumers only need the bounded-centered contract.
-/
...
/--
Lower a definitional generated-action raw-range/history-variance source into
the integrability-based centered-source interface.

This is the direct projection for consumers that need the centered-source
contract rather than the stronger bounded-centered or definitional interfaces.
-/
def generatedActionRandomPairCenteredSource_of_historyVarianceBoundedSource
  ...
  generatedActionRandomPairCenteredSource_of_boundedCenteredSource
    (mu := mu)
    (action := generatedActionFromRewardHistory
      policy state defaultAction reward)
    (rewardKernel := rewardKernel)
    (policy := policy)
    (context := context)
    ...
    (defaultAction := defaultAction)
    (reward := reward)
    (haction := generatedActionFromRewardHistory_measurable
      (policy := policy) (state := state) (defaultAction := defaultAction)
      (reward := reward) hreward
      source.base_source.definitional_map_source.hstate)
    (hreward := hreward)
...
coordinate law over `generatedActionFromRewardHistory`, not the full random
next-pair or centered-source interfaces.
-/
def generatedActionActualRewardMapSource_of_historyVarianceBoundedSource
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
...
/--
Lower a definitional generated-action raw-range/history-variance source into
the explicit generated random-pair map source.

This exposes the full random next-pair law over
`generatedActionFromRewardHistory` for downstream history-step and
`partialTraj` consumers while hiding the selected-history variance wrapper.
-/
def generatedActionRandomPairMapSource_of_historyVarianceBoundedSource
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
...
This exposes the full random next-pair law over
`generatedActionFromRewardHistory` for downstream history-step and
`partialTraj` consumers while hiding the uniform variance wrapper.
-/
...
next-pair or centered-source interfaces.
-/
def generatedActionActualRewardMapSource_of_uniformVarianceBoundedSource
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
...
This keeps deterministic centered reward bounds available for downstream
integrability and tail consumers while hiding the uniform variance wrapper when
those consumers only need the bounded-centered contract.
-/
...
The uniform-variance wrapper carries the practical raw/mean range regularity
and a global variance ceiling for MGF consumers.  Consumers that only need the
centered law and bounded-derived integrability can use this projection without
unpacking the uniform-variance source manually.
-/
def generatedActionRandomPairDefinitionalCenteredSource_of_uniformVarianceBoundedSource
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
...
route while deriving the exponential-integrability regularity contract from
the source bounds.
-/
...
/--
Consume the practical definitional raw-range source for the conditional MGF
route under a deterministic variance-proxy ceiling.

This replaces the trimmed-a.e. selected-history variance domination hypothesis
with a pointwise kernel-level ceiling on `varianceProxy`.
-/
...
/--
Consume the practical definitional raw-range source for the conditional MGF
route under a deterministic variance-proxy ceiling on the selected finite
reward histories at this time.

This is weaker than a global context/action ceiling and still removes the
...
/--
Consume a practical definitional raw-range source with a packaged deterministic
variance-proxy ceiling for the conditional MGF route.
-/
...
/--
Consume a practical definitional raw-range source with packaged time-indexed
selected-history variance-proxy ceilings for the conditional MGF route.
-/
theorem centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource
  ...
  exact
    centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource_of_history_variance_le
      ...
      (i := i)
      (c := varianceCeiling i)
      (source.variance_history_le i)

/--
Consume a packaged selected-history variance source with any deterministic
proxy that dominates the selected ceiling at the requested time.

This is useful when downstream tail APIs use a coarser shared proxy than the
model-side time-indexed variance schedule.
-/
theorem centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource_of_varianceCeiling_le
  ...
  exact
    centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource_of_history_variance_le
      ...
      (c := c)
      (fun history =>
        le_trans (source.variance_history_le i history) hceiling)

/--
Consume a uniform-variance source through the weaker selected-history variance
source interface.

This is a convenience wrapper for downstream callers that standardize on the
history-variance source API while their model supplies a global variance
ceiling.
-/
theorem centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_via_historyVarianceSource
  ...
  exact
    centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource
      ...
      (meanLo := meanLo)
      (meanHi := meanHi)
      (varianceCeiling := fun _ : Nat => varianceCeiling)
      (source := historySource)
      i

/--
Consume a packaged uniform-variance source with any deterministic proxy that
dominates the global variance ceiling.

This is the uniform-source companion to the selected-history larger-proxy
consumer above.
-/
...
raw/mean range regularity package and a global variance ceiling to obtain the
succ-indexed conditional MGF witness.

This is the history-step law-surface companion to the packaged
uniform-variance source consumer above.
-/
...
/--
Directly consume the canonical history-step next-pair law plus the practical
uniform variance package at any deterministic proxy that dominates the global
ceiling.

This is the coarser-proxy companion to
...
practical raw/mean range regularity package and a global variance ceiling to
obtain the succ-indexed conditional MGF witness.

This is the full-trace law-surface companion to the packaged
uniform-variance source consumer above.
-/
...
/--
Directly consume the full finite-pair-trace `partialTraj` law plus the
practical uniform variance package at any deterministic proxy that dominates
the global ceiling.
...
/--
Directly consume the full finite-pair-trace `partialTraj` law plus the
practical raw/mean range regularity package and a selected-history variance
ceiling to obtain the succ-indexed conditional MGF witness.

This is the full-trace law-surface companion to the packaged
history-variance source consumer above.
-/
...
/--
Directly consume the full finite-pair-trace `partialTraj` law plus the
practical selected-history variance package at any deterministic proxy that
dominates the selected ceiling at the requested time.
...
This is the extension-map law-surface companion to the packaged
uniform-variance source consumer above.
-/
...
/--
Directly consume the canonical history-step next-pair law plus the practical
raw/mean range regularity package and a selected-history variance ceiling to
obtain the succ-indexed conditional MGF witness.
...
/--
Directly consume the canonical history-step next-pair law plus the practical
selected-history variance package at any deterministic proxy that dominates the
selected ceiling at the requested time.
...
/--
Consume a packaged definitional actual-action reward-coordinate source plus
uniform variance regularity through the practical uniform-variance conditional
MGF route.

The packaged actual source supplies the reward-coordinate law and state
measurability.  The caller adds raw/mean range regularity and the global
variance ceiling; the proof builds the packaged uniform-variance source and
reuses its source-level MGF consumer.
-/
...
/--
Consume a packaged definitional actual-action reward-coordinate source plus
uniform variance regularity at a coarser deterministic proxy.
...
the proof builds the same packaged uniform-variance source and then reuses the
source-level larger-proxy consumer.
-/
...
/--
Build the packaged selected-history variance source from a packaged
definitional actual-action reward-coordinate source.

This is the history-variance companion to
`generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource_of_generatedActionDefinitionalActualRewardMapSource`:
the reward-coordinate source and raw/mean range regularity build the base
source, while `hvariance` supplies the time-indexed selected-history variance
ceiling used by downstream conditional MGF consumers.
-/
...
/--
Consume a packaged definitional actual-action reward-coordinate source plus
selected-history variance regularity through the practical history-variance
conditional MGF route.

The packaged actual source supplies the reward-coordinate law and state
measurability.  The caller adds raw/mean range regularity and the
time-indexed selected-history variance ceilings; the proof builds the packaged
history-variance source and reuses its source-level MGF consumer.
-/
...
/--
Consume a packaged definitional actual-action reward-coordinate source plus
selected-history variance regularity at a coarser deterministic proxy.
...
the proof builds the same packaged history-variance source and then reuses the
source-level larger-proxy consumer.
-/
...
This is the base source-level companion to the uniform/history variance source
constructors: the reward-coordinate law is first lifted to the frozen-prefix
extension-map `partialTraj` law, then packaged into
`GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource`.
-/
def generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource_of_reward_map_eq_actual_action
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
...
/--
Build the practical definitional generated-policy raw-range source from the
policy-selected reward-coordinate selected-measure law.

For `generatedActionFromRewardHistory`, the successor coordinate is
definitionally the policy-selected action.  This wrapper rewrites the
policy-facing reward law into the actual successor-action law and reuses the
base actual-action source constructor.
-/
def generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource_of_reward_map_eq_selected_policy
  ...
  have h_reward_map_eq_actual_action :
    ...
    simpa [generatedActionFromRewardHistory] using h_reward
  exact
    generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource_of_reward_map_eq_actual_action
      (mu := mu)
      (rewardKernel := rewardKernel)
      (policy := policy)
      (context := context)
...
/--
Directly consume the actual-action reward-coordinate selected-measure law plus
the practical raw/mean range regularity package and a global variance ceiling
to obtain the succ-indexed conditional MGF witness.

This is the reward-coordinate law-surface companion to the frozen-prefix
extension-map uniform-variance conditional MGF wrapper.
-/
...
/--
Build the practical definitional generated-policy raw-range source with a
packaged uniform variance ceiling from the actual-action reward-coordinate
selected-measure law.

This source-level wrapper preserves the reward-map law surface while exposing
the reusable packaged uniform-variance source used by downstream conditional
MGF consumers.
-/
...
/--
Directly consume the actual-action reward-coordinate selected-measure law plus
the practical uniform-variance regularity package at any deterministic proxy
that dominates the global variance ceiling.
...
This is the policy-facing counterpart of the actual-action reward-map wrapper:
callers can state the reward law at `(policy i).action (state i history)`
without first rewriting it to the generated successor action.
-/
...
/--
Consume the policy-selected reward-coordinate law plus the practical
uniform-variance regularity package through the bare definitional random-pair
map source.

The reward law first builds `GeneratedActionRandomPairDefinitionalMapSource`;
that bare source is then wrapped with raw/mean range regularity and the global
variance ceiling before the source-level conditional MGF consumer is applied.
-/
theorem centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_selected_policy_definitionalMapSource_rawRangeMeasurableMeanRangeUniformVarianceBounded
  ...
  let baseSource :
    ...
    mean_measurable := hmean
    kernel_law := hkernel
    definitional_map_source := mapSource
    raw_reward_range_bound := hraw
    mean_range_bound := hmean_range
  }
  let source :
  ...
  simpa [mapSource, baseSource, source] using
    centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource
      (mu := mu)
      (rewardKernel := rewardKernel)
      (policy := policy)
...
/--
Consume the policy-selected reward-coordinate law plus uniform-variance
regularity through the bare definitional random-pair map source at a coarser
deterministic proxy.
...
/--
Consume the policy-selected reward-coordinate law plus the practical
selected-history variance regularity package through the bare definitional
random-pair map source.

The reward law first builds `GeneratedActionRandomPairDefinitionalMapSource`;
that bare source is then wrapped with raw/mean range regularity and the
time-indexed selected-history variance ceiling before the source-level
conditional MGF consumer is applied.
-/
...
/--
Consume the policy-selected reward-coordinate law plus selected-history
variance regularity through the bare definitional random-pair map source at a
coarser deterministic proxy.
...
/--
Build the practical definitional generated-policy raw-range source with a
packaged uniform variance ceiling from the policy-selected reward-coordinate
selected-measure law.

This is the policy-facing source-level companion to
`generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_reward_map_eq_actual_action`.
-/
def generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_reward_map_eq_selected_policy
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
...
/--
Directly consume the policy-selected reward-coordinate selected-measure law plus
the practical uniform-variance regularity package at any deterministic proxy
that dominates the global variance ceiling.

This is the policy-facing coarser-proxy companion to
...
/--
Directly consume the actual-action reward-coordinate selected-measure law plus
the practical raw/mean range regularity package and selected-history variance
ceilings to obtain the succ-indexed conditional MGF witness.

This is the reward-coordinate law-surface companion to the frozen-prefix
extension-map history-variance conditional MGF wrapper.
-/
...
/--
Build the practical definitional generated-policy raw-range source with a
packaged selected-history variance ceiling from the actual-action
reward-coordinate selected-measure law.

This source-level wrapper preserves the reward-map law surface while exposing
the reusable packaged history-variance source used by downstream conditional
MGF consumers.
-/
...
/--
Directly consume the actual-action reward-coordinate selected-measure law plus
the practical selected-history variance package at any deterministic proxy
that dominates the selected ceiling at the requested time.
...
theorem centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_actual_action_definitionalRawRangeMeasurableMeanRangeHistoryVarianceBounded_of_varianceCeiling_le
  ...
  let source :
      GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource
        mu rewardKernel policy context state mean varianceProxy defaultAction
        reward hreward rewardLo rewardHi meanLo meanHi varianceCeiling :=
    generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource_of_reward_map_eq_actual_action
      (mu := mu)
      (rewardKernel := rewardKernel)
      (policy := policy)
      (context := context)
...
/--
Directly consume the policy-selected reward-coordinate law plus the practical
raw/mean range regularity package and selected-history variance ceilings to
obtain the succ-indexed conditional MGF witness.

This is the policy-facing history-variance counterpart of the actual-action
reward-map wrapper: callers can state the reward law at
`(policy i).action (state i history)` without first rewriting it to the
generated successor action.
-/
...
/--
Build the practical definitional generated-policy raw-range source with a
packaged selected-history variance ceiling from the policy-selected
reward-coordinate selected-measure law.
