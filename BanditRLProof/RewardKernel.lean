...
/--
Canonical action-marginal regular conditional distribution for the
action/reward trajectory generated by the local history-step kernel family.

This is the `Prod.fst` projection of
`actionRewardHistoryStepKernelFamily_condDistrib_trajMeasure`: on Mathlib's
canonical `trajMeasure`, conditioning the next action coordinate on the finite
action/reward prefix recovers the action marginal of the configured
`actionRewardHistoryStepKernelFamily`.
-/
      ...
      (ProbabilityTheory.condDistrib
        ...
        (ProbabilityTheory.Kernel.trajMeasure
          ...
          (actionRewardHistoryStepKernelFamily rewardKernel policy context state
            hcontext hstate)))
      ((actionRewardHistoryStepKernelFamily rewardKernel policy context state
        hcontext hstate n).map Prod.fst) := by
  let stepKernel :=
    actionRewardHistoryStepKernelFamily rewardKernel policy context state
      hcontext hstate
  let trajMeasure :=
...
/--
Canonical selected-action regular conditional distribution for the
action/reward trajectory generated by the local history-step kernel family.

This rewrites the action marginal from
`actionRewardHistoryStepKernelFamily_action_condDistrib_trajMeasure` into the
Dirac law at the action selected by the frozen finite-history policy state.
-/
theorem actionRewardHistoryStepKernelFamily_selectedAction_condDistrib_trajMeasure
    {Context : Type x} {State : Type u} {Action : Type y}
    {Reward : Type v}
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSpace Reward]
    ...
    Filter.EventuallyEq
      ...
      (fun history : (i : Finset.Iic n) -> Prod Action Reward =>
        Measure.dirac ((policy n).action (state n history))) := by
  have haction :=
    actionRewardHistoryStepKernelFamily_action_condDistrib_trajMeasure
      mu0 rewardKernel policy context state hcontext hstate n
  filter_upwards [haction] with history haction_eq
  rw [haction_eq]
  rw [ProbabilityTheory.Kernel.map_apply _ measurable_fst]
  exact
    actionRewardHistoryStepKernelFamily_action_map
