...
/--
Convert a real-valued `condDistrib` law into a trimmed
`condExpKernel.map` law without assuming `Countable Real`.

The proof first obtains equality on every rational left ray `Iic q`, upgrades
those scalar equalities to the trimmed measure using conditioning-space
measurability, and then reconstructs the full Borel measure by Mathlib's
countable rational-ray pi-system induction.
-/
theorem condExpKernel_map_eq_of_condDistrib_ae_eq_real_trim
    {Omega : Type u} {Condition : Type v}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [Nonempty Omega]
    [mCondition : MeasurableSpace Condition]
    (mu : Measure Omega) [IsFiniteMeasure mu]
    (X : Omega -> Real) (Y : Omega -> Condition)
    (hX : @Measurable Omega Real mOmega inferInstance X)
    (hY : @Measurable Omega Condition mOmega mCondition Y)
    (kernel : ProbabilityTheory.Kernel Condition Real)
    ...
    Filter.Eventually
      (fun omega : Omega =>
        ...
        kernel (Y omega))
      (ae (mu.trim hY.comap_le)) := by
  let lhs : Omega -> Measure Real := fun omega =>
    @Measure.map Omega Real mOmega inferInstance X
      (@ProbabilityTheory.condExpKernel Omega mOmega _ mu _
        (mCondition.comap Y) omega)
  let rhs : Omega -> Measure Real := fun omega => kernel (Y omega)
  have hcond_pullback :
      Filter.Eventually
        (fun omega : Omega =>
          ProbabilityTheory.condDistrib X Y mu (Y omega) =
  ...
  have hIic :
      forall q : Rat,
        Filter.EventuallyEq (ae (mu.trim hY.comap_le))
          (fun omega : Omega => lhs omega (Set.Iic (q : Real)))
          (fun omega : Omega => rhs omega (Set.Iic (q : Real))) := by
    intro q
    have hevent :
        Filter.EventuallyEq (ae mu)
          (fun omega : Omega =>
            ProbabilityTheory.condDistrib X Y mu (Y omega)
              (Set.Iic (q : Real)))
          (fun omega : Omega =>
            lhs omega (Set.Iic (q : Real))) := by
      simpa [lhs, ProbabilityTheory.Kernel.map_apply _ hX] using
        (ProbabilityTheory.condDistrib_apply_ae_eq_condExpKernel_map
          (μ := mu) hX hY (measurableSet_Iic :
            MeasurableSet (Set.Iic (q : Real))))
    have hambient :
        Filter.EventuallyEq (ae mu)
          (fun omega : Omega => lhs omega (Set.Iic (q : Real)))
          (fun omega : Omega => rhs omega (Set.Iic (q : Real))) := by
      filter_upwards [hevent, hcond_pullback] with omega heq hkernel
      rw [← heq, hkernel]
    have hlhs :
        @Measurable Omega ENNReal
          (mCondition.comap Y) inferInstance
          (fun omega : Omega => lhs omega (Set.Iic (q : Real))) := by
      have hmeas :=
        ((@ProbabilityTheory.condExpKernel Omega mOmega _ mu _
          (mCondition.comap Y)).map X).measurable_coe
            (measurableSet_Iic :
              MeasurableSet (Set.Iic (q : Real)))
      simpa [lhs, ProbabilityTheory.Kernel.map_apply _ hX] using hmeas
    have hrhs :
        @Measurable Omega ENNReal
          (mCondition.comap Y) inferInstance
          (fun omega : Omega => rhs omega (Set.Iic (q : Real))) := by
      exact
        (kernel.measurable_coe
          (measurableSet_Iic :
            MeasurableSet (Set.Iic (q : Real)))).comp
          (Measurable.of_comap_le le_rfl)
    exact
      ae_eq_trim_of_measurable hY.comap_le hlhs hrhs hambient
  have huniv :
      Filter.EventuallyEq (ae (mu.trim hY.comap_le))
        (fun omega : Omega => lhs omega Set.univ)
        (fun omega : Omega => rhs omega Set.univ) := by
    have hevent :
        Filter.EventuallyEq (ae mu)
          (fun omega : Omega =>
            ProbabilityTheory.condDistrib X Y mu (Y omega) Set.univ)
          (fun omega : Omega => lhs omega Set.univ) := by
      simpa [lhs, ProbabilityTheory.Kernel.map_apply _ hX] using
        (ProbabilityTheory.condDistrib_apply_ae_eq_condExpKernel_map
          (μ := mu) hX hY MeasurableSet.univ)
    have hambient :
        Filter.EventuallyEq (ae mu)
          (fun omega : Omega => lhs omega Set.univ)
          (fun omega : Omega => rhs omega Set.univ) := by
    ...
    have hlhs :
      ...
      have hmeas :=
        ((@ProbabilityTheory.condExpKernel Omega mOmega _ mu _
          (mCondition.comap Y)).map X).measurable_coe MeasurableSet.univ
      simpa [lhs, ProbabilityTheory.Kernel.map_apply _ hX] using hmeas
    have hrhs :
        @Measurable Omega ENNReal
          (mCondition.comap Y) inferInstance
          (fun omega : Omega => rhs omega Set.univ) := by
      exact
        (kernel.measurable_coe MeasurableSet.univ).comp
          (Measurable.of_comap_le le_rfl)
    exact
      ae_eq_trim_of_measurable hY.comap_le hlhs hrhs hambient
  have hsets :
      Filter.Eventually
        (fun omega : Omega =>
          forall ⦃s : Set Real⦄, MeasurableSet s ->
            lhs omega s = rhs omega s)
        (ae (mu.trim hY.comap_le)) := by
    apply @MeasurableSpace.ae_induction_on_inter
      Real Omega (mCondition.comap Y) (mu.trim hY.comap_le)
      (fun omega s => lhs omega s = rhs omega s)
      (⋃ q : Rat, {Set.Iic (q : Real)}) inferInstance
      Real.borel_eq_generateFrom_Iic_rat Real.isPiSystem_Iic_rat
    · simp
    · simp only [Set.iUnion_singleton_eq_range, Set.mem_range,
        forall_exists_index, forall_apply_eq_imp_iff]
      exact ae_all_iff.2 hIic
    · filter_upwards [huniv] with omega huniv_eq s hs heq
      rw [measure_compl hs (measure_ne_top _ _), heq, huniv_eq,
        measure_compl hs (measure_ne_top _ _)]
    · refine ae_of_all _ (fun omega f hdisj hf heq => ?_)
...
/--
Build a pair pushforward law from a deterministic action side and a reward
pushforward law.

This is the measure-level split helper behind the next-pair route, stripped of
filtration-specific hypotheses.  It is useful both for canonical trajectory
laws and for later ambient laws once the relevant `condExpKernel` has been
identified.
-/
theorem pair_map_eq_map_prod_mk_of_action_ae_eq_const_reward_map_eq
    ...
    (h_nextReward : @Measurable Omega Reward mOmega inferInstance nextReward)
    (h_action_ae :
    ...
    @Measure.map Omega (Prod Action Reward) mOmega inferInstance
      ...
      Measure.map (Prod.mk selectedAction) selectedReward := by
  have h_pair_ae :
      Filter.EventuallyEq (ae mu)
        (fun omega : Omega => (nextAction omega, nextReward omega))
        (fun omega : Omega => (selectedAction, nextReward omega)) := by
    filter_upwards [h_action_ae] with omega h_action
    simp [h_action]
  have h_prod_mk_meas :
      Measurable (Prod.mk selectedAction : Reward -> Prod Action Reward) := by
    fun_prop
  have h_map_map :
      Measure.map (Prod.mk selectedAction)
          (@Measure.map Omega Reward mOmega inferInstance nextReward mu) =
        @Measure.map Omega (Prod Action Reward) mOmega inferInstance
          (fun omega : Omega => (selectedAction, nextReward omega)) mu := by
    rw [Measure.map_map h_prod_mk_meas h_nextReward]
    rfl
  calc
    @Measure.map Omega (Prod Action Reward) mOmega inferInstance
        (fun omega : Omega => (nextAction omega, nextReward omega)) mu =
    ...
    _ =
      Measure.map (Prod.mk selectedAction)
        (@Measure.map Omega Reward mOmega inferInstance nextReward mu) := by
        exact h_map_map.symm
    _ = Measure.map (Prod.mk selectedAction) selectedReward := by
        rw [h_reward_map_eq]
...
`actionRewardHistoryStepKernelFamily_reward_condExpKernel_map_trajMeasure`: on
Mathlib's canonical action/reward trajectory measure, conditioning on the
finite pair prefix and pushing `condExpKernel` forward by the next pair
coordinate recovers the configured history-step kernel.
-/
theorem actionRewardHistoryStepKernelFamily_pair_condExpKernel_map_trajMeasure
    {Context : Type x} {State : Type u} {Action : Type v}
    {Reward : Type w}
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSpace Reward]
  ...
  let prefixMap :
    ...
    Preorder.frestrictLe n
  let nextPair : ((t : Nat) -> Prod Action Reward) -> Prod Action Reward :=
    fun trajectory => trajectory (n + 1)
  ...
  have hcond :
      Filter.EventuallyEq (ae (trajMeasure.map prefixMap))
        (ProbabilityTheory.condDistrib nextPair prefixMap trajMeasure)
        (stepKernel n) := by
    simpa [stepKernel, trajMeasure, prefixMap, nextPair] using
  ...
  have hbridge :=
    condExpKernel_map_eq_of_condDistrib_ae_eq_countable
      ...
      (X := nextPair)
      (Y := prefixMap)
      h_nextPair h_prefix (stepKernel n) hcond
  simpa [stepKernel, trajMeasure, prefixMap, nextPair] using hbridge

/--
Canonical `trajMeasure` next-action law in `condExpKernel.map` form.

This is the action-coordinate projection of
`actionRewardHistoryStepKernelFamily_pair_condExpKernel_map_trajMeasure`: on
Mathlib's canonical action/reward trajectory measure, conditioning on the
finite pair prefix and pushing `condExpKernel` forward by the next action
coordinate recovers the Dirac law at the policy-selected action.
-/
theorem actionRewardHistoryStepKernelFamily_action_condExpKernel_map_trajMeasure
    {Context : Type x} {State : Type u} {Action : Type v}
    {Reward : Type w}
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSpace Reward]
  ...
  have hpair :=
    actionRewardHistoryStepKernelFamily_pair_condExpKernel_map_trajMeasure
      mu0 rewardKernel policy context state hcontext hstate n
  filter_upwards [hpair] with trajectory h_pair
  ...
  calc
    @Measure.map ((t : Nat) -> Prod Action Reward) Action
        inferInstance inferInstance nextAction condKernel =
      Measure.map Prod.fst (Measure.map nextPair condKernel) := by
        rw [Measure.map_map measurable_fst h_nextPair_meas]
        rfl
    _ =
      Measure.map Prod.fst
        (stepKernel n (Preorder.frestrictLe n trajectory)) := by
        exact congrArg (Measure.map Prod.fst) h_pair
    _ =
      Measure.dirac
        ((policy n).action
          (state n (Preorder.frestrictLe n trajectory))) := by
        exact
...
/--
Canonical `trajMeasure` action-marginal law in `condExpKernel.map` form via
the action-coordinate `condDistrib` route.

This is the action-side analogue of
`actionRewardHistoryStepKernelFamily_reward_condExpKernel_map_trajMeasure`:
on the canonical action/reward trajectory measure, conditioning on the finite
pair prefix and pushing `condExpKernel` forward by the next action coordinate
recovers the `Prod.fst` marginal of the configured history-step kernel.  The
target countability contract is on `Action`, not on the whole
`(Action × Reward)` pair.
-/
theorem actionRewardHistoryStepKernelFamily_actionMarginal_condExpKernel_map_trajMeasure
  ...
  simpa [stepKernel, trajMeasure, prefixMap, nextAction] using hbridge
...
theorem actionRewardHistoryStepKernelFamily_selectedAction_condExpKernel_map_trajMeasure
  ...
  have haction :=
    actionRewardHistoryStepKernelFamily_actionMarginal_condExpKernel_map_trajMeasure
      mu0 rewardKernel policy context state hcontext hstate n
  filter_upwards [haction] with trajectory h_action
  calc
    @Measure.map ((t : Nat) -> Prod Action Reward) Action
        inferInstance inferInstance
        (fun y : (t : Nat) -> Prod Action Reward => (y (n + 1)).1)
      ...
      ((stepKernel n).map Prod.fst) (Preorder.frestrictLe n trajectory) := by
        simpa [stepKernel, trajMeasure] using h_action
    _ =
      Measure.dirac
...
This pushes
`actionRewardHistoryStepKernelFamily_pair_condExpKernel_map_trajMeasure`
through `History.extendPairHistorySucc`, yielding the one-step
`RewardKernel.actionRewardPartialTrajectoryKernel` surface used by the
extension-map consumers.  It remains a canonical `trajMeasure` theorem; it does
not transport an arbitrary ambient process.
-/
theorem actionRewardPartialTrajectoryKernel_extend_condExpKernel_map_trajMeasure
    {Context : Type x} {State : Type u} {Action : Type v}
    {Reward : Type w}
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSpace Reward]
  ...
  let condKernel : Measure ((t : Nat) -> Prod Action Reward) :=
    @ProbabilityTheory.condExpKernel
      ...
      ((inferInstance :
        ...
        (Preorder.frestrictLe n))
      trajectory
  let histPrefix : (i : Finset.Iic n) -> Prod Action Reward :=
    Preorder.frestrictLe n trajectory
  let nextPair : ((t : Nat) -> Prod Action Reward) -> Prod Action Reward :=
    fun y => y (n + 1)
  let extendNext :
      Prod Action Reward -> ((i : Finset.Iic (n + 1)) -> Prod Action Reward) :=
  ...
  have h_extendNext_meas : Measurable extendNext := by
    simpa [extendNext] using
      (History.measurable_extendPairHistorySucc
        (Action := Action) (Reward := Reward) (t := n)).comp
        h_extend_input_meas
  have h_extend_map :
        ...
        @Measure.map
          ...
          (fun y : (t : Nat) -> Prod Action Reward =>
            History.extendPairHistorySucc
              ...
              (y (n + 1)))
          condKernel := by
    rw [Measure.map_map h_extendNext_meas h_nextPair_meas]
    rfl
  calc
    @Measure.map
        ((t : Nat) -> Prod Action Reward)
      ...
      Measure.map extendNext (Measure.map nextPair condKernel) := by
        simpa [condKernel, histPrefix, nextPair, extendNext] using h_extend_map.symm
    _ =
      Measure.map extendNext
        (stepKernel n (Preorder.frestrictLe n trajectory)) := by
        simpa [condKernel, histPrefix, nextPair, stepKernel] using
          congrArg (Measure.map extendNext) h_pair
    _ =
      RewardKernel.actionRewardPartialTrajectoryKernel rewardKernel policy
        context state hcontext hstate n (n + 1)
        (Preorder.frestrictLe n trajectory) := by
        simpa [extendNext, histPrefix, stepKernel] using
          (RewardKernel.actionRewardPartialTrajectoryKernel_succ_extend_map_apply
            rewardKernel policy context state hcontext hstate n
            (Preorder.frestrictLe n trajectory)).symm
...
/--
Canonical `trajMeasure` selected-reward law in `condExpKernel.map` form.

This rewrites
`actionRewardHistoryStepKernelFamily_reward_condExpKernel_map_trajMeasure`
through `RewardKernel.actionRewardHistoryStepKernelFamily_reward_map`, yielding
the selected context/action reward measure directly at the finite pair prefix.
-/
theorem actionRewardHistoryStepKernelFamily_selectedMeasure_condExpKernel_map_trajMeasure
  ...
  have hreward :=
    actionRewardHistoryStepKernelFamily_reward_condExpKernel_map_trajMeasure
      mu0 rewardKernel policy context state hcontext hstate n
  filter_upwards [hreward] with trajectory hreward_eq
  rw [hreward_eq]
...
Ionescu-Tulcea trajectory measure: the selected-action conditional a.e. law
and the selected-reward pushforward law combine to recover the full
`RewardKernel.actionRewardHistoryStepKernelFamily` next-pair law.  Unlike the
direct pair-coordinate `condDistrib` bridge, this route only needs separate
countability of `Action` and `Reward`.
-/
theorem actionRewardHistoryStepKernelFamily_pair_condExpKernel_map_trajMeasure_of_selectedAction_ae_selectedMeasure
  ...
  have hreward :=
    actionRewardHistoryStepKernelFamily_selectedMeasure_condExpKernel_map_trajMeasure
      mu0 rewardKernel policy context state hcontext hstate n
  filter_upwards [haction, hreward] with trajectory h_action h_reward
  let condKernel : Measure ((t : Nat) -> Prod Action Reward) :=
    @ProbabilityTheory.condExpKernel
      ((t : Nat) -> Prod Action Reward) inferInstance _ trajMeasure _
      ((inferInstance :
  ...
  let selectedAction : Action :=
    (policy n).action (state n (Preorder.frestrictLe n trajectory))
  let selectedReward : Measure Reward :=
    RewardKernel.selectedMeasure rewardKernel
      (context n (Preorder.frestrictLe n trajectory)) selectedAction
  have h_nextReward : Measurable nextReward := by
    exact measurable_snd.comp (measurable_pi_apply (n + 1))
  have hsplit :
      @Measure.map ((t : Nat) -> Prod Action Reward) (Prod Action Reward)
          inferInstance inferInstance
          (fun y : (t : Nat) -> Prod Action Reward =>
            (nextAction y, nextReward y)) condKernel =
        Measure.map (Prod.mk selectedAction) selectedReward := by
    exact
      pair_map_eq_map_prod_mk_of_action_ae_eq_const_reward_map_eq
        condKernel nextAction nextReward selectedAction selectedReward
        h_nextReward h_action
        (by
          simpa [condKernel, nextReward, selectedReward, selectedAction,
            trajMeasure] using h_reward)
  calc
    @Measure.map ((t : Nat) -> Prod Action Reward) (Prod Action Reward)
        inferInstance inferInstance
        (fun y : (t : Nat) -> Prod Action Reward => y (n + 1))
        ...
        =
      Measure.map (Prod.mk selectedAction) selectedReward := by
        simpa [condKernel, nextAction, nextReward] using hsplit
    _ = stepKernel n (Preorder.frestrictLe n trajectory) := by
        simpa [stepKernel, selectedAction, selectedReward] using
...
/--
Center a real-valued conditional reward law by a quantity measurable in the
conditioning sigma-algebra.

The conditional kernel freezes the center at the conditioning point.  The
resulting residual pushforward is therefore the centered pushforward of the
supplied reward law, which can be consumed by the generic conditional
sub-Gaussian map-law theorem.
-/
theorem hasCondSubgaussianMGF_centered_of_condExpKernel_map_eq
    ...
    (mu : Measure Omega) [IsFiniteMeasure mu]
    (mcond : MeasurableSpace Omega) (hm : mcond <= mOmega)
    (X center : Omega -> Real) (c : NNReal)
    (hX : @Measurable Omega Real mOmega inferInstance X)
    (hcenter : @Measurable Omega Real mcond inferInstance center)
    (target : Omega -> Measure Real)
    (h_kernel_map_eq :
      Filter.Eventually
        (fun omega : Omega =>
    ...
    (h_target_subG :
      Filter.Eventually
        (fun omega : Omega =>
          ProbabilityTheory.HasSubgaussianMGF
            (fun z : Real => z - center omega) c (target omega))
        (ae (mu.trim hm))) :
    ProbabilityTheory.HasCondSubgaussianMGF mcond hm
      (fun omega => X omega - center omega) c mu := by
  let condKernel :=
    @ProbabilityTheory.condExpKernel Omega mOmega _ mu _ mcond
  have hcenter_map :=
    condExpKernel_map_eq_dirac_of_measurable
      (mOmega := mOmega) mu mcond hm center hcenter
  have h_residual_map :
      Filter.Eventually
        (fun omega : Omega =>
          @Measure.map Omega Real mOmega inferInstance
              (fun y => X y - center y) (condKernel omega) =
            Measure.map (fun z : Real => z - center omega)
              (target omega))
        (ae (mu.trim hm)) := by
    filter_upwards [h_kernel_map_eq, hcenter_map] with omega hreward hcenter_eq
    have hcenter_ae :
        Filter.EventuallyEq (ae (condKernel omega)) center
          (fun _y : Omega => center omega) :=
      eventuallyEq_const_of_map_eq_dirac
        (mOmega := mOmega) (condKernel omega) center (center omega)
        (hcenter.mono hm le_rfl) hcenter_eq
    calc
      @Measure.map Omega Real mOmega inferInstance
          (fun y => X y - center y) (condKernel omega) =
          @Measure.map Omega Real mOmega inferInstance
            (fun y => X y - center omega) (condKernel omega) := by
            apply Measure.map_congr
            filter_upwards [hcenter_ae] with y hy
            rw [hy]
      _ = Measure.map (fun z : Real => z - center omega)
          (@Measure.map Omega Real mOmega inferInstance X
            (condKernel omega)) := by
            symm
            simpa [Function.comp_def] using
              (Measure.map_map
                (measurable_id.sub_const (center omega)) hX)
      _ = Measure.map (fun z : Real => z - center omega)
          (target omega) := by rw [hreward]
  apply hasCondSubgaussianMGF_of_condExpKernel_map_eq (mOmega := mOmega)
    mu mcond hm (fun omega => X omega - center omega) c
    (hX.sub (hcenter.mono hm le_rfl))
    (fun omega =>
      Measure.map (fun z : Real => z - center omega) (target omega))
    h_residual_map
  filter_upwards [h_target_subG] with omega hsub
  exact
    (ProbabilityTheory.HasSubgaussianMGF.id_map_iff
      (measurable_id.sub_const (center omega)).aemeasurable).2 hsub

/--
Transport conditional sub-Gaussianity across equal conditioning measurable
spaces.  The two inclusion proofs are propositionally irrelevant.
-/
theorem hasCondSubgaussianMGF_congr_measurableSpace
    {Omega : Type u}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    (mu : Measure Omega) [IsFiniteMeasure mu]
    (mcond mcond' : MeasurableSpace Omega)
    (hm : mcond <= mOmega) (hm' : mcond' <= mOmega)
    (hspaces : mcond = mcond')
    (X : Omega -> Real) (c : NNReal)
    (h :
      ProbabilityTheory.HasCondSubgaussianMGF
        (mΩ := mOmega) mcond hm X c mu) :
    ProbabilityTheory.HasCondSubgaussianMGF
      (mΩ := mOmega) mcond' hm' X c mu := by
  subst mcond'
  simpa only using h
