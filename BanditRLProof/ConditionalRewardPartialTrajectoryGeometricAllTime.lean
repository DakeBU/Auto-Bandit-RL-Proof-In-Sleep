...
fixed-successor-horizon random-pull-count empirical-mean tail at one geometric
confidence share per time and arm. The compiled finite-index geometric
all-time adapter then controls the union over every positive successor horizon
and every arm in a nonempty finite action type.

The result is a model-specific confidence producer for the canonical generated
trajectory. It is not a UCB policy, pull-count regret argument, maximal
inequality, optional-stopping theorem, self-normalized bound, or general
Freedman inequality.
-/
...
/-- The union, over every positive successor horizon and every finite arm, of
the canonical positive-random-count empirical-mean failures at equal per-arm
geometric confidence shares. -/
noncomputable def successorArmEmpiricalMeanFintypeGeometricAllTimeBadEvent
...
/-- On one canonical generated action/reward trajectory, all positive
successor horizons and all arms in a nonempty finite action type satisfy the
random-count empirical-mean confidence radius outside one event of outer
measure at most `ENNReal.ofReal delta`. -/
theorem actionRewardHistoryStepKernelFamily_successorArmEmpiricalMean_simultaneous_fintype_geometricAllTime_abs_tail_ennreal_delta_trajMeasure
  ...
  have htail : forall n arm,
    ...
    intro n arm
    have hshare :
        0 < Concentration.geometricConfidenceShare delta n /
          (Fintype.card Action : Real) :=
      div_pos (Concentration.geometricConfidenceShare_pos hdelta n) hcard
  ...
  simpa [pairContext, pairState, hpairContext, hpairState, stepKernel, mu,
    action, reward, bad,
    successorArmEmpiricalMeanFintypeGeometricAllTimeBadEvent] using
