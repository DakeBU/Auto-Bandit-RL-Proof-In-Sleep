...
/-!
# Generated realized EXP3 regret with variance-sensitive mixed-square confidence

This module composes the generated predictable Bernstein-square regret theorem
with the one-sided realized-minus-exploration deviation tail. The resulting
four-event route controls generated selected scalar loss while using the
mixed-square second-moment coefficient `K / epsilon`.

The square control is fixed-tilt with a deterministic variance coefficient,
and the realized deviation retains its bounded-loss Hoeffding/Azuma radius.
This is not a general Freedman or ideal EXP3.P theorem.
-/
...
variance-sensitive mixed estimator-square event and two Bernstein confidence
radii. -/
noncomputable def sampledPredictableBernsteinSquareRealizedHighProbabilityRegretBudget
...
/-- Raw four-event form. The predictable component contributes the
variance-sensitive mixed-square event and two Bernstein confidence events; the
fourth event is the bounded realized-minus-predictable deviation. -/
theorem sampledPredictable_bernsteinSquareRealizedHighProbabilityRegret_tail
  ...
  have hpredictableTail :
    ...
    have h := sampledPredictable_bernsteinSquareHighProbabilityRegret_tail
      prior arms harms eta gamma heta hgamma_pos hgamma_lt_one loss comparator
        hcomparator horizon deltaSquare deltaConfidence hdeltaSquare hdeltaConfidence
    dsimp only at h
...
/-- Total-failure form: the mixed-square, pure-cross Bernstein,
