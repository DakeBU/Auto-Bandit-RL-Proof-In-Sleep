...
/--
An action marginal and the conditional law of its feedback determine the joint
action/feedback law. This is the measure-level composition used by the zeroth
step of an `IsAlgEnvSeq`-shaped process contract.
-/
theorem pair_map_eq_compProd_of_map_eq_of_condDistrib
    {Omega Action Feedback : Type*}
    [MeasurableSpace Omega] [MeasurableSpace Action]
    [MeasurableSpace Feedback] [StandardBorelSpace Feedback]
    [Nonempty Feedback]
    (mu : Measure Omega) [IsFiniteMeasure mu]
    (action : Omega -> Action)
    (feedback : Omega -> Feedback) (hfeedback : Measurable feedback)
    (p0 : Measure Action)
    (feedbackKernel : ProbabilityTheory.Kernel Action Feedback)
    [ProbabilityTheory.IsFiniteKernel feedbackKernel]
    (haction : Measure.map action mu = p0)
    (hcond : ProbabilityTheory.condDistrib feedback action mu =ᵐ[mu.map action]
      feedbackKernel) :
    Measure.map (fun omega : Omega => (action omega, feedback omega)) mu =
      p0 ⊗ₘ feedbackKernel := by
  have hjoint :
      Measure.map (fun omega : Omega => (action omega, feedback omega)) mu =
        Measure.map action mu ⊗ₘ feedbackKernel :=
    (ProbabilityTheory.condDistrib_ae_eq_iff_measure_eq_compProd
      action hfeedback.aemeasurable feedbackKernel).mp hcond
  rwa [haction] at hjoint

/--
The successor action policy and the feedback law conditional on
`(history, action)` combine into the conditional law of the observable
`(action, feedback)` pair given history. The proof is the joint-law
characterization of `condDistrib` followed by associativity of `compProd`.
-/
theorem condDistrib_pair_ae_eq_compProd_of_split
    {Omega History Action Feedback : Type*}
    ...
    (feedbackKernel : ProbabilityTheory.Kernel (History × Action) Feedback)
    [ProbabilityTheory.IsMarkovKernel feedbackKernel]
    (hactionCond : ProbabilityTheory.condDistrib action history mu =ᵐ[mu.map history]
      policy)
    (hfeedbackCond : ProbabilityTheory.condDistrib feedback
        (fun omega : Omega => (history omega, action omega)) mu =ᵐ[
          mu.map (fun omega : Omega => (history omega, action omega))]
      feedbackKernel) :
    ProbabilityTheory.condDistrib
        (fun omega : Omega => (action omega, feedback omega)) history mu =ᵐ[
          mu.map history]
      policy ⊗ₖ feedbackKernel := by
  refine
    (ProbabilityTheory.condDistrib_ae_eq_iff_measure_eq_compProd
      history (haction.prod hfeedback).aemeasurable
      (policy ⊗ₖ feedbackKernel)).mpr ?_
  have hactionJoint :
      Measure.map (fun omega : Omega => (history omega, action omega)) mu =
        Measure.map history mu ⊗ₘ policy :=
    (ProbabilityTheory.condDistrib_ae_eq_iff_measure_eq_compProd
      history haction.aemeasurable policy).mp hactionCond
  have hfeedbackJoint :
      Measure.map
          (fun omega : Omega =>
            ((history omega, action omega), feedback omega)) mu =
        Measure.map (fun omega : Omega => (history omega, action omega)) mu ⊗ₘ
          feedbackKernel :=
    (ProbabilityTheory.condDistrib_ae_eq_iff_measure_eq_compProd
      (fun omega : Omega => (history omega, action omega))
      hfeedback.aemeasurable feedbackKernel).mp hfeedbackCond
  calc
    Measure.map
        (fun omega : Omega =>
          (history omega, (action omega, feedback omega))) mu =
        (Measure.map
          (fun omega : Omega =>
            ((history omega, action omega), feedback omega)) mu).map
          MeasurableEquiv.prodAssoc := by
      rw [Measure.map_map MeasurableEquiv.prodAssoc.measurable
        ((hhistory.prod haction).prod hfeedback)]
      rfl
    _ = (Measure.map
          (fun omega : Omega => (history omega, action omega)) mu ⊗ₘ
            feedbackKernel).map MeasurableEquiv.prodAssoc := by
      rw [hfeedbackJoint]
    _ = ((Measure.map history mu ⊗ₘ policy) ⊗ₘ
          feedbackKernel).map MeasurableEquiv.prodAssoc := by
      rw [hactionJoint]
    _ = Measure.map history mu ⊗ₘ (policy ⊗ₖ feedbackKernel) :=
