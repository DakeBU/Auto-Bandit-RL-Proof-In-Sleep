...
```lean
def ConditionalExpectationReward.generatedActionRandomPairDefinitionalMapSource_of_generatedActionDefinitionalActualRewardMapSource
    {Omega : Type u} {Context : Type v} {State : Type w} {Action : Type x}
    [mOmega : MeasurableSpace Omega] [StandardBorelSpace Omega]
    [MeasurableSpace Context] [MeasurableSpace State]
    [MeasurableSpace Action] [MeasurableSingletonClass Action]
...
- Exact Lean-facing statement: a
  `GeneratedActionDefinitionalActualRewardMapSource`, plus measurable
  reward-history context extractors, builds the stronger
  `GeneratedActionRandomPairDefinitionalMapSource` over
  `generatedActionFromRewardHistory`.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.GeneratedActionDefinitionalActualRewardMapSource`,
  ...
  `History.finitePairHistoryOfTrace`,
  `History.pairHistoryRewardProjection`,
  `RewardKernel.actionRewardPartialTrajectoryKernel`, and Mathlib
  `partialTraj`/`Measure.map` interfaces already imported by the module.
- Intended proof route: first consume the definitional actual reward-coordinate
  source to obtain the full finite-pair `partialTraj` law over
  `generatedActionFromRewardHistory`; then feed that law, `hcontext`, and
  `source.hstate` to the existing definitional random-pair source constructor.
- Regularity contracts: standard Borel sample space, finite measure,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable reward trace, measurable reward-history
  context extractor, source-provided measurable reward-history state
  extractor, and the packaged definitional actual-action reward-coordinate
...
- Status: project-local compiled source-conversion leaf for
  ...
  `KERNEL-POLICY-BIND`, and `KERNEL-REWARD`.
- Failure policy: this is a composition/packaging wrapper.  It does not prove
  the definitional actual reward-coordinate law source, does not identify the
  ambient trajectory law with `condExpKernel`, and does not derive
  raw/mean-range regularity, integrability, conditional MGF witnesses,
  martingale witnesses, or final adaptive bandit/regret theorems.
...
```lean
def ConditionalExpectationReward.generatedActionRandomPairDefinitionalMapSource_of_reward_map_eq_selected_policy
    ...
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
    (hreward : forall t : Nat,
...
- Exact Lean-facing statement: a policy-selected reward-coordinate
  selected-measure law, context/state measurability, and timewise reward
  measurability build the bare
  `GeneratedActionRandomPairDefinitionalMapSource` over
  `generatedActionFromRewardHistory`.  This is below the raw-range and
  conditional-MGF source layers.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionFromRewardHistory`,
  `ConditionalExpectationReward.generatedActionFromRewardHistory_measurable`,
  ...
  `History.extendPairHistorySucc`, and
  `RewardKernel.actionRewardPartialTrajectoryKernel`.
- Intended proof route: rewrite the policy-facing selected-measure law with
  `simp [generatedActionFromRewardHistory]` to obtain the actual successor
  action reward law, lift that law through the generated-action frozen-prefix
  extension-map `partialTraj` route, and feed the resulting extension-map law
  into the existing definitional random-pair map source constructor.
- Regularity contracts: standard Borel sample space, finite measure,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable reward trace, measurable context and state
  extractors, and the policy-selected reward-coordinate `condExpKernel`
  selected-measure law.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-REWARD-MAP-TO-DEFINITIONAL-RANDOM-PAIR-MAP-SOURCE`;
  declaration is
  ...
  `LOCAL-LEAF-COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-MAP-SOURCE-CONTRACT`,
  `LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-EXTEND-MAP-CONSUMER`,
  `LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-EXTEND-MAP-REWARD-MAP`, and
  `LOCAL-LEAF-COND-EXPECT-REWARD-ACTION-FREEZE-GENERATED-TRACE-SOURCE`.
...
- Failure policy: this is not an ambient trajectory-to-`condExpKernel`
  theorem, not a raw/mean range source, not a variance-ceiling derivation, not
  a conditional MGF theorem, and not a final adaptive theorem.  It still
  assumes the policy-selected reward-coordinate selected-measure law.

`LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-DEFINITIONAL-MAP-SOURCE-RAW-RANGE-MEAN-ZERO`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_condExp_eq_zero_of_reward_map_eq_selected_policy_definitionalMapSource_rawRangeMeasurableMeanRangeBounded
    ...
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
    (hmean :
...
- Exact Lean-facing statement: the policy-selected reward-coordinate
  selected-measure law plus raw reward range, selected-mean range, and
  centered reward-kernel regularity yield ordinary succ-indexed conditional
  mean-zero through the bare `GeneratedActionRandomPairDefinitionalMapSource`
  route.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionRandomPairDefinitionalMapSource_of_reward_map_eq_selected_policy`,
...
- Intended proof route: construct the bare definitional random-pair map source
  from `h_reward_map_eq_policy`, then pass that source, raw reward bounds,
  selected mean bounds, and the centered kernel law to the existing
  source-level raw/mean range mean-zero consumer.
- Regularity contracts: standard Borel sample space, finite measure,
  measurable context/state/action spaces, measurable singleton and countable
  ...
  deterministic raw reward bounds, deterministic selected-mean bounds, and the
...
- Status: project-local compiled mean-zero consumer leaf for
  `COND-EXPECT-REWARD`, `ADAPTED-ACTION`, `MEAS-POLICY`, `MEAS-HISTORY`,
  `KERNEL-POLICY-BIND`, and `KERNEL-REWARD`.
- Failure policy: this is not an ambient trajectory-to-`condExpKernel`
  ...
  reward-coordinate law and deterministic raw/mean range regularity.

`LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-DEFINITIONAL-MAP-SOURCE-UNIFORM-VARIANCE-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_selected_policy_definitionalMapSource_rawRangeMeasurableMeanRangeUniformVarianceBounded
    ...
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
    (hmean :
...
- Exact Lean-facing statement: the policy-selected reward-coordinate
  ...
  law, and global variance-proxy ceiling yield the succ-indexed
  `HasCondSubgaussianMGF` witness through the bare
  `GeneratedActionRandomPairDefinitionalMapSource` route.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionRandomPairDefinitionalMapSource_of_reward_map_eq_selected_policy`,
...
- Intended proof route: construct the bare definitional random-pair map source
  ...
  source, add the global variance ceiling, and reuse the existing source-level
  uniform-variance conditional MGF consumer.
- Regularity contracts: standard Borel sample space, finite measure,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable reward trace, measurable context/state
  extractors, measurable selected-mean surface, centered reward-kernel law,
  deterministic raw reward bounds, deterministic selected-mean bounds, global
  variance-proxy ceiling, and the policy-selected reward-coordinate
...
- Failure policy: this is not an ambient trajectory-to-`condExpKernel`
  theorem, not a derivation of the model-side variance ceiling, and not a
...
`LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-DEFINITIONAL-MAP-SOURCE-UNIFORM-VARIANCE-LARGER-PROXY-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_selected_policy_definitionalMapSource_rawRangeMeasurableMeanRangeUniformVarianceBounded_of_varianceCeiling_le
    ...
...
- Exact Lean-facing statement: the policy-selected reward-coordinate
  selected-measure law plus raw/mean range regularity, centered reward-kernel
  law, a global variance-proxy ceiling, and `varianceCeiling <= c` yield the
  succ-indexed `HasCondSubgaussianMGF` witness at the coarser deterministic
  proxy `c` through the bare
  `GeneratedActionRandomPairDefinitionalMapSource` route.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionRandomPairDefinitionalMapSource_of_reward_map_eq_selected_policy`,
  `ConditionalExpectationReward.GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource`,
...
- Intended proof route: construct the bare definitional random-pair map source
  ...
  mean bounds, and the centered kernel law as the definitional raw/mean-range
  source, add the global variance ceiling, and reuse the existing source-level
  uniform-variance coarser-proxy conditional MGF consumer with `hceiling`.
- Regularity contracts: standard Borel sample space, finite measure,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable reward trace, measurable context/state
  extractors, measurable selected-mean surface, centered reward-kernel law,
  deterministic raw reward bounds, deterministic selected-mean bounds, global
  variance-proxy ceiling, coarser-proxy domination `varianceCeiling <= c`, and
  the policy-selected reward-coordinate `condExpKernel` selected-measure law.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-DEFINITIONAL-MAP-SOURCE-UNIFORM-VARIANCE-LARGER-PROXY-COND-MGF`;
  declaration is
  ...
  `MLIB-CONDITIONAL-EXPECTATION`, and
  `MLIB-PROBABILITY-SUBGAUSSIAN`.
- Status: project-local compiled coarser-proxy conditional-MGF consumer leaf
  for `COND-EXPECT-REWARD`, `ADAPTED-ACTION`, `MEAS-POLICY`,
  `MEAS-HISTORY`, `KERNEL-POLICY-BIND`, `KERNEL-REWARD`, and
  `TAIL-COND-SUBGAUSS`.
- Failure policy: this is not an ambient trajectory-to-`condExpKernel`
  theorem, not a derivation of the model-side variance ceiling, and not a
  final concentration or adaptive regret theorem.  It still assumes the
  policy-selected reward-coordinate law plus deterministic raw/mean range,
  variance, and coarser-proxy contracts.
...
`LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-DEFINITIONAL-MAP-SOURCE-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_selected_policy_definitionalMapSource_rawRangeMeasurableMeanRangeHistoryVarianceBounded_of_varianceCeiling_le
    ...
    (varianceCeiling : Nat -> NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Exact Lean-facing statement: the policy-selected reward-coordinate
  ...
  `c` satisfying `varianceCeiling i <= c`, through the bare
  `GeneratedActionRandomPairDefinitionalMapSource` route.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionRandomPairDefinitionalMapSource_of_reward_map_eq_selected_policy`,
  `ConditionalExpectationReward.GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource`,
...
- Intended proof route: construct the bare definitional random-pair map source
  ...
  mean bounds, and the centered kernel law as the definitional raw/mean-range
  source, add the time-indexed selected-history variance ceiling, and reuse
  the existing source-level coarser-proxy history-variance conditional MGF
  consumer with `hceiling`.
- Regularity contracts: standard Borel sample space, finite measure,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable reward trace, measurable context/state
  extractors, measurable selected-mean surface, centered reward-kernel law,
  deterministic raw reward bounds, deterministic selected-mean bounds,
  selected finite-history variance-proxy ceilings, coarser-proxy domination
  `varianceCeiling i <= c`, and the policy-selected reward-coordinate
  `condExpKernel` selected-measure law.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-DEFINITIONAL-MAP-SOURCE-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`;
  declaration is
...
- Status: project-local compiled coarser-proxy conditional-MGF consumer leaf
  ...
  `TAIL-COND-SUBGAUSS`.
- Failure policy: this is not an ambient trajectory-to-`condExpKernel`
  theorem, not a derivation of selected-history variance ceilings or their
  coarser proxy, and not a final concentration or adaptive regret theorem.  It
  still assumes the policy-selected reward-coordinate law plus deterministic
  raw/mean range and selected-history variance contracts.
...
```lean
def ConditionalExpectationReward.generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_generatedActionDefinitionalActualRewardMapSource
    ...
    (rewardLo rewardHi meanLo meanHi : Nat -> Real)
    (varianceCeiling : NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
...
- Exact Lean-facing statement: a packaged
  ...
  measurability, and a global model-side varianceProxy ceiling construct the
  packaged
  `GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource`.
  This is the source layer needed by the uniform-variance conditional MGF
  consumers.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.GeneratedActionDefinitionalActualRewardMapSource`,
  `ConditionalExpectationReward.GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource`,
...
- Intended proof route: first build the base definitional
  ...
  source, then store the global variance bound in the `variance_le` field of
  the uniform-variance source structure.
- Regularity contracts: standard Borel sample space, finite measure,
  measurable context/state/action spaces, measurable singleton and countable
  ...
  bounds, and a global varianceProxy ceiling.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-DEFINITIONAL-ACTUAL-REWARD-MAP-SOURCE-UNIFORM-VARIANCE-SOURCE`;
  declaration is
...
- Failure policy: this is not a proof of the packaged reward-coordinate law,
  ...
  of the variance ceiling from reward ranges, not the conditional MGF witness
  itself, and not a final adaptive theorem.  It assumes the packaged
...
```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionDefinitionalActualRewardMapSource_definitionalRawRangeMeasurableMeanRangeUniformVarianceBounded
    ...
    (rewardLo rewardHi meanLo meanHi : Nat -> Real)
    (varianceCeiling : NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
...
- Exact Lean-facing statement: a packaged
  `GeneratedActionDefinitionalActualRewardMapSource`, raw/mean range
  regularity, centered kernel law, measurable mean surface, context
  measurability, and a global model-side varianceProxy ceiling directly yield
  the succ-indexed `HasCondSubgaussianMGF` witness for the centered selected
  reward with proxy `varianceCeiling`.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.GeneratedActionDefinitionalActualRewardMapSource`,
  `ConditionalExpectationReward.generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_generatedActionDefinitionalActualRewardMapSource`,
  `ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource`,
...
- Intended proof route: build the packaged definitional
  raw-range/measurable-mean-range uniform-variance source from the packaged
...
- Failure policy: this is not a proof of the packaged reward-coordinate law,
  not an ambient trajectory-to-`condExpKernel` identification, not a derivation
  of the global variance ceiling from reward ranges, and not a final adaptive
  theorem.  It assumes the packaged definitional actual reward-map source,
  deterministic raw/mean range regularity, and model-side varianceProxy
  ceiling.
...
```lean
def ConditionalExpectationReward.generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeBoundedSource_of_reward_map_eq_actual_action
    ...
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
    (hmean :
...
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  ...
  `History.extendPairHistorySucc`, and
  `RewardKernel.actionRewardPartialTrajectoryKernel`.
- Intended proof route: derive generated-action timewise measurability from
  the reward history, use the actual-action reward-coordinate law to build the
  frozen-prefix extension-map `partialTraj` law, then feed that law plus
  raw/mean range regularity into the existing base source constructor.
- Regularity contracts: finite measure, standard Borel sample space,
  ...
  actual-action reward-coordinate `condExpKernel` selected-measure law.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-ACTUAL-REWARD-MAP-RAW-RANGE-MEASURABLE-MEAN-RANGE-BOUNDED-SOURCE`;
  declaration is
...
```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_actionRewardPartialTrajectoryKernel_map_eq_definitionalRawRangeMeasurableMeanRangeUniformVarianceBounded_of_varianceCeiling_le
    ...
    (varianceCeiling : NNReal)
    ...
    (hvariance :
...
- Exact Lean-facing statement: a full finite-pair generated-history
  `partialTraj` law, raw reward range, selected mean range, centered kernel
  law, and a global context/action variance ceiling directly yield the
  succ-indexed `HasCondSubgaussianMGF` witness at any deterministic proxy `c`
  satisfying `varianceCeiling <= c`.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_actionRewardPartialTrajectoryKernel_map_eq`,
  `ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_varianceCeiling_le`,
  `ConditionalExpectationReward.generatedActionFromRewardHistory`,
  ...
  `RewardKernel.actionRewardPartialTrajectoryKernel`, and
  `ProbabilityTheory.HasCondSubgaussianMGF`.
- Intended proof route: build the packaged
  `GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource`
  from the full finite-pair `partialTraj` law and raw/mean range regularity,
  then reuse the packaged uniform-source larger-proxy conditional MGF
  consumer with `hceiling : varianceCeiling <= c`.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range, global
  context/action variance ceiling, the full generated finite-pair
  `partialTraj`/`condExpKernel` law, and `varianceCeiling <= c`.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-UNIFORM-VARIANCE-LARGER-PROXY-COND-MGF`;
  declaration is
...
- Status: project-local compiled conditional-MGF consumer for
  ...
  `MEAS-REWARD`.
- Failure policy: this is not the ambient trajectory-law proof, not a
  variance-ceiling derivation, and not a final adaptive theorem.  It only
  weakens the exact global-variance proxy to a larger deterministic `c` and
  still assumes the full generated finite-pair `partialTraj`/`condExpKernel`
  identity plus a global model-side variance ceiling.
...
```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_actionRewardPartialTrajectoryKernel_map_eq_definitionalRawRangeMeasurableMeanRangeHistoryVarianceBounded_of_varianceCeiling_le
    ...
    (varianceCeiling : Nat -> NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Exact Lean-facing statement: a full finite-pair generated-history
  `partialTraj` law, raw reward range, selected mean range, centered kernel
  law, and selected finite-history variance ceilings directly yield the
  succ-indexed `HasCondSubgaussianMGF` witness at any deterministic proxy
  `c` satisfying `varianceCeiling i <= c`.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource_of_actionRewardPartialTrajectoryKernel_map_eq`,
  `ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource_of_varianceCeiling_le`,
  `ConditionalExpectationReward.generatedActionFromRewardHistory`,
  ...
  `RewardKernel.actionRewardPartialTrajectoryKernel`, and
  `ProbabilityTheory.HasCondSubgaussianMGF`.
- Intended proof route: build the packaged history-variance source from the
  full finite-pair `partialTraj` law and raw/mean range regularity, then reuse
  the packaged selected-history source coarser-proxy consumer with
  `hceiling`.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range,
  selected-history variance ceiling, the full finite-pair
  `partialTraj`/`condExpKernel` law, and the explicit proxy domination
  contract `varianceCeiling i <= c`.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`;
  declaration is
...
- Failure policy: this is not the ambient trajectory-law proof, not a
  variance-ceiling derivation, and not a final adaptive theorem.  It still
  assumes the full generated finite-pair `partialTraj`/`condExpKernel`
  identity, selected-history variance ceilings, and the caller-supplied
  coarser-proxy domination proof.
...
`LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-EXTEND-MAP-UNIFORM-VARIANCE-LARGER-PROXY-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_actionRewardPartialTrajectoryKernel_extend_map_eq_definitionalRawRangeMeasurableMeanRangeUniformVarianceBounded_of_varianceCeiling_le
    ...
    (varianceCeiling : NNReal)
    ...
    (hvariance :
...
- Intended proof route: build the packaged
  `GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource`
  from the frozen-prefix extension-map `partialTraj` law and raw/mean range
  regularity, then reuse the packaged uniform-source larger-proxy conditional
  MGF consumer with `hceiling : varianceCeiling <= c`.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range, global
  context/action variance ceiling, the frozen-prefix extension-map
  `partialTraj`/`condExpKernel` law, and `varianceCeiling <= c`.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-EXTEND-MAP-UNIFORM-VARIANCE-LARGER-PROXY-COND-MGF`;
  declaration is
...
- Failure policy: this is not the ambient trajectory-law proof, not a
  variance-ceiling derivation, and not a final adaptive theorem.  It only
  weakens the exact global-variance proxy to a larger deterministic `c` and
  still assumes the frozen-prefix extension-map `partialTraj`/`condExpKernel`
  identity plus a global model-side variance ceiling.
...
```lean
def ConditionalExpectationReward.generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_reward_map_eq_actual_action
    ...
    (varianceCeiling : NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Exact Lean-facing statement: an actual-action reward-coordinate
  selected-measure law, raw reward range, selected mean range, centered kernel
  law, and a global context/action variance ceiling build the packaged
  `GeneratedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource`.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionFromRewardHistory`,
  `ConditionalExpectationReward.generatedActionFromRewardHistory_measurable`,
...
- Intended proof route: derive the frozen-prefix extension-map `partialTraj`
  law from the generated-action actual reward-coordinate law using the existing
  reward-map-to-extension-map adapter.  Then call the compiled frozen-prefix
  extension-map uniform-variance source constructor.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
...
- Retrieval evidence: local card
  ...
  It reuses
  `LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-EXTEND-MAP-UNIFORM-VARIANCE-SOURCE`
  and sits beside the actual-action uniform-variance conditional MGF consumer.
...
```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_actual_action_definitionalRawRangeMeasurableMeanRangeHistoryVarianceBounded
    ...
    (varianceCeiling : Nat -> NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Intended proof route: derive the frozen-prefix extension-map `partialTraj`
  law from the generated-action actual reward-coordinate law using the existing
  reward-map-to-extension-map adapter.  Then call the compiled frozen-prefix
  extension-map history-variance conditional MGF consumer.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
...
`LOCAL-LEAF-COND-EXPECT-REWARD-ACTUAL-REWARD-MAP-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_actual_action_definitionalRawRangeMeasurableMeanRangeHistoryVarianceBounded_of_varianceCeiling_le
    ...
    (varianceCeiling : Nat -> NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  ...
  `ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource_of_varianceCeiling_le`,
  `History.historyFiltrationSucc`,
  `History.finiteRewardHistoryOfTrace`, `RewardKernel.selectedMeasure`, and
  `ProbabilityTheory.HasCondSubgaussianMGF`.
- Intended proof route: build the packaged actual-action history-variance
  source from the selected-measure law and regularity contracts, then reuse the
  packaged selected-history source coarser-proxy consumer with `hceiling`.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range, selected
  finite-history variance ceiling, the actual-action reward-coordinate
  `condExpKernel` selected-measure law, and the explicit proxy domination
  contract `varianceCeiling i <= c`.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-ACTUAL-REWARD-MAP-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`;
  declaration is
...
- Failure policy: this is not a reward-coordinate law constructor, not an
  ambient trajectory-to-`condExpKernel` theorem, not a selected-history
  variance-ceiling derivation, and not a final adaptive theorem.  It assumes
  the actual-action selected-measure law, selected-history variance ceilings,
  and the caller-supplied coarser-proxy domination proof.

`LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-UNIFORM-VARIANCE-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_selected_policy_definitionalRawRangeMeasurableMeanRangeUniformVarianceBounded
    ...
    (varianceCeiling : NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  ...
  `RewardKernel.selectedMeasure`, and
  `ProbabilityTheory.HasCondSubgaussianMGF`.
- Intended proof route: rewrite the policy-facing law at
  `(policy i).action (state i history)` into the generated successor-action
  law with `simpa [generatedActionFromRewardHistory]`, then reuse the
  actual-action reward-map uniform-variance conditional MGF consumer.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range, global
  context/action variance ceiling, and the policy-selected reward-coordinate
  `condExpKernel` selected-measure law.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-UNIFORM-VARIANCE-COND-MGF`;
  declaration is
...
- Failure policy: this is not a reward-coordinate law constructor, not an
  ambient trajectory-to-`condExpKernel` theorem, not a variance-ceiling
  derivation, and not a final adaptive theorem.  It assumes the
  policy-selected selected-measure law and a global model-side variance
  ceiling.
...
```lean
def ConditionalExpectationReward.generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_reward_map_eq_selected_policy
    ...
    (varianceCeiling : NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Intended proof route: rewrite the policy-facing selected-measure law at
  `(policy i).action (state i history)` into the generated successor-action
  law by `simpa [generatedActionFromRewardHistory]`.  Then call the compiled
  actual-action reward-map uniform-variance source constructor.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
...
- Retrieval evidence: local card
  ...
  It reuses
  `LOCAL-LEAF-COND-EXPECT-REWARD-ACTUAL-REWARD-MAP-UNIFORM-VARIANCE-SOURCE`
  and sits immediately below the policy-selected uniform-variance conditional
  MGF consumer.
- Status: project-local compiled source constructor for `COND-EXPECT-REWARD`,
  `ADAPTED-ACTION`, `MEAS-POLICY`, `MEAS-HISTORY`, `KERNEL-POLICY-BIND`,
  `KERNEL-REWARD`, `INT-REWARD-BOUNDED`, and `MEAS-REWARD`.
...
`LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-UNIFORM-VARIANCE-LARGER-PROXY-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_selected_policy_definitionalRawRangeMeasurableMeanRangeUniformVarianceBounded_of_varianceCeiling_le
    ...
    (varianceCeiling : NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Exact Lean-facing statement: a policy-selected reward-coordinate
  ...
  law, and a global context/action variance ceiling directly yield the
  succ-indexed `HasCondSubgaussianMGF` witness at any deterministic proxy
  `c` satisfying `varianceCeiling <= c`.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_reward_map_eq_selected_policy`,
  `ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeUniformVarianceBoundedSource_of_varianceCeiling_le`,
  `History.historyFiltrationSucc`,
  `History.finiteRewardHistoryOfTrace`, `RewardKernel.selectedMeasure`, and
  `ProbabilityTheory.HasCondSubgaussianMGF`.
- Intended proof route: build the packaged policy-selected uniform-variance
  source from the selected-measure law and regularity contracts, then reuse the
  packaged uniform-source coarser-proxy consumer with `hceiling`.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range, global
  context/action variance ceiling, the policy-selected reward-coordinate
  `condExpKernel` selected-measure law, and the explicit proxy domination
  contract `varianceCeiling <= c`.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-UNIFORM-VARIANCE-LARGER-PROXY-COND-MGF`;
  declaration is
...
- Failure policy: this is not a reward-coordinate law constructor, not an
  ambient trajectory-to-`condExpKernel` theorem, not a variance-ceiling
  derivation, and not a final adaptive theorem.  It assumes the
  policy-selected selected-measure law, a global model-side variance ceiling,
  and the caller-supplied coarser-proxy domination proof.
...
```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_selected_policy_definitionalRawRangeMeasurableMeanRangeHistoryVarianceBounded
    ...
    (varianceCeiling : Nat -> NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Intended proof route: rewrite the policy-facing law at
  `(policy i).action (state i history)` into the generated successor-action
  law with `simpa [generatedActionFromRewardHistory]`, then reuse the
  actual-action reward-map history-variance conditional MGF consumer.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range,
  selected finite-history variance ceiling, and the policy-selected
  reward-coordinate `condExpKernel` selected-measure law.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-HISTORY-VARIANCE-COND-MGF`;
  declaration is
...
- Failure policy: this is not a reward-coordinate law constructor, not an
  ambient trajectory-to-`condExpKernel` theorem, not a selected-history
  variance-ceiling derivation, and not a final adaptive theorem.  It assumes
  the policy-selected selected-measure law and selected-history variance
  ceilings.
...
```lean
def ConditionalExpectationReward.generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource_of_reward_map_eq_selected_policy
    ...
    (varianceCeiling : Nat -> NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Intended proof route: rewrite the policy-facing selected-measure law at
  `(policy i).action (state i history)` into the generated successor-action
  law by `simpa [generatedActionFromRewardHistory]`.  Then call the compiled
  actual-action reward-map history-variance source constructor.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
...
- Retrieval evidence: local card
  ...
  It reuses
  `LOCAL-LEAF-COND-EXPECT-REWARD-ACTUAL-REWARD-MAP-HISTORY-VARIANCE-SOURCE`
  and sits immediately below the policy-selected history-variance conditional
  MGF consumer.
- Status: project-local compiled source constructor for `COND-EXPECT-REWARD`,
  `ADAPTED-ACTION`, `MEAS-POLICY`, `MEAS-HISTORY`, `KERNEL-POLICY-BIND`,
  `KERNEL-REWARD`, `INT-REWARD-BOUNDED`, and `MEAS-REWARD`.
...
`LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_reward_map_eq_selected_policy_definitionalRawRangeMeasurableMeanRangeHistoryVarianceBounded_of_varianceCeiling_le
    ...
    (varianceCeiling : Nat -> NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  ...
  `History.finiteRewardHistoryOfTrace`, `RewardKernel.selectedMeasure`, and
  `ProbabilityTheory.HasCondSubgaussianMGF`.
- Intended proof route: build the packaged policy-selected history-variance
  source from the selected-measure law and regularity contracts, then reuse the
  packaged selected-history source coarser-proxy consumer with `hceiling`.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  ...
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range,
  selected finite-history variance ceiling, the policy-selected
  reward-coordinate `condExpKernel` selected-measure law, and the explicit
  proxy domination contract `varianceCeiling i <= c`.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-SELECTED-POLICY-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`;
  declaration is
...
- Failure policy: this is not a reward-coordinate law constructor, not an
  ambient trajectory-to-`condExpKernel` theorem, not a selected-history
  variance-ceiling derivation, and not a final adaptive theorem.  It assumes
  the policy-selected selected-measure law, selected-history variance ceilings,
  and the caller-supplied coarser-proxy domination proof.
...
`LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-EXTEND-MAP-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_actionRewardPartialTrajectoryKernel_extend_map_eq_definitionalRawRangeMeasurableMeanRangeHistoryVarianceBounded_of_varianceCeiling_le
    ...
    (varianceCeiling : Nat -> NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  ...
  `ProbabilityTheory.HasCondSubgaussianMGF`.
- Intended proof route: build the packaged history-variance source from the
  frozen-prefix extension-map `partialTraj` law and raw/mean range regularity,
  then reuse the packaged selected-history source coarser-proxy consumer with
  `hceiling`.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range,
  selected-history variance ceiling, the frozen-prefix extension-map
  `partialTraj`/`condExpKernel` law, and the explicit proxy domination
  contract `varianceCeiling i <= c`.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-EXTEND-MAP-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`;
...
`LOCAL-LEAF-COND-EXPECT-REWARD-HISTORYSTEP-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`
is compiled locally:

```lean
theorem ConditionalExpectationReward.centeredReward_succ_hasCondSubgaussianMGF_of_actionRewardHistoryStepKernelFamily_pair_map_eq_definitionalRawRangeMeasurableMeanRangeHistoryVarianceBounded_of_varianceCeiling_le
    ...
    (varianceCeiling : Nat -> NNReal)
    (hcontext : forall n : Nat, Measurable (context n))
    (hstate : forall n : Nat, Measurable (state n))
...
- Exact Lean-facing statement: a canonical generated-history history-step
  ...
  and time-indexed selected-history variance ceiling directly imply the
  succ-indexed `HasCondSubgaussianMGF` witness for the centered successor
  reward at any deterministic proxy `c` satisfying
  `varianceCeiling i <= c`.
- Local APIs/imports: `BanditRLProof.ConditionalRewardLawSource`,
  `ConditionalExpectationReward.generatedActionFromRewardHistory`,
  `ConditionalExpectationReward.generatedActionFromRewardHistory_measurable`,
...
- Intended proof route: package the canonical history-step next-pair law into
  ...
  via
  `generatedActionRandomPairDefinitionalRawRangeMeasurableMeanRangeHistoryVarianceBoundedSource_of_actionRewardHistoryStepKernelFamily_pair_map_eq`,
  then invoke the already compiled selected-history larger-proxy source
  consumer with `hceiling : varianceCeiling i <= c`.
- Regularity contracts: finite measure, standard Borel sample space,
  measurable context/state/action spaces, measurable singleton and countable
  action space, timewise measurable rewards, measurable context and state
  extractors, measurable mean surface, centered reward-kernel law,
  deterministic raw reward range, deterministic selected mean range,
  time-indexed selected-history variance ceiling, a deterministic coarser
  proxy `c`, and the canonical history-step next-pair `condExpKernel` law.
- Retrieval evidence: local card
  `LOCAL-LEAF-COND-EXPECT-REWARD-HISTORYSTEP-HISTORY-VARIANCE-LARGER-PROXY-COND-MGF`;
  declaration is
...
- Status: project-local compiled conditional MGF coarser-proxy consumer for
  `COND-EXPECT-REWARD`, `ADAPTED-ACTION`, `MEAS-POLICY`, `MEAS-HISTORY`,
  `KERNEL-POLICY-BIND`, `KERNEL-REWARD`, `INT-REWARD-BOUNDED`, and
  `MEAS-REWARD`.
- Failure policy: this is not a proof of the canonical history-step next-pair
  law and not a final adaptive theorem.  It still assumes the history-step
  pair-map `condExpKernel` identity and a model-side selected-history
  variance ceiling; it only weakens the proxy from `varianceCeiling i` to a
  larger deterministic `c`.
...
```lean
theorem OFUL.sum_range_prefix_update_le_two_log_det_sub_base_of_update_le_one
    {Feature : Type u} [Fintype Feature] [DecidableEq Feature]
    (lambda : Real) (hlambda : 0 < lambda)
    (history : Nat -> Feature -> Real) (T : Nat)
    (hupdate_le_one : forall t : Nat, t < T ->
...
- Exact Lean-facing statement: under `0 < lambda` and the explicit
  small-update contract `x_t^T V_t^{-1} x_t <= 1` for every `t < T`, the raw
  inverse-quadratic update sum is bounded by the log-det telescope endpoint
  `2 * (log det(V_T) - log(lambda^d))`.
- Local APIs/imports: `BanditRLProof.OFULEllipticalPotential`,
  `OFUL.regularizedPrefixFeatureGram`,
  `OFUL.sum_range_min_prefix_update_le_two_log_det_sub_base_of_pos_lambda`,
  and existing finite-sum, matrix, and logarithm APIs imported by the OFUL
  module.
- Intended proof route: define `u t = x_t^T V_t^{-1} x_t`; prove
  `sum_t u t = sum_t min 1 (u t)` by `Finset.sum_congr` and
  `min_eq_right (hupdate_le_one t ht)`; then reuse the clipped log-det
  endpoint consumer
  `OFUL.sum_range_min_prefix_update_le_two_log_det_sub_base_of_pos_lambda`.
- Regularity contracts: finite feature type with decidable equality, positive
  regularization `0 < lambda`, and explicit small-update evidence
  `dotProduct (history t) (V_t^{-1} * history t) <= 1` for every `t < T`.
- Retrieval evidence: local card
  `LOCAL-LEAF-OFUL-UNCLIPPED-SMALL-UPDATE-LOG-DET-SUB-BASE`; declaration is
  `OFUL.sum_range_prefix_update_le_two_log_det_sub_base_of_update_le_one`;
  dependency cards include `LOCAL-LEAF-OFUL-INVERSE-QUADRATIC-NONNEG-CONSUMER`,
  `LOCAL-LEAF-OFUL-PREFIX-LOG-DET-BASE`, and `MLIB-FINSET-SUMS`.
- Status: project-local compiled deterministic OFUL/LinUCB
  log-det-endpoint-to-raw-sum handoff.
- Failure policy: this does not prove terminal determinant upper bounds,
  trace/AM-GM simplifications, the small-update contract, self-normalized
  martingale concentration, confidence ellipsoids, or final OFUL/LinUCB
  regret.
...
`LOCAL-LEAF-OFUL-UNCLIPPED-SMALL-UPDATE-DIM-SCALED` is compiled locally:

```lean
theorem OFUL.sum_range_prefix_update_le_two_trace_average_dim_scaled_of_update_le_one
    {Feature : Type u} [Fintype Feature] [DecidableEq Feature]
    [Nonempty Feature]
    (lambda : Real) (hlambda : 0 < lambda)
...
- Exact Lean-facing statement: under finite nonempty feature type, positive
  ...
  by the pre-cancellation dimension-scaled endpoint
  `2 * (d * (T*L2/(d*lambda)))`.
- Local APIs/imports: `BanditRLProof.OFULEllipticalPotential`,
  `OFUL.regularizedPrefixFeatureGram`,
  `OFUL.sum_range_prefix_update_le_two_of_trace_average_bound_of_update_le_one`,
  `OFUL.trace_average_pow_le_lambda_pow_mul_exp_dim_scaled`, and existing
  finite-sum, matrix, exp, and order APIs imported by the OFUL module.
- Intended proof route: instantiate the generic small-update raw-sum handoff
  with `B := (Fintype.card Feature : Real) *
  ((T * L2) / ((Fintype.card Feature : Real) * lambda))`; discharge the scalar
  trace-average certificate using
...
- Retrieval evidence: local card
  `LOCAL-LEAF-OFUL-UNCLIPPED-SMALL-UPDATE-DIM-SCALED`; declaration is
  ...
  `LOCAL-LEAF-OFUL-SCALAR-TRACE-AVERAGE-EXP-BOUND`, and
  `MLIB-FINSET-SUMS`.
- Status: project-local compiled deterministic OFUL/LinUCB
  elliptical-potential consumer.
- Failure policy: this preserves the pre-cancellation deterministic endpoint
  only.  It does not prove the small-update contract, self-normalized
  martingale concentration, confidence ellipsoids, least-squares estimation,
  or final OFUL/LinUCB regret.
...
```lean
theorem OFUL.sum_range_prefix_update_le_two_trace_average_log_of_update_le_one
    {Feature : Type u} [Fintype Feature] [DecidableEq Feature]
    [Nonempty Feature]
    (lambda : Real) (hlambda : 0 < lambda)
    (history : Nat -> Feature -> Real) (T : Nat) (L2 : Real)
...
- Exact Lean-facing statement: the raw inverse-quadratic update sum
  `sum_t x_t^T V_t^{-1} x_t` is bounded by the standard logarithmic
  trace/radius endpoint when each update scalar is at most one.  This removes
  the `min 1` clipping only under the explicit small-update contract.
- Local APIs/imports: `BanditRLProof.OFULEllipticalPotential`,
  `OFUL.regularizedPrefixFeatureGram`,
  `OFUL.sum_range_min_prefix_update_le_two_trace_average_log`,
  `Mathlib.Data.Real.Basic`, `Mathlib.Data.Matrix.Basic`, and
  `Mathlib.Data.Finset.Basic` through the existing module imports.
- Intended proof route: define the update scalar `u t`; prove
  `sum_t u t = sum_t min 1 (u t)` by `Finset.sum_congr` and
  `min_eq_right (hupdate_le_one t ht)`; reuse the compiled clipped theorem
  `OFUL.sum_range_min_prefix_update_le_two_trace_average_log`.
- Regularity contracts: finite nonempty feature type, decidable feature
  equality, positive regularization `0 < lambda`, nonnegative feature-radius
  budget `0 <= L2`, pointwise squared-feature bound
  `dotProduct (history t) (history t) <= L2` for `t < T`, and the explicit
  small-update assumption
  `dotProduct (history t) (V_t^{-1} * history t) <= 1` for `t < T`.
- Retrieval evidence: local card
  `LOCAL-LEAF-OFUL-UNCLIPPED-SMALL-UPDATE-LOG`; declaration is
  `OFUL.sum_range_prefix_update_le_two_trace_average_log_of_update_le_one`;
  dependency cards include `LOCAL-LEAF-OFUL-SCALAR-TRACE-AVERAGE-EXP-LOG`,
  `LOCAL-LEAF-OFUL-INVERSE-QUADRATIC-NONNEG-CONSUMER`, and
  `MLIB-FINSET-SUMS`.
- Status: project-local compiled deterministic OFUL/LinUCB elliptical-potential
  consumer.
- Failure policy: this is not a self-normalized martingale tail, not a
  confidence ellipsoid, and not an OFUL regret theorem.  It only converts the
  already-compiled clipped logarithmic determinant-growth bound to an
  unclipped raw-sum bound under `u_t <= 1`; any route needing arbitrary
  unclipped sums must first prove or assume that small-update contract.
