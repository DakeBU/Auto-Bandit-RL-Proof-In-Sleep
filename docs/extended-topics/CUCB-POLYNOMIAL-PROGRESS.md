...
2026-09-18. Update: both exact Theorem2 branches have now compiled; see CUCB-THEOREM-TWO-PROGRESS.md. The following records their prerequisite construction, not independent acceptance. Repaired Theorem1 remains compiled. These declarations construct the actual cutoff bound and scalar ingredients needed for its exact distribution-independent endpoint.
...
`PowerTailIntegral` proves the exact finite power-tail integral bound for q>1 and positive endpoints, the balancing identity K*((K/N)^(1/q))^(-q)=N, and the resulting objective coefficient q/(q-1). These scalar identities do not themselves constitute Theorem2. The source instantiation must use q=2/omega, derive all positivity conditions, choose the cutoff, handle H=1 and cutoff>=Delta_max, integrate/sum the actual threshold envelopes and rewrite into the exact frozen gamma/log/H exponents and coefficients. Both full terminals have since compiled. Remaining acceptance includes concrete noisy canaries, independent source/repair acceptance, shared mappings and all-topic ICLR evidence. No experiment, merge or deployment is implied.
