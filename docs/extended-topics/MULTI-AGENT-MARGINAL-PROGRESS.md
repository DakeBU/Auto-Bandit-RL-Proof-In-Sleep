# Actual finite path marginals and conditional coordination performance

Source: Rosenski, Shamir and Szlak, *Multi-Player Bandits -- a Musical Chairs
Approach*, ICML 2016, static Algorithm 2 and supplement A.1 Lemma 4, consumed
inside the frozen unknown-population static Algorithm 1 contract. The previous
numeric-gap exploration probability and disclosed all-player failure-budget
repair are unchanged. This packet connects the actual learned-candidate draw
path to the accepted recursive coordination performance calculation. It proves
conditional coordination pseudo-regret, not the full learner regret theorem.

## The actual process and its marginals

The path law is the existing FinitePMF.iid(jointDraw(C),L). Private candidate
sets C are nonempty; they need not agree for the generic marginal identities.
The existing trajectory starts every player unfixed, reads draws in chronological
order, fixes an unfixed player after a collision-free played round, and retains
every fixed arm thereafter. The previously constructed continuationAction uses
that actual state and current draw. Fixed players ignore later private draws.

The reusable iid_sum_snoc and iid_snoc split the existing finite product PMF
into its first T coordinates and one fresh final draw. They are proved by finite
sum reindexing with Fin.snocEquiv and product factorization, not postulated.
iid_take proves the prefix marginal for every T <= L, including T=0 and T=L.
Two reusable finite real expectation identities transport map and bind; finiteness
of PMF masses justifies ENNReal.toReal over the finite sums.

The trajectory prefix and last-step identities consume the original update.
iid_trajectory_stateLaw identifies the length-T path's terminal state PMF with
the existing stateLaw(C,T). iid_trajectory_marginal extends this to every T<=L
inside one length-L path. iid_last_state_draw and iid_round_state_draw prove
that the pre-round state/current draw joint PMF is exactly

    stateLaw(C,t).bind (s => jointDraw(C).map (a => (s,a))).

Thus the current private draw is fresh relative to the state at that round for
each fixed C. This is not an independence assertion about played actions,
which depend on permanent fixation, nor unconditional independence from the
exploration history. iid_continuationAction_marginal maps this joint law through
the actual action function and identifies the played-action marginal.

For every exploration sample z, including bad histories,
learnedDrawPathKernel_state_marginal and learnedDrawPathKernel_round_marginal
identify the corresponding measure pushforwards with these PMFs at the actual
learnedConfig(z). State has an explicit discrete measurable space and measurable
singletons. No supplied distribution-equality premise is used. Padding a finite
path after L cannot affect any state at T<=L or any action at t<L.

## Real path regret and the accepted performance theorem

For arbitrary reference arm set A and scores mu, pathCoordinationRegret is the
finite REAL sum over t:Fin L of

    sum(a in A, mu(a))
      - sum(i, if CollisionFree(actualAction(t),i)
               then mu(actualAction(t,i)) else 0).

It uses the real mean deficit, not a clipped positive-part loss. Its generic
expectation identity imposes no nonnegativity on mu or the deficit.
iid_round_regret_expectation and iid_path_regret_expectation transport this
actual path functional to the existing stateLaw/current-draw weighted sums.
Only when applying iid_path_regret_le do we require all players' candidates
to be the same nonempty n-arm set A and mu in [0,1]. The already accepted
expectedCoordinationRegret_real_le then gives expected path regret <= 8*n^2
for every finite L, including zero. The constant is the earlier proved stronger
coordination component; it is not presented as a literal source constant.

## Actual exploration joint law and normalized event expectation

Let P be the existing explorationContinuationLaw, formed by compProd of the
actual exploration action/reward measure and actual learned-candidate path
kernel. Let G(A) be the actual exploration event: every player estimates the
correct population and returns A. For any measurable E subset G(A), the new
restricted-path measure identity proves

    map(snd, P restricted to fst^{-1}(E)) = P_explore(E) * Q_A,

where Q_A is the actual common-A iid draw-path measure and the right side is
measure scalar multiplication. This follows from the previously proved exact
rectangle factorization and holds even when P_explore(E)=0. Mapping the finite
real path functional gives an exact restricted integral identity. Finite path
space makes that function integrable; this is not a conditional realized-reward
identity and introduces no future reward law.

If P_explore(E)>0, it is also finite because the exploration law is a probability
measure. The actual joint marginal has the same mass. Real-valued cancellation
therefore gives

    integral_{fst^{-1}(E)} pathCoordinationRegret / P(fst^{-1}(E)) <= 8*n^2.

This is a normalized event expectation, not a regular conditional distribution
or conditioning on a singleton continuous history. Positivity of G does not
assert positivity of arbitrary subsets; the generic E theorem explicitly needs
its own positive mass.

source_conditional_coordination_regret instantiates this at A=trueTopArms(nu,n),
mu=armMean(nu), E=G(A), and the exact explorationLength. It DERIVES A's size and
nonemptiness, mean bounds, G measurability and positive probability from the
previous producer under the primitive source assumptions: 0<n<k, fixed
probability arm laws supported a.e. on [0,1], 0<eps<actual numeric boundary gap,
and 0<delta<1. The exploration coordinates retain independent arm/round reward
samples and independent private action coins. No correct-candidate premise,
external separating set, guessed population, or assumed good-event probability
remains in this source-facing coordination endpoint.

## Boundary diagnostics and validation scope

Six canary theorems include zero horizon, a real two-round transient deficit
5/4 after an initial collision followed by separation, zero deficit when already
fixed players ignore colliding later private draws, the actual two-player path
expectation bound 32, allowed single-draw mass 1/4, and exact probability 1/16
for that transient path under the actual learned kernel at the earlier concrete
three-round exploration fixture. Every transient draw is inside the learned
candidate set. The positive path mass rules out an impossible-coin diagnostic.
The fixture is not a full stochastic exploration experiment or a demonstration
of a positive true gap for its constant-one reward values.

The focused scratch compiled cleanly. This packet adds 32 scanned declarations
(29 theorems, three definitions), plus two measurable-state instances. Axiom
and distinct blind/source review evidence is bound in
runs/extended-topics-20260920/multi-agent-marginal-review.json. Full shared public
root, Tests, harness, registry and site gates have not been rerun for this scratch.

## Remaining source endpoint

The state/action marginal and conditional coordination-performance bridge are
now supplied. Still required: actual global exploration/coordination phase splice
for every total horizon (especially horizons shorter than exploration), global
true-top comparator optimality and pathwise bounds on bad histories, the initial
exploration cost and unconditional delta*n*T residual, source residual/corollary,
any future reward law needed for expected realized reward, the full learner
stochastic canary, production extraction/integration, recent-source review and
all-topic ICLR evidence. This is not topic completion, main integration or live
publication. All 10 topics remain within the persistent whole-program Goal.

<details>
<summary>Exact Lean scratch, including unchanged prior handoff context</summary>
