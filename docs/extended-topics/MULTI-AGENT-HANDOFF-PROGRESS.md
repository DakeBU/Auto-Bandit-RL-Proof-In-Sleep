# Numeric source gap and actual learned-candidate continuation law

Source: Rosenski, Shamir and Szlak, *Multi-Player Bandits -- a Musical Chairs
Approach*, ICML2016, main static Algorithm1/Definition1/Theorem1 and supplement
A.1 Lemmas2--4. Frozen scope and all-player failure-budget repair remain in
MULTI-AGENT-CONTRACT.md. This packet supplies the exact numeric source-gap
interface and an actual finite exploration/continuation draw law, including its
normalized good-event restriction. It does not yet identify played-action/state
marginals with the existing coordination regret calculation or prove full regret.

## Exact numeric boundary gap

rankedArm(score,j) is entry j of the actual fixed-tie score-descending list;
rankIndex is its inverse, implemented by idxOf. Both inverse identities are
proved. Membership in topArms(score,n) is exactly rankIndex<n. The sorted-list
pairwise property gives antitone ranked scores.

For0<n<k define

    boundaryGap(score,n) = score(rankedArm(score,n-1))
                          - score(rankedArm(score,n)).

These are precisely the source Nth and (N+1)th means using zero-based indices.
The validity proofs prevent out-of-range indexing. The gap is nonnegative;
for a in the first n set and b outside it, the actual sorted scores imply
boundaryGap<=score(a)-score(b).

explorationGoodEvent_orderStatistic_probability now instantiates the previous
cross-set theorem at trueTopArms(nu,n), derives its cardinality and all cross-set
inequalities, and uses only0<eps<boundaryGap(armMean(nu),n). No external
separating set or numerical lower-gap witness remains in its premises. For
arbitrary fixed arm probability laws supported a.e. on[0,1], independent across
arms/rounds and independent of exploration coins,0<n<k and0<delta<1, it proves
actual correct Nhat/common top-n candidates with probability>=1-delta at

    S = ceil_nat(max{(16k/eps^2) log(4k^2/delta),
                     50k^2 log(4k/delta)}).

The source delta is the previously disclosed extra k in the second logarithm.
The final theorem derives eps<=1 through the prior bounded-mean/gap argument.
explorationGoodEvent_positive also proves this event has strictly positive
probability; the later normalized-event formula is not dividing by zero.

## Actual measurable learned configuration

CandidateConfig(n,k) is the finite subtype of per-player arm sets with every
set nonempty, equipped explicitly with the discrete sigma algebra. learnedConfig
applies the ACTUAL localCandidateSet to each player's local feedback. Previous
all-history nonemptiness supplies the subtype proof even on bad histories and
at zero abstract exploration duration; k>1 is retained.

measurableSet_actualCandidate_eq proves every equality fiber for a local
candidate output measurable. It combines the finite action-dependent Nhat size
condition with finitely many measurable real-score comparisons; no correct-N
or good-event premise is required. Taking finite intersections of fibers proves
the whole learnedConfig measurable. True n appears as the number of player
coordinates in the joint model, not as a local learner input.

## Fresh draws and actual joint law

For a configuration C and a future horizon L, configDrawPathKernel(C) is the
measure of the ACTUAL FinitePMF.iid(jointDraw(C),L). jointDraw is the existing
independent uniform Cartesian-product draw on each nonempty local candidate
set. This is not a supplied probability/independence premise.
learnedDrawPathKernel is its measurable pullback along learnedConfig. Every
history, including bad histories, therefore has a normalized probability law.

The induced path measure is proved exactly equal to the product over L copies
of jointDraw(C). iIndepFun_pi proves independence of its time coordinates for
every fixed exploration history. Within-round private-draw factorization is
inherited from the existing jointDraw construction. These are latent draws:
a fixed player ignores subsequent candidate draws and plays its permanent arm.
No unconditional independence of future draws from exploration is claimed;
their law generally depends on the learned configuration.

explorationContinuationLaw is the actual measure/kernel composition

    explorationRewardLaw compProd learnedDrawPathKernel.

It is a probability measure on pairs (exploration record, future draw path).
Its exploration marginal is proved equal to explorationRewardLaw. Thus the
numeric-gap good-event probability also holds in this combined law.

On explorationGoodEvent(S), learnedConfig is exactly the common configuration
with every player's candidate set S. Consequently, for ANY measurable
E subset explorationGoodEvent(S) and any future-draw event B,

    P_joint(E x B) = P_exploration(E) * P_iid_common_S(B).

This is proved from compProd's integral and the pointwise common-kernel identity.
If P_exploration(E)>0, division by the ACTUAL lifted event probability gives

    P_joint(E x B) / P_joint(fst in E) = P_iid_common_S(B).

This is an exact normalized event-probability formula, with finite nonzero
denominator proved/required explicitly. It is not a regular condDistrib object
or a claim about conditioning on a zero-probability individual real history.
For E=G, positivity comes from the numeric-gap probability theorem. Nor does
this identify conditional realized reward with pseudo-regret; future reward
arrays and their separate conditional expectation connection are still absent.

## Actual finite continuation actions and causality

extendedCoordinationDraws extends a finite future-draw path by a dummy arm0
after L. continuationAction at t:Fin L uses the existing trajectory from its
all-unfixed initial state and action(state,draw). The trajectory step permanently
fixes a player at its first collision-free coordination play, preserving old
fixed arms even when later collisions occur. The initial reset is the source
coordination phase after exploration; exploration itself does not fix players.

trajectory_eq_of_prefix and continuationAction_prefix prove that the action at
t depends only on draws through t. The arbitrary dummy extension after L cannot
enter any earlier state through this prefix dependency. The fixed-stays theorem
connects the actual action to a fixed arm. This supplies a total causal finite
continuation action map; it does not yet splice it into a single full-horizon
learner with exploration actions, or identify its state/action distribution with
stateLaw for the regret proof.

## Concrete checks and scope

Seven canaries check: the scrambled numeric boundary gap1/4; an allowed tie
inside the top set with boundary gap1/2; zero gap for a constant score vector;
the actual two-player local configuration from the previous three-round
transcript equals the common{0,1} family; its future path law equals iid common
sampling for EVERY finite L; actual first coordination actions equal their
private draws; and after first-round separation on0/1, the second-round actions
stay0/1 even though the new private draws are both0. These later latent draws
remain inside the candidate set, so this fixation diagnostic is not manufactured
using impossible candidate draws. No atom mass or full stochastic learner result
is claimed by these examples.

The numeric-gap interface requires0<n<k. Generic finite kernels support L=0,
which gives the probability law on the unique empty draw path. learnedConfig
requires k>1 and is meaningful on all histories, without a population-accuracy
premise. The joint probability instance additionally requires probability arm
laws; range bounds are required for the good-event guarantee. No time-varying
reward laws, reward-dependent exploration sampling or absent collision sensing
are covered.

The new layer contains41 scanned declarations (29theorems,12definitions) plus
six named instances, for47 audited names. The prior ranking code is retained
unchanged beneath the new measure/kernel import. Receipt:
runs/extended-topics-20260920/multi-agent-handoff-review.json. Independent
blind/source review and compilation/axiom evidence are separate and hash-bound.
This is scratch-only, without a new public-root/Tests/harness/site gate claim.

Required next: actual causal path/state/action marginal identities feeding the
existing coordination performance theorem, global true-top comparator bounds,
full phase splice and same-process conditional/unconditional regret including
delta*n*T, future reward/expected realized-regret connection, full-learner
canary, shared extraction/public gates, recent-source audit and all-topic ICLR
evidence. Neither this kernel construction nor an old fixed-set regret consumer
alone completes the multi-agent topic or the ten-topic Goal.

<details>
<summary>Exact scratch Lean, including unchanged dependency context</summary>
