# Actual collision concentration and joint exploration statistics

Source: Rosenski, Shamir and Szlak, *Multi-Player Bandits -- a Musical Chairs
Approach*, ICML2016, static Algorithm1 and supplement A.1 Lemma3, together with
the reviewed random-count mean proof and all-player failure allocation in
MULTI-AGENT-CONTRACT.md and MULTI-AGENT-PROOF-ROUTE.md. The second exploration
logarithm is the explicitly disclosed repair log(4k/delta); the source proof's
per-player event is insufficient for an all-player conclusion without this
union. The first term is unchanged. This is a scratch component, not a complete
unknown-population algorithm or a public-root integration.

## Exact construction and concentration

The previous independent exploration PMF is retained exactly. A new generic
identity proves that FinitePMF.iid(p,T).toMeasure equals the actual finite
product Measure.pi of p.toMeasure, by equality of all singleton masses.
For any event E in the finite discrete action space, its real indicator lies
in[0,1]. Its integral is the ACTUAL event probability, and evaluation of the
product measure preserves this mean at every time. Mathlib's bounded-variable
MGF theorem gives centered sub-Gaussian proxy1/4. iIndepFun_pi derives temporal
independence; no independence of collision indicators across players is used.

The centered sum has proxyT/4. The positive and negative tail events combine
to bound the closed absolute tail by2exp(-u^2/(2(T/4))). The exact event-count
sum identity converts u=T*eta, for T>0 and eta>=0, into

    P(abs(eventCount(E)/T - p(E)) >= eta) <= 2 exp(-2T eta^2).

Here E is instantiated as a collision of the tagged player in an actual joint
uniform draw. Its mean is proved equal to

    p = 1 - (((k-1 : Nat) : Real)/k)^(n-1).

For k>0 this is the source expression1-(1-1/k)^(n-1). The local collision
rate is defined directly from the collision bits in explorationFeedback;
localCollisionRate_eq identifies it with collisionCount/T for EVERY raw
reward array. Thus this statistic does not inspect unobserved rewards.
With eta=1/(10k), the actual exploration law satisfies

    P(abs(localCollisionRate_i-p) >= 1/(10k))
      <= 2 exp(-T/(50k^2)).

Union over n players, then n<=k, gives2k exp(-T/(50k^2)). If
T>=50k^2 log(4k/delta), this is at mostdelta/2. The event allCollisionAccurate
uses STRICT error<1/(10k); it is exactly the complement of the union of closed
bad events. Its probability is at least1-ENNReal.ofReal(delta/2).

## Joint action/reward law and actual duration

explorationRewardLaw_fst_event proves that pulling any action-schedule event
back to the constructed action/reward product retains its exact probability.
The collision bound therefore holds on the SAME actual law as the previously
accepted arbitrary-[0,1]-reward simultaneous mean bound. A union, with no
independence assertion between the two statistics events, gives failure<=delta.
explorationStatisticsAccurate explicitly requires both:

- every player's every arm mean has absolute error<eps/2;
- every player's local collision rate has absolute error<1/(10k).

The final construction sets the natural exploration length to

    S = ceil_nat(max{(16k/eps^2) log(4k^2/delta),
                     50k^2 log(4k/delta)}).

Both budget inequalities are derived from this actual definition. For k>0 and
0<delta<1, S>0 follows from the second term, so no positive-duration premise is
left to an external caller. For n<=k, arbitrary Borel probability measures
nu_a supported a.e. on[0,1], 0<eps<=1 and0<delta<1, the final theorem is

    P_actual_joint(explorationStatisticsAccurate at S) >= 1-delta.

The precise Lean lower bound uses truncated ENNReal subtraction. The generic
finite-T threshold theorems only require delta>0, and can consequently become
vacuous at large delta. The final duration instantiation imposes0<delta<1.
This is a statistical event, NOT a claim of correct Nhat or common top-n set.
The gap condition and log-inverse rounding/ranking producers are still needed.

## Boundaries, reuse and concrete checks

The finite-event theorem requires T>0 to divide the centered-sum identity;
there is no false empty-sample accuracy claim. Earlier sum bounds cover zero
horizons only as loose totalized bounds. k>0 is retained. Tagged statements
require Fin n; the all-player union permits n=0 vacuously. Component n<=k is
more general than the complete source contract1<=n<k. k=n=1 is admissible for
this statistical component; the later inverse logarithm needs its own domain
conditions. Counts and actions are finite and measurable, all indicators are
bounded/integrable, and no stopping-time or anytime guarantee is stated.

The mean component's arbitrary continuous/discrete/mixed reward-law scope is
unchanged. No reward assumptions enter the collision-only theorem. There is no
new conditional-distribution theorem, adaptive-selection theorem or hypothesis
that supplies the desired concentration result.

Three new canaries verify the exact n=2,k=3 mean1/3, the actual two-round mixed
collision feedback rate1/2 for every reward array, and a numerically informative
actual collision-frequency bound at T=9000: failure<=1/10. The last uses the
proved tail and exp(20)>=21; it is not an empirical simulation and does not
stand in for the required complete learner canary.

The new layer contains37 scanned declarations, including28 theorems and nine
definitions, with no new probability instance. It reuses the unchanged previous
reward/exploration scratch as dependency context. Required separate evidence:
runs/extended-topics-20260920/multi-agent-collision-review.json. Independent
semantic review and compiler/axiom audit are recorded there. No new combined
root/Tests/harness/site gate or public mapping is claimed for this scratch.

Remaining: actual inverse-log rounding and Nhat recovery, deterministic empirical
ranking/common true top-n set, full source good event, fresh conditional
coordination continuation, complete learner/regret/canary, shared extraction and
public gates, recent-source audit and all-topic ICLR evidence. No topic-complete
flag changes.
