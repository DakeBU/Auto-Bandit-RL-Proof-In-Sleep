# Global comparator, actual phase splice, and finite-horizon pseudo-regret

Source: Rosenski, Shamir and Szlak, *Multi-Player Bandits -- a Musical Chairs
Approach*, ICML 2016, static Algorithm 1/2, Theorem 1 and supplement A.1. The
frozen contract's repaired all-player exploration budget remains unchanged.
This packet completes the all-horizon expected PSEUDO-regret calculation for
the constructed staged action process. It does not yet supply future stochastic
reward observations, expected realized-regret equality, a complete stochastic
learner canary, public integration, or all mandatory topic acceptance.

## Comparator without successful-exploration premises

For nonnegative real scores, ordered_set_sum_max proves that an ordered finite
set A dominates the score sum of any finite B with card(B)<=card(A), provided
every selected score is at least every unselected score. If A is empty, B is
empty. Otherwise choose a minimum-score element of A. Each element of B\A is
below that threshold and each element of A\B is above it; the first difference
has no larger cardinality. Nonnegative threshold and the shared intersection
then give the sum inequality. This uses no positive gap or correctness premise.

topArms_sum_max applies that reusable fact to the actual fixed-tie sorted top-n
set for n<=k. Ties are allowed. collisionFreePlayers and earnedArms are the actual
collision-free player set and its action image. The action map is injective on
those players, so the earned mean reward is exactly the score sum of earnedArms,
whose cardinality is at most n. Thus, for ANY simultaneous action vector,

    globalRoundRegret(mu,a)
      = sum(b in actual top-n, mu(b))
        - sum(i, if collision-free(a,i) then mu(a_i) else 0)

is nonnegative. When every mu is in [0,1], it is at most n. These bounds apply
to arbitrary bad candidate sets and unreachable state diagnostics as well as
good histories. They require n<=k, not correct population estimates, candidate
agreement, an invariant, or collision-free actions. The existing coordination
roundPseudoRegret at this reference set is exactly this quantity.

## Actual chronological phase splice

Let S be the exploration length and H the requested total horizon. The latent
sample consists of a full S-round exploration action/reward sample and a fresh
learned-candidate draw path of length L=H-S (natural subtraction). learnerAction
at time t<S plays that exploration draw. At time t>=S it uses the original
trajectory started all-unfixed and its current private draw at t-S. At indices
S+t with t:Fin L this is proved equal to continuationAction. Fixed players
therefore keep their fixed arm and ignore later latent draws.

The action function is total on natural time. Its padding beyond S+L is a
mathematical totalization; only t<H is counted. learnerAction_prefix states
that the action depends only on exploration draws at indices <=t and continuation
draws whose shifted indices are <=t. For post-exploration decisions the actual
learned kernel already depends only on completed S-round local feedback. This
packet does not replace that kernel with externally supplied correct candidates.

For H<S, the full latent exploration sample includes unused later coordinates.
The action at any counted t reads only its actual prefix; the continuation path
is empty. Sampling this extension in the probability model does not make future
feedback an action input. A canary changes an unused exploration coordinate and
checks that the observed-prefix regret is unchanged.

learnerRegret is the sum of actual globalRoundRegret for t=0,...,H-1. The exact
split, including the H<S and H=0 cases, is

    learnerRegret = explorationPrefixRegret(min(H,S))
                    + pathCoordinationRegret(L=H-S).

No initial or endpoint round disappears. The exploration part lies in
[0, n*min(H,S)] and total pathwise pseudo-regret lies in [0,n*H]. The split
identifies the true-top reference in both phases with the same fixed mean vector.

## Actual measure, integrability and the bad-event term

The measure is the existing explorationContinuationLaw with T=S and L=H-S:
actual independent exploration actions and fixed bounded arm reward laws,
followed by the measurable iid path kernel at ACTUAL learnedConfig. The real
regret functions depend on finite action/path projections, so their integrability
is proved for any finite measure by mapping to those finite spaces. Their real
expectations are not silently replaced by positive parts or zero values from
nonintegrable Bochner integrals.

For a measurable E contained in the actual candidate-correctness event,
the prior restricted path measure identity and conditional coordination bound
now give the unnormalized inequality

    integral_{first in E} learnerRegret
      <= P_explore(E) * [n*min(H,S) + 8*n^2].

The exploration cost is bounded pathwise, so conditioning on the reward-dependent
E does not require independence of exploration rewards from E. For the complement,
the global bound gives integral_bad <= P(bad)*n*H. Actual exploration marginal
preservation and P(E)>=1-delta imply P(bad)<=delta. Adding the two restricted
integrals, and separately integrating the global n*H bound, gives

    E[learnerRegret]
      <= min(n*H, n*min(H,S) + 8*n^2 + delta*n*H).

The factor P(E) is bounded by one; no conditional expectation is applied to a
null event in this unconditional argument. The generic assembly exposes its
event-probability premise, while the source-facing endpoint actually supplies it
from the earlier exploration producer. Fixed delta retains its linear H residual.

## Primitive-source specialization and source constants

source_expected_learnerRegret_le uses only 0<n<k; fixed probability arm laws nu_a
supported a.e. on [0,1]; 0<eps<the actual numeric sorted boundary gap; and
0<delta<1. It derives the actual top-set cardinality/nonemptiness, bounded means,
measurable event and event-probability bound at

    S = ceil_nat(max{(16k/eps^2)*log(4k^2/delta),
                     50k^2*log(4k/delta)}).

The extra k in the second logarithm is the already frozen all-player repair.
The good event is correct Nhat and canonical top-set agreement, sufficient for
the continuation proof; it is not asserted equal to the source's entire
correct-ranking predicate. Unknown n is a global player-index parameter, not an
input to the actual local estimation/ranking algorithm.

source_expected_learnerRegret_coarse exposes the frozen endpoint

    E[R_H] <= min(n*H, n*S + 8*n^2 + delta*n*H).

The positive-good-event normalized real integral is also bounded by
n*min(H,S)+8*n^2. For H<S, that optional conditioning refers to the full latent
S-round extension, not an event already observed by horizon H. The source-facing
published conditional residual theorem explicitly requires S<=H and gives
n*S+2*exp(2)*n^2. Its comparison follows from exp(1)>=2 and exp(2)=exp(1)^2.
The unconditional published-residual version retains delta*n*H.

For H>=2, source_expected_learnerRegret_inverse_horizon sets delta=1/H, derives
0<delta<1, keeps the exact explorationLength(k,eps,1/H), and proves the failure
term is exactly n. This supplies the corresponding horizon-dependent logarithmic
exploration bound with initial cost and residual intact.

## Canary and validation boundaries

Seven new canary theorems check: a distinct action outside the top set has mean
deficit 1/4; complete collision has deficit 5/4; zero players give zero deficit;
zero horizon gives zero regret; two counted rounds before a three-round
exploration boundary cost 5/2; an unused exploration-coordinate change does not
affect that prefix; and three colliding exploration rounds followed by a
collision/separation coordination transient have total regret 5. These are exact
deterministic semantics checks. They are not a probability assertion about the
whole exploration sample or a complete noisy Bernoulli learner run. The previous
packet's positive-mass continuation diagnostic remains separately scoped.

The new layer has 42 scanned declarations: 33 theorems and nine definitions,
including seven canary theorems. Focused compilation succeeded. A separate axiom
audit and distinct blind/source semantic review are recorded in the hash-bound
runs/extended-topics-20260920/multi-agent-comparator-review.json. The exact prior
marginal context is unchanged below. Public root/Tests/harness/site acceptance
has not been rerun for this scratch.

## Remaining mandatory work

The finite staged-action pseudo-regret endpoints and source residual/corollary
are now proved. Future reward variables and actual post-exploration reward
observations remain absent. Expected realized-regret equality needs a separate
unconditional proof; the reward-dependent exploration good event does not license
conditional realized=pseudo equality. A complete visible local-feedback trace,
full noisy unknown-N learner canary, shared extraction and public gates,
recent-primary-source comparison, and whole-program ICLR evidence remain open.
This packet alone does not complete the multi-agent topic or any of the ten-topic
Goal. It is local scratch evidence, not a main merge or live publication.

<details>
<summary>Exact Lean scratch including unchanged preceding marginal context</summary>
