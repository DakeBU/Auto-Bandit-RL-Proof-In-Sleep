# Actual empirical ranking and the exploration-good event

Source: Rosenski, Shamir and Szlak, *Multi-Player Bandits -- a Musical Chairs
Approach*, ICML2016, static Algorithm1 and supplement A.1 Lemmas2/3. Frozen
scope and explicit probability-budget repair are in MULTI-AGENT-CONTRACT.md
and MULTI-AGENT-PROOF-ROUTE.md. This packet closes the actual exploration
estimate/candidate event on the constructed exploration law; it does not yet
construct the continuation of a complete learner or prove full regret.

## Actual sorting and local candidate construction

scoreOrder(score,a,b) holds if score(a)>score(b), or the scores are equal and
a<=b in the public arm-index order. Its transitivity, antisymmetry and totality
are proved; a classical decidability instance supports this real-valued
mathematical definition. rankedArms uses Finset.univ.sort with that exact
relation. topArms(score,m) converts the first m entries to a finite set.
Thus this is the actual sorted-prefix algorithm, not an abstract oracle that
is assumed to select the desired arms. No numerical executable precision claim
is made for exact real comparisons/logs.

The selected set has cardinality min(m,k). The sorted-list pairwise theorem,
combined with take_append_drop, proves that every selected arm precedes every
unselected arm. Conversely, an m-element set whose members precede every
outside arm is exactly that sorted prefix. Antisymmetry and equal cardinalities
exclude an extra outside element. A strict score-separation specialization is
then available for the source gap argument.

localCandidateSet(f) is EXACTLY topArms(localEmpiricalMean(f),
localPopulationEstimate(f)). Its inputs are the player's local exploration
feedback plus known k and duration, with the previously proved zero-count,
collision sensing, inverse-log rounding, all-collision fallback and cap rules.
True n, means, optimal set and correctness events are not implementation inputs.

populationEstimate_pos proves positivity for k>1 and every valid natural
collision count C<=T, including T=0 and C=T through the fallback. Together
with the global cap and local count bound, the actual local candidate set is
nonempty on EVERY local history for k>1, and its cardinality equals the actual
local population estimate. This includes bad-event histories; it does not
condition a future sampler on eventual exploration success.

## Gap transfer and canonical true top set

Let S be an analytical n-element subset of the k arms, and suppose a positive
number gap lower-bounds every difference mu(a)-mu(b) with a in S,b outside S.
This is an explicit cross-set formulation of the source's boundary separation;
taking gap to be the Nth-versus-(N+1)th ordered mean difference is the intended
source instantiation. It allows ties within S and within its complement.
The packet does not define an order-statistic numeric gap function or claim a
separate theorem evaluating one. Instead it proves that ANY such separating S
is exactly trueTopArms(nu,n), defined by the same fixed-order sort of true means.
Thus the assumption is primitive reward separation, not a supplied learned
candidate set or successful exploration event.

If every empirical score has error<eps/2 and eps<gap, then

    score(a)-score(b) > mu(a)-mu(b)-eps >= gap-eps > 0.

Therefore the actual first n empirical entries form exactly S. Combining this
with the ACTUAL local population estimator=N gives localCandidateSet=S.
The ordering of arms inside S can differ across players without changing the
set. The public tie rule makes every score vector total, including bad paths.

The arm means are proved to lie in[0,1] from the supplied arbitrary Borel
probability laws' a.e. support, with bounded integrability explicit. Since
0<n<k, both S and its complement contain an arm; comparing one such pair gives
gap<=1. Consequently eps<gap implies eps<=1, and the final source-facing theorem
does not ask the caller to supply this extra concentration-domain assumption.

## Measurable actual source event and probability

explorationGoodEvent(S) is exactly the event that, for EVERY player, the actual
local population estimate equals n AND the actual local sorted candidate set
is S. It is not defined to be the earlier sufficient statistics event.

To prove measurability, localCandidateSet_eq_iff characterizes equality with S
by its actual estimated size and finitely many scoreOrder comparisons. For
S.card=n, the full event equals populationRecovered pulled back from the finite
action schedule, intersected with all those comparisons. Strict comparison and
equality of measurable local empirical means are measurable, including ties.
This proves the actual event measurable without assuming a measurable sorting
oracle or silently relying on a false independence claim.

The already constructed explorationEstimatesCorrect event is a subset of this
actual source event. Combining the actual probability producer with this
inclusion proves, for0<n<k, arbitrary fixed arm probability laws supported a.e. on[0,1],
sampled independently across arms and rounds and independently of exploration
action coins, eps>0, eps<gap and0<delta<1,

    P_actual_joint(all local Nhat=n AND all local candidate sets=trueTopArms)
      >= 1-delta,

at the EXACT duration

    S_time = ceil_nat(max{(16k/eps^2) log(4k^2/delta),
                          50k^2 log(4k/delta)}).

The Lean lower bound is1-ENNReal.ofReal(delta). The increased second logarithm
remains the disclosed all-player union repair; the first budget is unchanged.
No main concentration result, correct population or correct-set premise is
supplied to this final theorem. The deterministic local-candidate lemma does
have accuracy/population premises, and its actual probability caller proves
them using the preceding layers rather than declaring them primitive inputs.

## Concrete checks and boundaries

Nine new canary theorems check actual output behavior:

- scores[1/2,3/4,1/4] sort to[1,0,2] and select{0,1};
- scores[3/4,3/4,1/4] sort to[0,1,2], fixing the within-top tie explicitly;
- a zero request selects nothing; a request of7 from3 arms selects all3;
- a two-round local all-collision history produces all3 candidates;
- on the concrete three-round joint action transcript retained from the
  population canary, player0's empirical vector is[1,0,0] and player1's is
  [0,1,0] when all latent rewards are1; both actual local candidate sets equal
  {0,1}, with estimated population2.

The last diagnostic includes collision-zero feedback and never-observed arm
zero-count means. It is a deterministic positive-support action transcript;
no atom mass or high-probability claim for that particular transcript is made.
Its empirical scores are not asserted to satisfy the final theorem's accuracy
premises for a specified reward distribution. These are component canaries,
not the required stochastic full-learner execution.

The generic sort is total for k=0, m=0 and m>k. Candidate nonemptiness requires
k>1 because it uses the valid inverse-estimator domain, and holds even for
T=0. The final source probability endpoint uses0<n<k and the proved positive
explorationLength. There is no stopping-time, adaptive exploration or post-event
conditional reward claim. Measurability here concerns the actual exploration
success event; a general candidate-valued kernel/continuation still needs its
own construction and measurability evidence.

## Acceptance boundary

The new layer has40 scanned declarations (30 theorems and10 definitions) plus
four explicit order/decidability instances; all44 named declarations are audited.
The previous population file is unchanged beneath the new sort import. The
reader contains that complete exact scratch below. Evidence is bound by
runs/extended-topics-20260920/multi-agent-ranking-review.json.

This remains scratch-only: no new shared public-root/Tests/harness/site gate is
claimed. Required next steps are actual fresh coordination continuation from
the learned local candidates (including all bad paths), same-process conditional
and unconditional regret with the delta*n*T residual, full-learner canary,
shared extraction/public acceptance, recent-source audit and all-topic ICLR
evidence. The multi-agent topic and all ten-topic Goal remain incomplete.
