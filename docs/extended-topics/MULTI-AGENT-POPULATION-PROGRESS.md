# Actual local population recovery after exploration

Source: Rosenski, Shamir and Szlak, *Multi-Player Bandits -- a Musical Chairs
Approach*, ICML2016, static Algorithm1 and supplement A.1 Lemma3. This implements
the separately reviewed gamma=2/5 inverse-margin proof in
MULTI-AGENT-PROOF-ROUTE.md and the exact totality conventions in
MULTI-AGENT-CONTRACT.md. The increased second exploration logarithm remains
log(4k/delta), explicitly paying the all-player union. This checkpoint is
scratch-only and does not yet establish candidate ranking or full regret.

## Exact estimator and local inputs

For natural arm count k, duration T and collision count C define

    populationInverse(k,T,C)
      = 1 + log(((T-C : Nat) : Real)/T) / log(1-1/k).
    populationEstimate(k,T,C)
      = if C=T then k else min(k, toNat(round(populationInverse(k,T,C)))).

Mathlib round(x)=floor(x+1/2), so half-integers round towards positive infinity.
Int.toNat clamps negative integers to0; min k caps the resulting natural at k.
The local estimator takes only the player's local feedback history, extracts
its collision bits and calls this definition. True n, arm means, other players'
feedback and correctness events are absent from the estimator's inputs.

The feedback count is proved at most T. For k>1 and C<T the real inverse is
at least1: the survival fraction lies in(0,1], its log is nonpositive, and the
base log is strictly negative. populationEstimate_regular_cast proves that the
natural estimator agrees exactly with integer min(k,round(inverse)) in this
valid regular branch, so toNat is not silently changing the source policy.
All-collision counts return k. For T>0 and k>0, zero collisions return1.
The natural complement (T-C) is converted to1-C/T only with C<=T and T>0.

## Exact inverse margin

Assume1<=n<=k and k>1. Write b=1-1/k and q=b^(n-1). The earlier actual
collision mean is p=1-q. Reusing the proved avoidance lower bound givesq>=1/4.
The upstream real-exponent Bernoulli inequality gives

    b^(2/5) <= 1-(2/5)/k.

Reciprocal algebra with b>0 yields

    b^(-2/5) >= 1+(2/5)/k.

Multiplying both margins by q>=1/4 makes each one at least1/(10k).
Consequently, for any real pHat with abs(pHat-p)<=1/(10k),

    q*b^(2/5) <= 1-pHat <= q*b^(-2/5).

The left side is strictly positive; thus pHat<1 and the logarithm is legitimate.
Taking logs and dividing by log b<0, with the inequality directions reversed,
gives

    n-2/5 <= 1+log(1-pHat)/log b <= n+2/5.

This entire closed interval is strictly inside the half-integer rounding cell
[n-1/2,n+1/2). The rounded integer therefore equals n. The cap does nothing
because n<=k. For the actual count C, the accuracy premise rules out C=T;
hence the definition's fallback cannot be used to fake estimator correctness.
No Taylor approximation, injected population estimate or correct-N premise
appears in the proof.

## Actual probability endpoints

populationRecovered is the finite action-schedule event that all player counts
produce estimate n. The prior allCollisionAccurate event is proved a subset.
For positive T, delta>0 and the second budget

    T >= 50k^2 log(4k/delta),

its ACTUAL exploration-law probability is at least1-ENNReal.ofReal(delta/2).
This assertion does not depend on rewards.

On the same constructed independent action/arbitrary bounded reward law,
explorationEstimatesCorrect explicitly means BOTH every local empirical mean
has error<eps/2 and every local population estimator equals n. It is proved
measurable by rewriting to the intersection of allMeanAccurate and the finite
populationRecovered event pulled back from the action coordinate. The actual
joint statistics-accuracy event is a subset.

For arbitrary Borel probability measures nu_a supported a.e. in[0,1],
1<=n<=k, k>1, 0<eps<=1 and0<delta<1, at the exact previously defined duration

    S = ceil_nat(max{(16k/eps^2) log(4k^2/delta),
                     50k^2 log(4k/delta)}),

explorationEstimatesCorrect_probability proves

    P_actual_joint(all means accurate AND all local Nhat=n) >= 1-delta.

This uses the existing probability producer, not a premise asserting the good
event. The true n in the analysis is not a learner input. All casts, log domains,
count bounds, measurability and probability monotonicity are explicit.

## Boundaries and concrete checks

The scalar accuracy theorem permits non-strict error<=1/(10k), while the
probabilistic producer supplies the stronger strict inequality. n=1 is covered.
n=k is a legitimate extension of this component; the complete source contract
still requires1<=n<k. k=1 is excluded from inverse correctness because log b
would be log0. Definitions remain total at k=0,1, T=0 and invalid C>T; these
values are not claimed to satisfy the source inverse formula's valid domains.
At T=0 the actual count is0=T and the estimator returns k. For C>T natural
subtraction saturates, and no regular-branch semantic theorem is claimed.
For delta outside(0,1), only the explicitly weaker fixed-T population statement
can apply and the ENNReal lower bound may be vacuous. No stopping-time, adaptive
exploration, correlated-reward or conditional-continuation claim is made.

Seven new canaries verify:

- k=3,T=30,C=10 returns2 at the exact collision mean1/3;
- C=9 and C=11 both return2, including the CLOSED1/(10k) error boundaries;
- zero collisions return1 and all30 collisions return the cap3;
- a concrete three-round,two-player,three-arm action schedule has one collision;
- its ACTUAL local estimator returns2 for every reward array.

The concrete schedule is an exploration transcript with positive finite uniform
coin probability, but this packet does not calculate that atom's mass. It is
not a complete learner canary and does not establish empirical rankings.

## Acceptance scope and remaining chain

The new layer contains38 declarations:32 theorems and six definitions, with no
new probability instances. The prior collision/reward/exploration source is
retained exactly beneath two new imports. Axiom audit, independent blind/source
review and hash-bound evidence belong in
runs/extended-topics-20260920/multi-agent-population-review.json. No new public
root/Tests/harness/site acceptance is inferred from scratch compilation.

Still required: actual deterministic ranking with a fixed tie rule, learned
common top-n candidate sets and source good event G; fresh continuation and full
causal learner/regret assembly; full learner canary, shared extraction/public
gates, recent-source comparison and all-topic ICLR evidence. Population recovery
alone does not complete the multi-agent topic or the ten-topic Goal.
