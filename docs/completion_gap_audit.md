...
| Linear/OFUL/LinUCB | local Gram PSD wrapper, Mathlib Schur-complement rank-one determinant-update wrapper, scalar regularized-Gram base wrapper, regularized-Gram quadratic-form/PSD wrapper, strict-positive qform wrapper, Mathlib PosDef determinant/IsUnit bridge, arbitrary regularized-Gram rank-one determinant recursion, and one-step log-det increment wrapper compiled; paper/weapon cards for the rest | finite-horizon log-det telescope, determinant growth inequalities, confidence ellipsoid, self-normalized tail |
