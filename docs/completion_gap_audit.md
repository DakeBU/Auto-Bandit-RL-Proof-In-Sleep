...
| Linear/OFUL/LinUCB | local Gram PSD wrapper, Mathlib Schur-complement rank-one determinant-update wrapper, scalar regularized-Gram base wrapper, regularized-Gram quadratic-form/PSD wrapper, strict-positive qform wrapper, and Mathlib PosDef determinant/IsUnit bridge for arbitrary regularized Grams compiled; paper/weapon cards for the rest | log-det telescope, determinant growth inequalities, confidence ellipsoid, self-normalized tail |
