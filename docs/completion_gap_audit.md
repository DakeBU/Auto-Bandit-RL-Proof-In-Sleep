...
| Linear/OFUL/LinUCB | local Gram PSD wrapper, Mathlib Schur-complement rank-one determinant-update wrapper, scalar regularized-Gram base wrapper, regularized-Gram quadratic-form/PSD wrapper, strict-positive qform wrapper, Mathlib PosDef determinant/IsUnit bridge, arbitrary regularized-Gram rank-one determinant recursion, one-step log-det increment wrapper, abstract finite-horizon log-det telescope, concrete Nat-prefix log-det telescope, scalar-base endpoint, and first min/log determinant-growth consumer compiled; paper/weapon cards for the rest | inverse-quadratic nonnegativity discharge, determinant upper bounds/dimension-radius simplifications, confidence ellipsoid, self-normalized tail |
