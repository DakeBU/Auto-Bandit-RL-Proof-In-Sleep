...
| compiled local UCB confidence-algebra/event leaves | 2 | `UCB.confidenceScore`, `UCB.meanGap`, `UCB.meanGap_le_two_radius_of_confidenceScore_max`, `UCB.not_two_radius_lt_meanGap_of_confidenceScore_max`, `UCB.upperConfidenceBad`, `UCB.lowerConfidenceBad`, `UCB.confidenceBadEvent`, and `UCB.meanGap_le_two_radius_of_not_confidenceBadEvent`, the deterministic and event-level good-event/index-maximality consumers showing `gap <= 2 * chosenRadius`; no event measurability, log/sqrt radius formula, concentration tail producer, expected pull-count theorem, or final regret theorem |
...
| UCB/ETC textbook routes | wrappers plus theorem cards, with abstract finite-horizon UCB bad-event summability and deterministic/event-level UCB confidence-radius consumers now compiled | UCB confidence-event measurability, log/sqrt radius formula, pull-count theorem, and final regret; ETC expected-regret assembly remains separate |
