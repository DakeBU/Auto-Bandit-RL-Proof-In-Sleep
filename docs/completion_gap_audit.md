...
| compiled local UCB confidence-algebra/event leaves | 3 | `UCB.confidenceScore`, `UCB.meanGap`, `UCB.meanGap_le_two_radius_of_confidenceScore_max`, `UCB.not_two_radius_lt_meanGap_of_confidenceScore_max`, `UCB.upperConfidenceBad`, `UCB.lowerConfidenceBad`, `UCB.confidenceBadEvent`, `UCB.meanGap_le_two_radius_of_not_confidenceBadEvent`, and `UCB.measure_confidenceBadEvent_le_sum_upper_lower`, the deterministic and event-level good-event/index-maximality consumers showing `gap <= 2 * chosenRadius` plus the finite-arm confidence bad-event union bound; no event measurability, log/sqrt radius formula, concentration tail producer, expected pull-count theorem, or final regret theorem |
...
| UCB/ETC textbook routes | wrappers plus theorem cards, with abstract finite-horizon UCB bad-event summability, deterministic/event-level UCB confidence-radius consumers, and the finite-arm confidence bad-event union bound now compiled | UCB confidence-event measurability, log/sqrt radius formula, pull-count theorem, and final regret; ETC expected-regret assembly remains separate |
