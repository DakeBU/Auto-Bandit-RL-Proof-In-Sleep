...
| compiled local UCB confidence-algebra leaves | 1 | `UCB.confidenceScore`, `UCB.meanGap`, `UCB.meanGap_le_two_radius_of_confidenceScore_max`, and `UCB.not_two_radius_lt_meanGap_of_confidenceScore_max`, the deterministic good-event/index-maximality consumer showing `gap <= 2 * chosenRadius`; no log/sqrt radius formula, concentration tail producer, expected pull-count theorem, or final regret theorem |
...
| UCB/ETC textbook routes | wrappers plus theorem cards, with abstract finite-horizon UCB bad-event summability and deterministic UCB confidence-radius algebra now compiled | UCB log/sqrt radius formula, pull-count theorem, and final regret; ETC expected-regret assembly remains separate |
