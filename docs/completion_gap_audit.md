...
| Linear/OFUL/LinUCB | local Gram PSD wrapper, Mathlib Schur-complement rank-one determinant-update wrapper, scalar regularized-Gram base wrapper, regularized-Gram quadratic-form/PSD wrapper, and strict-positive qform wrapper compiled; paper/weapon cards for the rest | determinant positivity, arbitrary regularized Gram invertibility, log-det telescope, determinant growth inequalities, confidence ellipsoid, self-normalized tail |
