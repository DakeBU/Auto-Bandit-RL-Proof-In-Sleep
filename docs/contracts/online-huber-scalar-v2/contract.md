...
The v1 header probe and leaf01 report a noncomputable-definition compilation error: real decidable comparison is noncomputable. V2 changes only `def huber` to `noncomputable def huber`. The mathematical value, both headers, hypotheses and endpoints are identical. Preserve the failed v1 logs. Re-review accepts this explicit implementation-context repair before recompilation; it is not a weakening of a terminal.
