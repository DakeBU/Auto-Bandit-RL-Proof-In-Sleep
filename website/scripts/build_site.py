...
<section id="active-source-ports"><h2>Active source ports outside the comparison atlas</h2><p>These two NeurIPS 2025 ports record current compiled progress and exact nonclaims. They remain outside the minimax ledger until their paper-level terminal contracts and comparison partners are frozen.</p><div class="wiki-source-port-grid">{render_active_source_audits(frontier_page)}</div></section>
