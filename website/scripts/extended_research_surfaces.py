...
def build_extended_research_surfaces(
    ...
    *,
    ...
    _build_setting_atlas(
        output, atlas, layout, write_page, href_from, verified, generated_at
    )
...
<p>{html.escape(item['summary'])}</p>{aliases}
</article>"""
            )
        groups.append(
...
<section class="hero" id="atlas">
  ...
  <div class="hero-actions">
    <a class="button primary" href="{href_from(page, 'banditrlwiki/index.html#settings')}">Coarse assumption families</a>
    <a class="button" href="{href_from(page, 'banditrlwiki/frontier-problems/index.html')}">Frontier problem history</a>
  </div>
</section>
<section id="taxonomy-rule">
  ...
  <div class="callout warning"><strong>Ambiguous acronyms stay quarantined.</strong> OMDP, SLB and Transform are not silently normalized until a cited source fixes their meaning.</div>
</section>
{''.join(groups)}
...
def _build_frontier_registry(
    ...
    cards = "".join(_problem_card(page, p, href_from) for p in frontier["problems"])
    body = f"""
<section class="hero" id="registry">
  ...
  <div class="stats-grid">
    <div class="stat"><span class="stat-value">{len(frontier['problems'])}</span><span class="stat-label">audited problem histories</span></div>
...
"""
    write_page(
        ...
        layout(
            ...
            frontier["title"],
            body,
            [("registry", "Registry"), ("status-contract", "Status contract"), ("problems", "Problems"), ("contribute", "Contribute")],
