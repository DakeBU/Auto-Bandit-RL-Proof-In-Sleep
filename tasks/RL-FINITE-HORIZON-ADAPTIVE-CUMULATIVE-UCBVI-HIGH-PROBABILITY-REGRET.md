...
`0 < delta <= 1`, the planned terminal bounds the generated cumulative
...
| `UCBVI-PAPER-COUNT` | aggregate `N_k(x,a)` across all stages, with exact alignment to the generated prefix | compiled; planner integration pending |
| `UCBVI-CLIPPED-PLAN` | recurrent `Q_{k,h} = min(Q_{k-1,h}, H, R + P_hat V + b)`; zero-count value `H`; measurable finite argmax | planned |
| `UCBVI-SAME-SOURCE-CONFIDENCE` | simultaneous adaptive confidence over `k,h,x,a,y` on the same trajectory law, self-normalized by actual cumulative counts | planned |
| `UCBVI-OPTIMISM` | good event implies Bellman optimism for the selected episode plan | planned |
| `UCBVI-REGRET-DECOMPOSITION` | generated per-episode regret equals/obeys optimism plus occupancy/bonus plus martingale noise terms | planned |
| `UCBVI-BONUS-SUM` | cumulative visit-count and clipped Hoeffding bonus sum with explicit `S,A,H,K,L` bound | planned |
| `UCBVI-CH-TERMINAL` | same-process event mass at most `delta` and the frozen cumulative regret bound | planned |
| `UCBVI-EXPECTED` | expected-regret corollary with the failure-event contribution retained and bounded | planned |
...
## Current semantic mismatch ledger

- The cross-stage aggregate `N_k(x,a)` view and update now compile, but the
  generated source still feeds stage-indexed cumulative counts to the existing
  `EstimatedModelPlan`. The recurrent planner must consume the aggregate view;
  the new declaration alone does not change the algorithm.
- The existing planner clips a local radius but does not carry the paper's
  previous-episode `Q` table and therefore does not implement
  `min(Q_previous,H,backup)`.
- Existing adaptive count confidence controls raw coordinate martingales and
  often relies on exploratory reachability floors.  It is not the required
  self-normalized transition-value confidence at the selected algorithm's own
  random counts.
- Existing natural-causal high-probability terminals sum batch-average regret
  coordinates.  They do not state the paper's raw episode cumulative regret.
...
- [x] Full repository gate for the compiled foundation: 3664 Lean build jobs;
  43 Python tests passed with one expected skip.
- [x] Paper aggregate-count view/update node.
- [ ] Clipped-plan/confidence/decomposition/bonus/terminal nodes.
- [ ] Full terminal reviewer gate.
- [ ] Repeat full `python3 tools/bandit.py check` after the terminal is closed.

Until every planned Hoeffding node compiles, keep this task and the website
`partial`.  Do not substitute a caller-supplied confidence witness, an offline
