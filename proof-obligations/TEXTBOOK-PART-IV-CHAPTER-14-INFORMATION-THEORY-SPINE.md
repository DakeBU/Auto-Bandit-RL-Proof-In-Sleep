...
| `CH14-SOURCE-CODING` | arithmetic/block-code achievability and converse | product distributions, block prefix codes, asymptotics | exact n-fold entropy and finite-n realizable rate sandwich; code-family limit and arithmetic algorithm remain open | `exists_sourceBlock_code_rate_sandwich`, `sourceBlock_code_rate_lower_bound` focused-build/canary passed | chapter terminal | partial |
