"""Export and summarize the accepted MusicalChairs compiled graph, without efficiency claims.

Run at the pinned snapshot or with byte-equivalent reviewed multi_agent/library inputs.
...
SNAPSHOT = "4a09e817cabd67f4ffc3b248ff1487ae55f6dea3"
...
def inputs():
    ...
    paths = sorted(p.relative_to(ROOT).as_posix()
                   for p in (ROOT / "BanditRLProof/Algorithms").glob("MusicalChairs*.lean"))
    require(len(paths) == 12, "Frozen MusicalChairs module scope changed")
    for path in paths:
    ...
    for packet in ["occupation", "regret", "learner-promotion"]:
        receipt = json.loads((RUN / ("multi-agent-" + packet + "-review.json")).read_text(encoding="utf-8"))
        for path, expected in receipt["production_hashes"].items():
...
def export(graph_path, paths):
    ...
    # match the pinned snapshot, not just the 12 owner modules.
    ...
    replacements = [
        ...
        ('"project-direct-with-external-boundary"', '"multi_agent-owned-direct-with-all-boundary-nodes"', 1),
...
def summarize(graph_path, paths, expected_hash):
    ...
    require(graph["extraction"]["closure"] == "multi_agent-owned-direct-with-all-boundary-nodes",
            "Wrong graph scope")
    ...
    proof_pairs = [e for e in graph["edges"]
                   if e["kind"] == "value" or e.get("also_in_value") is True]
    cross_module = [e for e in proof_pairs if e["target"] in focused
                    and nodes[e["source"]]["module"] != nodes[e["target"]]["module"]]
    proof_categories = Counter()
    for edge in proof_pairs:
        target = nodes[edge["target"]]
        if edge["target"] in focused:
            category = ("within-same-owner-module" if nodes[edge["source"]]["module"] == target["module"]
                        else "within-case-cross-owner-module")
        elif target["scope"] == "project":
            category = "project-outside-case"
        elif target["module"].startswith("Mathlib."):
            category = "Mathlib"
        else:
            category = "Lean-core-or-other-external"
        proof_categories[category] += 1
    ...
    validation_path = RUN / "multi-agent-learner-validation.json"
    ...
    return {"kind": "descriptive-development-evidence-not-controlled-evaluation",
            ...
            "proof_value_pair_categories": dict(sorted(proof_categories.items())),
            "cross_module_case_pairs": len(cross_module),
            "cross_module_case_distinct_targets": len({e["target"] for e in cross_module}),
            "focused_modules_present_at_initial_base": [p for p in paths if blob(BASE, p) is not None],
            ...
            "semantic_receipt_sha256": {p: sha(RUN / ("multi-agent-" + p + "-review.json")) for p in ["occupation", "regret", "learner-promotion"]},
            "production_validation_sha256": sha(validation_path),
            "recorded_production_commit": validation["code_commit"],
            "recorded_gates": {"harness": validation["harness"], "site": validation["site"], "contributor_gate": validation["contributor_gate"]}, "full_gates_rerun_by_this_script": False,
            "limitations": ["Direct constant-reference pairs, not use frequency or effort saved",
                            "Internal cross-module pairs are development within this case, not reuse from the initial library",
                            "Zero outside-case project pairs is a negative descriptive result, not evidence of library speedup",
                            "Mathlib/core pairs include constructors, instances and generated proof machinery, not just reusable mathematical lemmas",
                            ...
                            "Includes all 12 MusicalChairs-prefixed production modules; excludes Tests as seeds",
...
if __name__ == "__main__":
    ...
    args.output.write_text(json.dumps(summarize(graph_path, paths, expected), indent=2) + "\n",
                           encoding="utf-8")
    print("Wrote multi_agent descriptive summary; no controlled runs or efficiency estimate.")
