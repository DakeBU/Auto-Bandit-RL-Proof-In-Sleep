"""Export and summarize the accepted causal compiled graph, without efficiency claims.

Run at the pinned snapshot or with byte-equivalent reviewed causal/library inputs.
...
SNAPSHOT = "296b09127c041670fe552df18d7d6846e8560ae5"
...
def inputs():
    ...
    paths = sorted(p.relative_to(ROOT).as_posix()
                   for p in (ROOT / "BanditRLProof/Algorithms").glob("Causal*.lean"))
    require(len(paths) == 20, "Frozen causal module scope changed")
    for path in paths:
    ...
    for packet in ["sampling", "heterogeneous", "noisy-diagnostics", "parallel"]:
        receipt = json.loads((RUN / ("causal-" + packet + "-review.json")).read_text(encoding="utf-8"))
...
def export(graph_path, paths):
    ...
    # match the pinned snapshot, not just the 20 owner modules.
    ...
    replacements = [
        ...
        ('"project-direct-with-external-boundary"', '"causal-owned-direct-with-all-boundary-nodes"', 1),
...
def summarize(graph_path, paths, expected_hash):
    ...
    require(graph["extraction"]["closure"] == "causal-owned-direct-with-all-boundary-nodes",
            "Wrong graph scope")
    ...
    validation_path = RUN / "causal-parallel-validation.json"
    ...
    return {"kind": "descriptive-development-evidence-not-controlled-evaluation",
            ...
            "semantic_receipt_sha256": {p: sha(RUN / ("causal-" + p + "-review.json")) for p in ["sampling", "heterogeneous", "noisy-diagnostics", "parallel"]},
            "production_validation_sha256": sha(validation_path),
            "recorded_production_commit": validation["code_commit"],
            "recorded_gates": {k: validation[k] for k in ["lean_root_jobs", "tests_jobs", "python_tests", "python_skipped", "full_gate", "site_check"]}, "full_gates_rerun_by_this_script": False,
            "limitations": ["Direct constant-reference pairs, not use frequency or effort saved",
                            ...
                            "Includes all 20 Causal-prefixed production modules; excludes Tests as seeds",
...
if __name__ == "__main__":
    ...
    args.output.write_text(json.dumps(summarize(graph_path, paths, expected), indent=2) + "\n",
                           encoding="utf-8")
    print("Wrote causal descriptive summary; no controlled runs or efficiency estimate.")
