"""Recheck shared causal identities and rendered declaration destinations."""
import json
...
def verify():
    ...
    topic = next(t for t in wiki["topics"] if t["id"] == "causal")
    ...
    setting = next(s for s in registry["settings"] if s["id"] == "causal")
    graph = json.loads((OUT / "lean-graph/scopes/settings/causal.json").read_text(encoding="utf-8"))
    ...
    page = OUT / "banditrlwiki/topics/causal/index.html"
    ...
    for key in sorted(expected):
        node = nodes[key]
        assert "causal" in node["settings"]
