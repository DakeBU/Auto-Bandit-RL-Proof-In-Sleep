...
p=Path('tmp/online-convex-graph.json');raw=p.read_bytes();graph=json.loads(raw)
ns='BanditRL.OnlineConvex.'
local=list(json.loads(Path('docs/contracts/online-convex-extended-v2/headers.json').read_text(encoding='utf-8')))+['effectiveDomain','realEpigraph','IsConvexExtended','extendedIndicator']
...
required=[('convex_indicator_iff','convex_effectiveDomain'),('convex_indicator_iff','effectiveDomain_indicator'),('convexExtended_iff_toReal','realEpigraph_toReal'),('theorem_2_4','convexExtended_iff_toReal')]
checks=[]
for a,b in required:
