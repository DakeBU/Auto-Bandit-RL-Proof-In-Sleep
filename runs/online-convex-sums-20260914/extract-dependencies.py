...
p=Path('tmp/online-convex-sums-graph.json');raw=p.read_bytes();graph=json.loads(raw)
ns='BanditRL.OnlineConvex.'
local=list(json.loads(Path('docs/contracts/online-convex-sums-v1/headers.json').read_text(encoding='utf-8')))+['upperAdd']
...
required=[('convex_upperAdd','upperAdd_le_coe_iff'),('convex_nonneg_mul','positive_mul_le_coe_iff'),('convex_nonneg_mul','convexExtended_coe_iff'),('convex_nonneg_linear_combination','convex_upperAdd'),('convex_nonneg_linear_combination','convex_nonneg_mul')]
checks=[]
for a,b in required:
