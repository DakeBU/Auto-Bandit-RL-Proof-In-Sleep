...
| 16, Instance-Dependent Lower Bounds | exact Definition 16.1 / Theorem 16.2 / Lemma 16.3 / Theorem 16.4 contracts; generic consistency, exact unit-Gaussian `d_inf`, eventual power/log growth, one-arm same-policy history KL, measurable majority event, BH information inequality, and scalar log assembly | twenty-declaration dependency/event-information slice compiled; chapter partial and its own terminals blocked | prove the two source-general event-to-expected-pseudo-regret bounds, then complete the finite-time and liminf consumers without weakening zero/finite/infinite information branches |
