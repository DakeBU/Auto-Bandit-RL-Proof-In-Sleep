...
| `MLIB-KRAFT-MCMILLAN` | `Mathlib.InformationTheory.Coding.KraftMcMillan`, `Mathlib.InformationTheory.Coding.UniquelyDecodable` | `InformationTheory.UniquelyDecodable`, `kraft_mcmillan_inequality`, `List.flatten`, `List.IsPrefix` | finite binary prefix codes, unique decoding, Kraft bounds, and future source-coding leaves |
| `MLIB-KL-TRIM-CONDITIONAL-JENSEN` | `Mathlib.InformationTheory.KullbackLeibler.Basic`, `Mathlib.MeasureTheory.Function.ConditionalExpectation.CondJensen`, `Mathlib.MeasureTheory.Function.ConditionalExpectation.RadonNikodym` | `Measure.trim`, `toReal_rnDeriv_trim`, `ConvexOn.map_condExp_le`, `integral_condExp`, `integral_trim` | arbitrary-sub-sigma-algebra KL data processing via conditional Jensen |
...
| `MLIB-GAUSSIAN-REAL-TAIL` | `Mathlib.Probability.Distributions.Gaussian.Real`, `Mathlib.Probability.Distributions.Gaussian.HasGaussianLaw.Independence`, `Mathlib.Probability.Moments.SubGaussian`, `Mathlib.Analysis.SpecialFunctions.Gaussian.GaussianIntegral` | `gaussianReal`, `charFun_map_sum_pi_eq_prod`, `charFun_gaussianReal`, `gaussianReal_map_div_const`, `mgf_id_gaussianReal`, `HasSubgaussianMGF.measure_ge_le`, `integral_gaussian_Ioi` | Chapter 13 canonical finite-iid empirical-mean law and two-hypothesis Chernoff upper tail compile from these APIs; no exact Mills-ratio upper/lower theorem was found at pinned Mathlib `5e932f9`, the separate local `GaussianMillsRatio.lean` candidate now proves both exact integral bounds and the GaussianHypothesisTesting consumer compiles Eq. (13.1) |
