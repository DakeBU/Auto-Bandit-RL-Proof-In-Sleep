# Static unknown-N Musical Chairs: actual learner and expected regret

Rosenski, Shamir and Szlak, *Multi-Player Bandits -- a Musical Chairs Approach*,
ICML 2016, PMLR 48:155--163. Source anchors: main physical p3, Algorithms 1/2,
Definition 1, Theorem 1 and its conditioning paragraph; supplement A.1 pp1--7,
Lemmas 2--4 and their proofs. Main SHA256:
...
This page promotes the separately reviewed complete static chain. It does not
accept the dynamic-entry algorithm, MEGA comparison, an unknown-gap policy,
recent-source comparisons, or the entire multi-agent topic. Actual validation
and promotion-review states are recorded in the accompanying receipts; a
compiled module is distinct from all publication and shared-project gates.

## Model, source repair and exact algorithm

There are synchronized players 0<n<k. Arm a has a stationary Borel probability
law nu_a on Real, supported almost surely on [0,1], with mean mu_a. Primitive
arm/round rewards and private uniform coins are independent. A collision gives
reward zero and an explicitly sensed collision bit; a genuine zero reward does
not imply collision. The learner uses k, epsilon and delta, not n or true means.
Assume 0<epsilon<mu_(n)-mu_(n+1) and 0<delta<1. Ties within the top set are
allowed; public arm order breaks empirical and true-mean ties.

Use exactly S rounds, indexed 0,...,S-1, where

    S = ceil max(16k/epsilon^2 * log(4k^2/delta),
                 50k^2 * log(4k/delta)).

The second logarithm explicitly repairs the printed log(4/delta) to pay an
all-player probability union. This is not a proof that the smaller printed
duration is impossible. The first coefficient and logarithm are retained via
the random-count proof below. The source's inclusive loop notation is resolved
to exactly S observations, consistently with the denominator.

During exploration each player draws uniformly and independently. Its empirical
mean uses only its collision-free observations, with zero for an empty count.
From collision count C it computes

    Nhat = min(k, toNat(round(1 + log((S-C)/S)/log(1-1/k)))).

When C=S it returns k. Natural subtraction, real casts, half-up rounding and
clamping are explicit. The sorted prefix of length Nhat is its candidate set,
proved nonempty even on bad histories. Each unfixed player then draws uniformly
from its own learned candidates. The first collision-free action becomes its
permanent arm. A later collision does not unfix it. A finite horizon H<S reads
only its H actual observations; the latent unused suffix does not affect them.

## Derivation and Lean correspondence

1. **Exploration law.** FinitePMF.iid and explorationDraw construct the actual
   independent uniform schedule. Rectangle probabilities give effective
   observation probability q=(1/k)(1-1/k)^(n-1) and collision probability
   p=1-(1-1/k)^(n-1). For n<=k, q>=1/(4k). Observation and collision count
   generating functions are derived from this same product law.

2. **Selected reward concentration.** rewardLaw is the product over arms and
   rounds of nu_a. Fixing an exploration action schedule fixes the set of
   collision-free samples independently of rewards. Coordinate projection,
   bounded centered MGF and finite-sum independence give the signed sample-sum
   tails; the zero-count branch is retained. Integrating the resulting factor
   against the actual count PGF yields
   P(|localMean-mu_a|>=epsilon/2)<=2 exp(-S epsilon^2/(16k)). A union over all
   n*k pairs and n<=k pays delta/2 with the first duration term. No selected
   independence, count distribution or confidence event is assumed as input.

3. **Population producer.** The actual collision-frequency Hoeffding bound,
   unioned over players, gives |pHat-p|<=1/(10k) except with probability delta/2.
   Set b=1-1/k, q=b^(n-1)>=1/4. Real-exponent Bernoulli and reciprocal algebra
   show b^(2/5)<=1-2/(5k) and b^(-2/5)>=1+2/(5k). The inverse-log estimate
   therefore lies in [n-2/5,n+2/5], strictly inside the rounding cell. This
   proves actual Nhat=n and excludes the all-collision fallback on this event.

4. **Actual ranking.** scoreOrder and Finset.sort produce the actual fixed-tie
   prefix. Its cardinality and selected-before-unselected property are proved.
   The numeric boundaryGap separates every top arm from every outside arm.
   Strict epsilon/2 empirical errors preserve this separation, so the actual
   local candidate sets equal trueTopArms on the same exploration success event
   G. Thus P(G)>=1-delta. A complete epsilon-correct ordering is not needed;
   the event records true top-set and population recovery.

5. **Learned continuation and cost.** learnedConfig and learnedDrawPathKernel
   are measurable and total. Their compProd with explorationRewardLaw is the
   actual continuation law, not a supplied correct-candidate process. On G,
   rectangle factorization reduces the kernel to iid draws on the common true
   top set. Actual path/state/current-draw marginals match the existing
   coordination process. Its proved hazard 1/(4n), finite occupation <=4n^2
   and actual deficit charge <=2U give conditional coordination cost <=8n^2.
   Waiting times need not be independent.

6. **Every horizon and failed exploration.** learnerAction splices exploration
   with actual coordination, and learnerRegret splits exactly at min(H,S).
   Successful played arms are distinct, so the true top-n comparator maximizes
   their mean sum on every history. Hence each round's pseudo-regret is in
   [0,n], including failed exploration. Integrating the good/bad split gives

       E[R_H^pseudo] <= min(nH, n min(H,S) + 8n^2 + delta nH)
                     <= min(nH, nS + 8n^2 + delta nH).

   The normalized conditional pseudo-regret is <=n min(H,S)+8n^2. The source
   2exp(2)n^2 residual wrapper retains its S<=H qualification where needed.
   Delta=1/H for H>=2 leaves the explicit additive n failure residual.

7. **Actual rewards and visible process.** completeLearnerLaw adds fresh
   independent continuation rewards. learnerFeedback records played arm,
   collision bit and actual reward chronologically. Candidate recovery and
   local fixation updates are identities on this feedback. Prefix identities
   exclude access to later latent coordinates. A shared finite-schedule first-
   moment lemma, signed integrability, Fubini and the exact first marginals
   prove unconditional E[R_H^realized]=E[R_H^pseudo]. visibleLearnerLaw is the
   measurable trace pushforward, so source_expected_visibleRegret_le gives
   the same bound for the sum of actual observed rewards. Samplewise realized
   regret can be negative. Since G depends on exploration rewards, no
   conditional realized=pseudo equality is inferred.

## Full noisy consumer and exact limits

The public-root test module instantiates n=2,k=3 with genuinely noisy Bernoulli
laws of means (3/4,1/2,1/4), epsilon=1/8 and delta=1/4, using the exact S.
It proves the means, numeric gap 1/4, P(G)>=3/4 and every-H expected visible
bound min(2H,2S+32+H/2). Effective observation and collision probabilities are
exactly 2/9 and 1/3. A learned two-round path first collides then separates;
its joint and complete-law event has mass >=3/64 and its coordination
pseudo-regret is 5/4. Future rewards remain random. Separate ranking and
zero-bit fixtures are diagnostics, not claimed measured full-trace events.

This is symbolic theorem instantiation, not stochastic simulation or a claim
of sublinear regret with fixed delta=1/4. The finite generative construction
establishes the selected static learner; no general filtration, condDistrib,
cross-horizon projective consistency or exact-real executable runtime is claimed.

Actual reused project parents include MusicalChairsCoordination, CoordinationTime
and CoordinationRegret. Mathlib supplies PMFs, pi/product/compProd measures,
sub-Gaussian MGF concentration, finite sorting, real log/rpow bounds, integrals
and measurable pushforwards. The ten scratch receipts bind independent blind
and source reviews, and the production extraction receipt binds exact code.
Imports alone are not claimed proof dependencies or certified cross-topic
transports. Recent-source audit, ICLR descriptive/controlled evidence and final
topic acceptance remain open. No merge, deployment or whole-Goal completion.

<details>
<summary>BanditRLProof/Algorithms/MusicalChairsExploration.lean</summary>
...
<details>
<summary>BanditRLProof/Algorithms/MusicalChairsReward.lean</summary>
...
<details>
<summary>BanditRLProof/Algorithms/MusicalChairsCollision.lean</summary>
...
<details>
<summary>BanditRLProof/Algorithms/MusicalChairsPopulation.lean</summary>
...
<details>
<summary>BanditRLProof/Algorithms/MusicalChairsRanking.lean</summary>
...
<details>
<summary>BanditRLProof/Algorithms/MusicalChairsHandoff.lean</summary>
...
<details>
<summary>BanditRLProof/Algorithms/MusicalChairsMarginal.lean</summary>
...
<details>
<summary>BanditRLProof/Algorithms/MusicalChairsLearnerRegret.lean</summary>
...
<details>
<summary>BanditRLProof/Algorithms/MusicalChairsRealized.lean</summary>
...
<details>
<summary>Tests/MusicalChairsLearnerCanary.lean</summary>
