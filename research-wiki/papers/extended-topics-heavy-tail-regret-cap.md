# A finite upper cap for raw-moment regret

Source: Genalti, Marsigli, Gatti and Metelli, *[epsilon,u]-Adaptive Regret
Minimization in Heavy-Tailed Bandits*, COLT2024/PMLR247, Eq1/Eq2 and Theorem2
Eq5 on pp1-2,7. Frozen PDF SHA256:
`65eceb2cd402baa8c7f5b60d273a103fbd181cf6803a4b7f0d2f4092742fe5b6`.
[Published source](https://proceedings.mlr.press/v247/genalti24a.html).

The literal Eq5 asserts an infinite supremum of expected regret divided by
$u^{1/(1+\epsilon)}$ at a fixed sufficiently large finite horizon. This page
establishes an upper obstruction on the positive-scale domain. It does not
reject asymptotic nonadaptivity or certify the paper's other lower bounds.

## Model and mathematical proof

Let $K\ge1$, $\epsilon\ge0$, $p=1+\epsilon$, $u>0$. Every arm is a probability
law with integrable raw absolute moment $\mathbb E|X_i|^p\le u$. Put
$\mu_i=\mathbb E X_i$ and $s=u^{1/p}$. The means exist because
$|x|\le1+|x|^p$. Jensen gives
\[
 |\mu_i|\le\mathbb E|X_i|\le (\mathbb E|X_i|^p)^{1/p}\le s.
\]
Every arm gap therefore lies in $[0,2s]$. For any action trace and any natural
horizon, including zero,
\[
 0\le R_T=\sum_{t<T}(\max_i\mu_i-\mu_{A_t})\le2Ts.
\]
Coordinatewise measurable actions give a measurable finite sum. Its bound
proves integrability; integration under the actual probability law and positive
division yield $\mathbb E R_T/u^{1/p}\le2T$. No confidence event, regret premise,
independence of actions or specified UCB policy is used.

Define $V(K,\epsilon,T)$ as extended-real values witnessed by an actual scale
$u>0$, a Markov arm kernel with the preceding moments, and any probability law
on infinite finite-arm action traces. The value is the actual normalized
integral, not an assumed numerical bound. Every member is at most the finite
real $2T$, hence $\sup V\le2T<+\infty$ by completeness of EReal.

For a real action process on $(\Omega,Q)$, the trace map $A$ is measurable.
Its actual image law $Q\circ A^{-1}$ is a probability law, and integral change
of variables preserves the expected regret exactly. The declaration
`process_value_mem` supplies this bridge. The larger set admits noncausal
trace laws too; its upper bound therefore bounds each fixed causal algorithm's
values. The code does not define a named fixed-algorithm class/subset theorem.

For $K=2,\epsilon=1,u=1$, choose Dirac reward laws at $+1,-1$ and always play
the negative arm. Both raw second moments equal1 and regret equals $2T$.
Thus $\sup V(2,1,T)=2T$. The public-root canary uses $T=1000$, value2000,
actual moment witnesses and the actual-process membership adapter. This is
sharpness over policies, not a lower bound for each fixed algorithm.

## Source versus Lean

- **Same:** raw reward moments, expected mean-gap regret, finite horizon.
- **Generalization:** $K\ge1$, $\epsilon\ge0$, arbitrary measurable action laws;
  the source has $K\ge2$, $0<\epsilon\le1$ and causal algorithms.
- **Explicit representation:** raw moment integrability plus its Bochner bound;
  regret integrability is derived. Rounds $0,\ldots,T-1$ are $T$ source rounds.
- **Boundary:** $u=0$ is excluded from the ratio; the original $0/0$ wording is
  not a meaningful source of infinity. General nonemptiness is not separately
  formalized; the nonempty sharp witness is the stated two-arm slice.
- **Remaining:** no repaired Eq6 lower theorem, asymptotic impossibility proof,
  fixed-algorithm sharpness or complete heavy-tail topic acceptance.

## Reuse and formal interfaces

`integrable_id_of_raw_moment`, `realKernelMean`, `realMeanRegret`,
`realMeanGap` and `realMeanRegret_eq_finset_sum_gap` come from the shared ABRL
library. Mathlib supplies `convexOn_rpow`, `ConvexOn.map_integral_le`, finite
sums, `Measure.isProbabilityMeasure_map`, `integral_map`, EReal coercions and
complete-lattice suprema. The proof imports the shared project and pinned
Mathlib; it creates no second Lean project.

The source-blind decoder and independent source reviewer accepted the enlarged
class and explicit parameter differences. This result is a source obstruction,
not a new learning rate or a topic completion badge. The compiler-derived
module/declaration graph records the real imports; reading links are not proof
edges. No conceptual functor is asserted.

<details>
<summary>Exact Lean statements and proofs</summary>
