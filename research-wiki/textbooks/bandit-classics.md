...
| `TXT-LS-2020-COR-17-3-UNIFORM-TAIL-IMPOSSIBILITY` | Ch. 17, §17.1, Corollary 17.3, CUP print pp. 187--188 / author-online pp. 217--218 / physical PDF pp. 226--227 | no single policy has strict `<delta` tail at `B sqrt((k-1)n) log(1/delta)^p` for all horizons, confidence levels, and Gaussian environments when `p in (0,1)` | exact target frozen; Theorem 17.1 and the Gamma integral leaf compile, while tail rescaling and horizon-confidence calibration remain blocked |
