...
| `TXT-LS-2020-CH14-ENTROPY-CODING` | Ch. 14, §14.1, Eqs. (14.1)--(14.3), author-online pp. 186--188 / physical PDF pp. 195--197 | finite binary prefix codes, expected length, base-two/natural entropy, and the Huffman/source-coding bounds | code/entropy definitions, exact finite uniquely-decodable/prefix equivalence, Huffman global optimum and one-bit sandwich, named exact-real arithmetic block coding and universal rate converse compile; nonempty-word and uniform-law qualifications remain explicit; whole-chapter publication is not claimed |
| `TXT-LS-2020-CH14-RELATIVE-ENTROPY` | Ch. 14, §14.2 (CUP starts p. 162), Eqs. (14.4)--(14.6) and Thm. 14.1, author-online pp. 188--190 / physical PDF pp. 197--199 | extended-real `D(P,Q)`; arbitrary finite-alphabet and Bernoulli endpoints; RN/log-likelihood representation on the absolutely-continuous integrable branch; infinity otherwise | compiled finite-alphabet KL/support endpoints, finite-discretisation/RN supremum equivalence and Eq. (14.6) common-density adapters, including singular/nonintegrable branches; pre-integration full gate cf3ed1f passed; current-main integration is separately revalidated |
...
| `TXT-LS-2020-EX15-7-STOPPING-TIME-DIVERGENCE` | Ch. 15, §15.5, Ex. 15.7, CUP print p. 176 / author-online p. 205 / physical PDF p. 214; hint cites Exercises 14.10 and 14.9 | for a bounded stopping time `tau` and `F_tau`-measurable random element `X`, `D(P_nu^X,P_nu'^X) <= sum_i E_nu[T_i(tau)]D(P_i,P_i')` in the first-law direction | partial: generic measurable-map KL data processing and its deterministic-history Lemma 15.1 corollary compile as `klDiv_map_le` and `klDiv_observedBanditHistory_le_expectedPulls_sum`; stopped-history information and `F_tau` factorization remain planned |
...
| `TXT-LS-2020-THM-16-2-ASYMPTOTIC` | Ch. 16, §16.1, Theorem 16.2 / Eq. (16.2), CUP print pp. 177--179 / author-online pp. 207--208 / physical PDF pp. 216--217 | for an unstructured product class and a consistent policy, `liminf R_n/log n` is at least `sum_{i:Delta_i>0} Delta_i/d_inf(P_i,muStar,M_i)` | compiled as `consistentPolicy_liminf_expectedRegret_div_log_ge`; exact product class, n-pull horizons, extended-real inverse-infimum branches, and finite-count Fatou |
| `TXT-LS-2020-LEMMA-16-3-FINITE-TIME` | Ch. 16, §16.2, Lemma 16.3 / Eq. (16.4), CUP print p. 180 / author-online p. 209 / physical PDF p. 218 | for a one-arm change making arm `i` uniquely optimal, lower-bound original-law expected pulls using the exact log regret-sum numerator divided by `D(P_i,P_i')` | compiled as `expectedPullCount_ge_log_regret_changeOfMeasure`, using certified finite arm-law integral means; zero KL is excluded, finite KL uses the exact consumer, and infinite KL is explicit |
| `TXT-LS-2020-THM-16-4-GAUSSIAN-FINITE-TIME` | Ch. 16, §16.2, Theorem 16.4 / Eq. (16.5), CUP print p. 180 / author-online pp. 209--210 / physical PDF pp. 218--219 | under the local unit-Gaussian `C n^p` envelope, sum the exact positive-part per-gap terms with factor `2/(1+epsilon)^2` | compiled as `gaussianExpectedRegret_ge_finiteTimeInstanceDependent` with unrestricted real means, the exact local class and horizon quantifiers, `8C`, and positive-part placement |
| `TXT-LS-2020-THM-17-1-STOCHASTIC-TAIL` | Ch. 17, §17.1, Theorem 17.1, CUP print pp. 186--187 / author-online pp. 216--217 / physical PDF pp. 225--226 | a uniform `B sqrt((k-1)n)` expected-regret envelope forces random pseudo-regret above `(1/4) min{n,(1/B)sqrt((k-1)n)log(1/(4delta))}` with probability at least `delta` on some Gaussian instance | compiled with the full gap-at-most-one Gaussian premise and an embedded unit-cube witness, using the same randomized policy, original-to-alternative history KL, and original-law expected pulls |
| `TXT-LS-2020-COR-17-2-STOCHASTIC-MINIMAX-TAIL` | Ch. 17, §17.1, Corollary 17.2 / Eqs. (17.6)--(17.7), CUP print p. 187 / author-online p. 217 / physical PDF p. 226 | under the exact horizon-confidence side condition, every policy has a Gaussian instance with the stated outer-quarter minimax random-pseudo-regret tail | compiled via the exact expectation contradiction and `B=sqrt(2 log(1/(4delta)))` specialization |
| `TXT-LS-2020-COR-17-3-UNIFORM-TAIL-IMPOSSIBILITY` | Ch. 17, §17.1, Corollary 17.3, CUP print pp. 187--188 / author-online pp. 217--218 / physical PDF pp. 226--227 | no single policy has strict `<delta` tail at `B sqrt((k-1)n) log(1/delta)^p` for all horizons, confidence levels, and Gaussian environments when `p in (0,1)` | exact terminal compiles through layer-cake integration, the Gamma bound, and horizon-confidence calibration against Theorem 17.1 |
| `TXT-LS-2020-THM-17-4-ADVERSARIAL-TAIL` | Ch. 17, §17.2, Theorem 17.4, CUP print pp. 188--190 / author-online pp. 218--220 / physical PDF pp. 227--229 | under `n>=Ck log(1/(2delta))`, some deterministic bounded reward matrix has strict random-regret tail at `c sqrt(nk log(1/(2delta)))` at least `delta` | approved corrected terminal focused compiled: `0<delta<=1/32`, `c=1/160`, `C=64`; same-policy coupling, deterministic extraction and CDF interface proved; full local gate passed |
| `TXT-LS-2020-CLAIMS-17-5-17-7` | Ch. 17, §17.2, Claims 17.5--17.7 and Eq. (17.8), CUP print pp. 189--190 / author-online pp. 219--220 / physical PDF pp. 228--229 | first-moment deterministic witness, clipped-normal pull-small event, pathwise regret comparison, and clipping-count concentration | Claims 17.5--17.7 and full-family Eq. (17.8) focused compiled; Claim 17.6 has approved non-strict `T_i<=n/2` correction; Claim 17.7 uses exact literal boundary event; full local gate passed |
...
information and tail consumers remain separate targets. Exercise 15.7 is now
decomposed conservatively: its generic measurable-observation data-processing
leaf compiles, while the stopped-history KL bound and final `F_tau` consumer
remain an explicit open problem.
