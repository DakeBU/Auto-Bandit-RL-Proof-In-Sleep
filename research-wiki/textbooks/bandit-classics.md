...
| `TXT-LS-2020-LEMMA-15-1-DIVERGENCE-DECOMPOSITION` | Ch. 15, §15.1, Lemma 15.1 / Eq. (15.1), printed pp. 198--199 / PDF pp. 207--208 | for one common possibly randomized nonanticipating policy, `D(P_nu^pi,P_nu'^pi)=sum_i E_nu^pi[T_i(n)]D(P_i,P_i')` with first-law pulls and KL direction | exact target frozen; blocked on conditional composition-product KL integral and stochastic-policy history law |
| `TXT-LS-2020-THM-15-2-GAUSSIAN-MINIMAX` | Ch. 15, §15.2, Theorem 15.2, printed pp. 199--201 / PDF pp. 208--210 | for `k>1`, `n>=k-1`, every policy has a unit-variance Gaussian mean vector in `[0,1]^k` with expected regret at least `sqrt((k-1)n)/27` | unit-Gaussian RN, integrability, exact arm KL, and changed-arm `2 Delta^2` cost compiled; history and minimax terminals blocked |
