...
| `BRL-OP-RL-BELLMAN-001` | finite-horizon RL | Bellman/value/regret interface | finite MDP/Bellman/occupancy foundations and the canonical known-reward generated Hoeffding UCBVI-CH route compile through aggregate same-prefix counts, previous-Q clipping, same-law confidence, optimism, raw cumulative episode pseudo-regret, the frozen `20/250` high-probability terminal, and the failure-aware expectation consumer | next choose one honest extension: Bernstein/variance-aware minimax UCB-VI, stochastic-reward UCBVI, realized sampled-return regret, posterior-sampling RL, or continuous/model-free RL; natural-causal consistency and stopping-time routes remain separate compiled branches |
