...
- **2026-08 — Third source-frozen external theorem audit.** The Baudry--Johnson--Vary--Pike-Burke--Rebeschini NeurIPS 2025 stochastic-gradient-bandit audit now compiles 26 named declarations for the finite-action, history-conditioned algebra underlying Algorithm 1 and Equations (3)--(7): softmax positivity/normalization, the exact selected/nonselected zero-sum update, the finite conditional-mean increment in policy-value and gap coordinates, the best-arm cumulative minimum-gap lower bound, and the post-convergence plus squared-failure-mass decomposition. This is the source algebra after fixing a pre-action history, not a recursive SGB trajectory or conditional-expectation theorem; the learning-rate thresholds, failure-probability estimates, and Theorems 1--4 remain uncompiled.
