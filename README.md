...
- **2026-09 — SGB missing-pull count bridge.** The finite phase split now feeds a compiled missing-pull-to-terminal-count theorem; its expected-regret consumer, future/no-return law, ballot bound, asymptotic assembly, and Theorem 2 remain open.
...
- **2026-09 — SGB Appendix-C probability dichotomy and missing-pull count bridge.** Ten declarations split the pure latent `S0/S1` probability exactly into the source-generated all-pulls-present event and an explicit measurable missing-pull event. A downstream compiled theorem now sends a missing requested pull, represented by an actual `WithTop.top` coordinate, into the measurable finite-horizon terminal-count-below event. This remains a deterministic interface: it does not supply the expected-regret consumer, the source fixed-cutoff trigger, selected IID, a positive probability bound, future/no-return, ballot probability, asymptotic assembly, or Theorem 2.
