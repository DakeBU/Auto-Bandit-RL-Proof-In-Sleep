...
- **2026-08 — Part IV Chapters 13--16 lower-bound spine.** The separate source-faithful textbook layer compiles Chapter 13 minimax semantics, least-explored-arm and error-bearing two-environment algebra; Chapter 14 extended-real relative entropy, event-level binary data processing, endpoint-complete testing, and unconditional Bretagnolle--Huber; Chapter 15 unit-Gaussian likelihood-ratio/KL plus exact gap/exponent tuning dependencies; and Chapter 16 exact consistency/`d_inf` interfaces, Gaussian alternative costs, and subpolynomial power/log-growth dependencies. Lemma 15.1, the `1/27` minimax terminal, and Chapter 16's bandit information/liminf/finite-time terminals remain blocked on a conditional-kernel KL integral and a canonical stochastic-policy history law. Compiled dependencies are not reported as source terminals.
