...
- **2026-08 — SGB Theorem-2 action/readout interface and count-cap scaffold.** Thirteen new indexed declarations separate the already valid action half from the still-missing reward-freshness producer. The latent mixture's next action follows the history policy, and the observed successor reward equals the selected latent coordinate almost surely. A finite/sub-Markov consumer derives a branch product law only when supplied the explicitly typed branch-locality contract; that contract is not proved. The count-cap set, its measurability, and the exact pull-count extension recurrence now compile, narrowing the next producer to a restricted-measure induction. Branchwise freshness, native-prefix and visible-law identification, selected-reward IID, future-cylinder/no-return, ballot/asymptotic assembly, and Theorem 2 remain blocked.
- **2026-08 — SGB Theorem-2 deferred-decisions prefix factorization.** Eight new indexed declarations prove the exact finite stream-box product law, kernel-law locality of the latent generated trajectory prefix to that box, a Markov visible-prefix kernel, and the exact joint finite stream-box/visible-prefix mixture. This is a compiled finite-prefix factorization, not the still-missing identification of its visible marginal with the native fixed-IID SGB prefix. Branchwise prefix-plus-next-pair freshness, the visible-marginal/native-prefix identification, the native visible law, conditional no-return probability, ballot phase, asymptotic assembly, and the Theorem-2 terminal remain blocked.
- **2026-08 — SGB Theorem-2 latent reward product bridge.** Seven indexed declarations prove a fixed arm's finite latent-prefix product law, lift it through the exact stream marginal of the coupled SGB trajectory, specialize it to the optimal arm, and identify every finite nth-pull reward with its corresponding latent coordinate almost surely. Two normalization leaves feed the later finite-prefix factorization. The work does not make totalized or occurrence-conditioned stopped rewards IID: native prefix and visible-law equality, the stopped-prefix future cylinder, ballot phase, and polynomial terminal remain blocked.
