...
- **2026-08 — SGB Theorem-2 branch-locality producer.** Twenty-eight new indexed declarations—two reusable restricted-semidirect-product measure bridges and 26 route-specific leaves—extend the preceding 13-declaration action/readout scaffold. A count-capped restricted-measure induction now proves `latentArmStreamVisiblePrefixNextActionBranchLocality`, discharging the typed `LatentArmStreamVisiblePrefixNextActionBranchLocality` contract, and an unconditional theorem derives the exact branchwise product law for the selected latent coordinate. This is not yet an aggregate selected-reward freshness or IID theorem: visible-marginal/native-prefix identification, the native visible law, future-cylinder/no-return, ballot/asymptotic assembly, and Theorem 2 remain blocked.
