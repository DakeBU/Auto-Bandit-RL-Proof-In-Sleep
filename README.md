...
- **2026-09 — SGB Appendix-C phase-event transport.** Fourteen new declarations formalize the finite `S0/S1` reward pattern, including the unlucky all-`-1` start, exact recovery terminal sum, and every nonpositive recovery prefix. Separate measurable events keep the all-pulls-present boundary explicit and transport the source generated-process event to the corresponding latent event. The latent event still intersects the adaptive occurrence event, so no product law or occurrence-conditioned IID claim is made; the missing-pull/all-present probability dichotomy, future/no-return, ballot probability, asymptotic assembly, and Theorem 2 remain open.
