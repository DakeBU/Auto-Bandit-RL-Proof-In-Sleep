...
- **2026-08 — Second source-frozen external theorem audit.** The Zeng--Honorio NeurIPS 2025 succinct stochastic-bandit audit now compiles 36 named declarations for the symmetric unit-atom system, the literal succinct-support contract, support orthogonality, the source-shaped `Q` and `R`, and Lemmas 3.1--3.2 on succinct combinations. A compiled diagnostic shows that the globally real-valued `R` candidate set is unbounded when a nonzero ambient vector is orthogonal to every atom. This records a regularity/codomain obligation rather than declaring the source incorrect; Lemmas 3.3--3.6, Assumption 3.7, Theorem 3.8, and its regret endpoints remain uncompiled.
