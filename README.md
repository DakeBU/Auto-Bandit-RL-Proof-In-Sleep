...
- **2026-09 — Chapter 14 information-theory acceptance.** The frozen required body is compiled: Huffman global optimality and its entropy sandwich, finite uniquely-decodable/prefix-code equivalence including Kraft equality, exact-real arithmetic block coding and its entropy-rate converse, finite-alphabet KL with exact support endpoints, finite-discretisation/RN equivalence, common-density and source affinity/overlap bounds, Gaussian testing, and full sub-sigma-algebra data processing. Event DPI and unconditional Bretagnolle--Huber remain intact. Coding uses nonempty words and classical exact-real constructions; arbitrary-cardinality uniform fixed-length optimality is explicitly qualified, not asserted. Independent source review, PR #106, main run 33959196451, Pages and live desktop/mobile acceptance passed; see [acceptance evidence](reviews/2026-09-05-chapter-14-live-acceptance.md). Optional notes/exercises and Chapter 15 history KL are not claimed as Chapter 14 completion.
- **2026-09 — Part IV Chapters 13--17 lower-bound spine.** The separate source-faithful textbook layer compiles Chapter 13 minimax semantics and fixed-class minimax-optimality, the exact empirical-mean pushforward for a canonical finite iid Gaussian product, both Gaussian midpoint error events and their explicitly weaker `exp(-n*Delta^2/8)` maximum-risk Chernoff companion, least-explored-arm and error-bearing two-environment algebra; Chapter 14 extended-real relative entropy, event-level binary data processing, endpoint-complete testing, and unconditional Bretagnolle--Huber; and Chapter 15's general finite-arm, same-randomized-policy history KL decomposition (Lemma 15.1) together with the exact unit-Gaussian Theorem 15.2 existence and minimax chain, including the `1/27` constant. This also yields Chapter 13's source-order statement with explicit universal constant `1/54`. The frozen Chapter 13 main-text contract is complete: the exact two-sided Mills-ratio Eqs. (13.4) and (13.1) now compile, and the broad gaps-in-[0,1] fixed-horizon MOSS near-minimax consequence now compiles; PR #105, main run 33953476610, Pages and live desktop/mobile acceptance pass, while Notes/Exercises remain optional and unformalized. Chapter 16 now compiles the finite-mean source environment, exact Definition 16.1, Theorem 16.2, Lemma 16.3, and Theorem 16.4, preserving all finite/infinite information branches. Chapter 17 now compiles Theorem 17.1, Corollaries 17.2–17.3, Claims 17.5–17.7, Eq. (17.8), and the deterministic-matrix CDF terminal. Explicit user-approved corrections: Claim 17.6 uses T_i <= n/2; Theorem 17.4 uses 0 < delta <= 1/32, c=1/160, C=64. These are not proofs of the unchanged printed statements. Chapter 15's optional notes/exercises are not claimed complete, and compiled dependencies are never reported as later Chapter 16--17 source terminals.
- **Chapter 15 optional extension:** Generic measurable-map KL contraction and its deterministic-history corollary compile; Exercise 15.7 remains partial pending stopped-history information and F_tau factorization.
