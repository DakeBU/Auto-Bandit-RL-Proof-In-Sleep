...
- **2026-09 — Chapter 14 information-theory extension.** The source-faithful layer now compiles finite binary prefix-code and entropy definitions, a proof that the codebook is uniquely decodable, a Kraft--McMillan adapter, KL separation, arbitrary finite-alphabet Eq. (14.4) with exact zero/support endpoints, the finite-discretisation/RN equivalence in Eq. (14.5), and the full finite-measure sub-sigma-algebra data-processing inequality from Exercise 14.10. The earlier event DPI and unconditional Bretagnolle--Huber remain intact. Whole-chapter status stays `partial`: Huffman optimality/the one-bit entropy sandwich, asymptotic source coding, common-density/measure-overlap route, and general Gaussian application are not compiled.
