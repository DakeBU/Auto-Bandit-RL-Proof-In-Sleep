...
LOCAL_LEAF_CARDS = [
    ...
    },
    {
        ...
        "role": "Compiled project-local named finite pair-trace hookup for the generated-history concrete pair-law consumer: History now exposes the Finset.Iic-indexed action/reward pair-coordinate trace prefix and its measurability/projection equality, and ConditionalExpectationReward states the remaining condExpKernel pair-law hypothesis using History.finitePairHistoryOfTrace. This aligns the consumer with RewardKernel.actionRewardPartialTrajectoryKernel's pair-coordinate prefix shape; the actual condExpKernel pair-law identity remains open.",
        "mathlib_routes": [
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-FINITE-HISTORY-PRODUCT-MEASURABILITY",
    ...
    },
    {
        ...
        "role": "Compiled generated-action plus actual/random-pair reward-law hookup for the COND-EXPECT-REWARD route: if the conditional law is stated as an actual-action pair-product law, Prod.snd marginalization gives the actual-action reward-coordinate law and a reusable adapter exposes the resulting full finite-pair-trace partialTraj law; if it is stated for the fully random next pair, the generated action trace first freezes the action coordinate with Measure.map_congr and a second reusable adapter exposes the same full-trace law. The route then rewrites the selected action to the policy action, combines it with the split-law builder to produce the full next-pair condExpKernel law, pushes that through the extension-map partialTraj builder, and provides succ-indexed conditional mean-zero consumers under integrability; it now also consumes explicit generated-action equality plus either reward-coordinate law or actual-action pair-product law and raw/mean range regularity directly into mean-zero without first packaging a source or passing a separate integrability hypothesis. This still assumes the pair/reward law source and does not prove the full ambient trajectory-to-condExpKernel identification.",
        "mathlib_routes": [
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-COND-EXPECT-REWARD-NEXTPAIR-SPLIT-LAW-BUILDER",
            "LOCAL-LEAF-COND-EXPECT-REWARD-ACTION-FREEZE-GENERATED-TRACE-SOURCE",
    ...
    },
    {
        ...
        "role": "Compiled generated-policy actual reward-coordinate map source contract for the COND-EXPECT-REWARD route: it packages shifted generated-action equality plus the per-step conditional law of only the next reward coordinate under the actual next action, then reuses the existing actual reward-map route to expose full finite-pair-trace partialTraj law and succ-indexed conditional mean-zero under integrability; it now also directly consumes the same source plus raw reward and selected-mean range regularity into succ-indexed conditional mean-zero without a separate integrability hypothesis. This is weaker than the random next-pair source and still assumes the actual reward-coordinate law, ambient trajectory-to-condExpKernel identification, raw/mean range regularity or integrability, and final adaptive theorem.",
        "mathlib_routes": [
            "MLIB-CONDITIONAL-EXPECTATION",
    ...
    },
    {
        ...
        "role": "Compiled definitional generated-action actual reward-coordinate map source contract for the COND-EXPECT-REWARD route: it removes the explicit action trace and timewise action-measurability inputs from the actual reward-map source by using generatedActionFromRewardHistory plus measurable reward-history state extractors, then converts to the existing actual reward-map source to expose full finite-pair-trace partialTraj law and succ-indexed conditional mean-zero under integrability; it now also directly consumes the definitional actual reward-map source plus raw reward and selected-mean range regularity into succ-indexed conditional mean-zero without a separate integrability hypothesis. This still assumes the actual reward-coordinate law, ambient trajectory-to-condExpKernel identification, raw/mean range regularity or integrability, and final adaptive theorem.",
        "mathlib_routes": [
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-COND-EXPECT-REWARD-GENERATED-ACTUAL-REWARD-MAP-SOURCE-CONTRACT",
            "LOCAL-LEAF-COND-EXPECT-REWARD-ACTION-FREEZE-GENERATED-TRACE-SOURCE",
    ...
    },
    {
        ...
        "role": "Compiled generated-policy random next-pair law source contract for the COND-EXPECT-REWARD route: it packages shifted generated-action equality plus the per-step random next-pair condExpKernel map law as a reusable structure, then consumes the existing generated-action/random-pair route to expose full finite-pair-trace partialTraj law and succ-indexed conditional mean-zero. This is a contract surface; it still does not construct the pair/reward law source, ambient trajectory-to-condExpKernel identification, integrability, or a final adaptive theorem.",
        "mathlib_routes": [
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-COND-EXPECT-REWARD-NEXTPAIR-GENERATED-ACTION-ACTUAL-REWARD-HOOKUP",
            "LOCAL-LEAF-COND-EXPECT-REWARD-ACTION-FREEZE-GENERATED-TRACE-SOURCE",
    ...
    },
    {
        ...
        "role": "Compiled definitional generated-action map source contract for the COND-EXPECT-REWARD route: it removes the explicit action-trace equality field from GeneratedActionRandomPairMapSource by defining the action trace as the shifted policy-generated trace over finite reward histories, derives timewise action measurability from measurable reward-history state extractors and reward traces, converts to the existing random-pair map source, exposes adapters from full finite-pair and frozen-prefix extension-map partialTraj law hypotheses into that source contract, and exposes full finite-pair-trace partialTraj law plus succ-indexed conditional mean-zero consumers. This still assumes the ambient trajectory-to-condExpKernel law shape as an input and does not construct it, prove integrability from kernels alone, or prove a final adaptive theorem.",
        "mathlib_routes": [
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-FINITEPAIRTRACE-CONSUMER",
            "LOCAL-LEAF-COND-EXPECT-REWARD-PARTIALTRAJ-EXTEND-MAP-CONSUMER",
    ...
    },
    {
        ...
        "role": "Compiled definitional generated-action raw-range/measurable-mean-range source contract for the COND-EXPECT-REWARD route: it removes the explicit action trace and timewise action-measurability inputs from the practical raw-range source by using generatedActionFromRewardHistory plus GeneratedActionRandomPairDefinitionalMapSource; it now also builds that practical top-layer source directly from full finite-pair and frozen-prefix extension-map partialTraj law hypotheses plus raw/mean range regularity, converts to the existing raw-range source to expose integrability and full finite-pair-trace partialTraj law, directly consumes the frozen-prefix extension-map law plus regularity into succ-indexed conditional mean-zero, directly consumes the canonical history-step next-pair law plus regularity into the same mean-zero fact, and directly consumes the generated-action actual-action reward-coordinate selected-measure law plus regularity into mean-zero. This still assumes the reward-coordinate/next-pair/ambient trajectory-to-condExpKernel law shape as an input and does not construct sub-Gaussian witnesses or final adaptive theorem.",
        "mathlib_routes": [
            "MLIB-MEASURE-INTEGRAL",
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-MAP-SOURCE-CONTRACT",
    ...
    },
    {
        ...
        "role": "Compiled project-local regularity leaf for the COND-EXPECT-REWARD route: the practical definitional raw-reward-range/measurable-mean-range generated-policy random next-pair source now exposes centered successor reward a.e. measurability and the centered interval bound directly, by lowering through the existing explicit raw-range source conversion and reusing the raw-range regularity consumers. It still assumes the definitional random next-pair law, mean measurability, deterministic raw reward and mean range bounds, ambient trajectory-to-condExpKernel identification, and final adaptive theorem.",
        "mathlib_routes": [
            "MLIB-MEASURE-INTEGRAL",
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-RAW-RANGE-MEASURABLE-MEAN-RANGE-BOUNDED-SOURCE-CONTRACT",
...
def cmd_unfinished(args: argparse.Namespace) -> int:
    ...
    print("- COND-EXPECT-REWARD-PAIR-MAP-CONSUMER is compiled locally as the next action/reward pair-law consumer: a condExpKernel pushforward identity to RewardKernel.actionRewardHistoryStepKernelFamily now marginalizes through Prod.snd into the reward-coordinate map-law consumer; the actual partialTraj-to-condExpKernel pair-law identification remains open.")
    print("- COND-EXPECT-REWARD-PAIR-MAP-HISTORYFILTRATION-HOOKUP is compiled locally as the generated History.historyFiltrationSucc specialization of the pair-law consumer; next-coordinate and prefix measurability are supplied by local history-filtration APIs, while the actual condExpKernel pair-law identity remains open.")
    ...
    print("- COND-EXPECT-REWARD-GENERATED-ACTUAL-REWARD-MAP-SOURCE-CONTRACT is compiled locally as the narrower source contract that packages only the actual next-action reward-coordinate conditional map law, then reuses the existing generated-action actual reward-map route for finite-pair-trace partialTraj law and conditional mean-zero; it now also consumes raw/mean range regularity directly into mean-zero without a separate integrability hypothesis; it still assumes that reward-coordinate law and ambient trajectory-to-condExpKernel identification.")
    ...
    print("- COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-ACTUAL-REWARD-MAP-SOURCE-CONTRACT is compiled locally as the definitional generated-action variant of the actual reward-map source: it removes explicit action-trace/haction inputs by using generatedActionFromRewardHistory and measurable reward-history state extractors, reuses the actual reward-map consumers, and now directly consumes the source plus raw/mean range regularity into succ-indexed conditional mean-zero without a separate integrability hypothesis; it still assumes the actual reward-coordinate law and ambient trajectory-to-condExpKernel identification.")
    ...
    print("- COND-EXPECT-REWARD-GENERATED-RANDOM-PAIR-SOURCE-CONTRACT is compiled locally as a reusable generated-policy random next-pair law source package plus consumers for finite-pair-trace partialTraj law and succ-indexed conditional mean-zero; it still assumes the pair/reward law source, integrability, and ambient trajectory-to-condExpKernel identification.")
    ...
    print("- COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-MAP-SOURCE-CONTRACT is compiled locally as the variant that defines the action trace as the shifted policy-generated trace over finite reward histories and derives its timewise measurability from measurable state extractors plus reward traces; it now also exposes source constructors from full finite-pair and frozen-prefix extension-map partialTraj law hypotheses, then reuses the generated random-pair map source consumers; it still assumes the ambient trajectory-to-condExpKernel law shape and does not construct it.")
    ...
    print("- COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-RAW-RANGE-MEASURABLE-MEAN-RANGE-CENTERED-REGULARITY is compiled locally as a regularity leaf: a definitional raw-reward-range/measurable-mean-range bounded generated random-pair source now exposes centered successor reward a.e. measurability and the centered interval bound directly; it still assumes the definitional raw-range/measurable-mean-range source fields and random pair law.")
