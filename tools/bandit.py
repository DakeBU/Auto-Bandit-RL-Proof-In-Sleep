...
def cmd_unfinished(args: argparse.Namespace) -> int:
    ...
    print("- COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-ACTUAL-REWARD-MAP-SOURCE-CONTRACT is compiled locally as the definitional generated-action variant of the actual reward-map source: it removes explicit action-trace/haction inputs by using generatedActionFromRewardHistory and measurable reward-history state extractors, then reuses the actual reward-map consumers; it still assumes the actual reward-coordinate law, integrability, and ambient trajectory-to-condExpKernel identification.")
