...
LOCAL_LEAF_CARDS = [
    ...
    },
    {
        ...
        "role": "Compiled count-facing UCB score-argmax bridge: sums the single-time concrete large-gap selected-arm probability bounds over an explicit finite time set, lifts the result through the existing finite-sum selected-action indicator lower-integral API, yielding a selected-time count budget `|times| * delta`, specializes `times = Finset.range T` through the existing recursive `pullCount` lower-integral identity to bound the selected arm's horizon pull count by `T * delta` when the large-gap condition holds throughout the horizon, exposes a threshold/suffix-shaped pull-count split charging explicit `freeTimes` by `1` and all other horizon times by `delta`, consumes an abstract free-time budget to produce `freeBudget + T * delta`, discharges that budget with `freeTimes.card` through a generic free-time indicator cardinality wrapper, adds a pathwise selected-small-pull-count budget proving selected occurrences with prior `pullCount < B` sum to `min (pullCount T) B` and hence at most `B`, lifts that selected-small budget to a probability-facing lower-integral bound under an arbitrary probability measure, splits selected pulls into selected-small and selected-large-count indicators, and exposes the pointwise ENNReal budget `pullCount <= B + selectedLargeCount`, instantiates the split with concrete textbook-radius large-gap/free-time Finsets, adds a threshold consumer yielding `B + T * delta` once all horizon times `t >= B` satisfy the large-gap radius condition, exposes the standard half-gap threshold adapter, exposes square/eight-proxy-log threshold consumers, adds a proxy-small threshold consumer under positive log scale, adds a sample-count threshold consumer from `proxy <= varianceProxy / count`, and adds a lower-bound-on-count consumer using a global threshold `B`. Empirical-mean construction, history-recursive UCB action trace, integration of the selected-large-count source with UCB large-gap tail bounds, and final regret remain separate.",
