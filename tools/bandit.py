...
LOCAL_LEAF_CARDS = [
    ...
    },
    {
        ...
        "role": "Compiled definitional generated-action actual reward-coordinate map source contract for the COND-EXPECT-REWARD route: it removes the explicit action trace and timewise action-measurability inputs from the actual reward-map source by using generatedActionFromRewardHistory plus measurable reward-history state extractors, can package either a policy-selected reward-coordinate law or a full finite-pair partialTraj law into the actual generated-successor reward-map source, then converts to the existing actual reward-map source to expose full finite-pair-trace partialTraj law and succ-indexed conditional mean-zero under integrability; it now also directly consumes the definitional actual reward-map source plus raw reward and selected-mean range regularity into succ-indexed conditional mean-zero without a separate integrability hypothesis. This still assumes the reward-coordinate or ambient trajectory-to-condExpKernel/partialTraj law, raw/mean range regularity or integrability, and final adaptive theorem.",
        "mathlib_routes": [
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-COND-EXPECT-REWARD-GENERATED-ACTUAL-REWARD-MAP-SOURCE-CONTRACT",
            "LOCAL-LEAF-COND-EXPECT-REWARD-ACTION-FREEZE-GENERATED-TRACE-SOURCE",
...
def cmd_unfinished(args: argparse.Namespace) -> int:
    ...
    print("- COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-ACTUAL-REWARD-MAP-SOURCE-CONTRACT is compiled locally as the definitional generated-action variant of the actual reward-map source: it removes explicit action-trace/haction inputs by using generatedActionFromRewardHistory and measurable reward-history state extractors, can package either a policy-selected reward-coordinate law or a full finite-pair partialTraj law into the actual generated-successor reward-map source, reuses the actual reward-map consumers, and now directly consumes the source plus raw/mean range regularity into succ-indexed conditional mean-zero without a separate integrability hypothesis; it still assumes the reward-coordinate or ambient trajectory-to-condExpKernel/partialTraj law.")
