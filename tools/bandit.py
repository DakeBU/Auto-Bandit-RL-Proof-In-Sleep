...
def cmd_unfinished(args: argparse.Namespace) -> int:
    ...
    print("- UCB-CONCRETE-SCORE-ARGMAX-COUNT-BUDGET is compiled locally as a count-facing bridge: finite-time-set selected large-gap probabilities for concrete UCB score argmax sum to at most `|times| * delta`, the same budget controls the lower integral of the selected-time indicator count, the `Finset.range T` specialization bounds recursive selected-arm pullCount by `T * delta` under an all-horizon large-gap contract, and a threshold/suffix split charges free times by `1` and all other large-gap times by `delta`; concrete radius-threshold cardinality and final regret remain separate.")
