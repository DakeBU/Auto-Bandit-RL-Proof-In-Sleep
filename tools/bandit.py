...
LOCAL_LEAF_CARDS = [
    ...
    },
    {
        ...
        "role": "Compiled definitional generated-action raw-range/measurable-mean-range source contract for the COND-EXPECT-REWARD route: it removes the explicit action trace and timewise action-measurability inputs from the practical raw-range source by using generatedActionFromRewardHistory plus GeneratedActionRandomPairDefinitionalMapSource; it now also builds that practical top-layer source directly from full finite-pair and frozen-prefix extension-map partialTraj law hypotheses plus raw/mean range regularity, then converts to the existing raw-range source to expose integrability, full finite-pair-trace partialTraj law, and succ-indexed conditional mean-zero. This still assumes the ambient trajectory-to-condExpKernel law shape as an input and does not construct sub-Gaussian witnesses or final adaptive theorem.",
        "mathlib_routes": [
            "MLIB-MEASURE-INTEGRAL",
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-MAP-SOURCE-CONTRACT",
