...
def cmd_unfinished(args: argparse.Namespace) -> int:
    ...
    print("- COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-MAP-SOURCE-CONTRACT is compiled locally as the variant that defines the action trace as the shifted policy-generated trace over finite reward histories and derives its timewise measurability from measurable state extractors plus reward traces; it now also exposes source constructors from full finite-pair and frozen-prefix extension-map partialTraj law hypotheses, then reuses the generated random-pair map source consumers including the raw/mean range regularity mean-zero route; it still assumes the ambient trajectory-to-condExpKernel law shape and does not construct it.")
