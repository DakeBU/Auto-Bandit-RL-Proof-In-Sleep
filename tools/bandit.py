...
LOCAL_LEAF_CARDS = [
    ...
    },
    {
        ...
        "role": "Compiled project-local conditional-MGF consumer for COND-EXPECT-REWARD: a generic condExpKernel pushforward-map equality plus a target HasSubgaussianMGF witness gives Mathlib HasCondSubgaussianMGF, and the history-step specialization feeds RewardKernel.historyStepKernelFamily_centeredReward_hasSubgaussianMGF through a frozen-past centered-variable equality plus an explicit deterministic variance-proxy upper bound. Exponential integrability, reward-coordinate map law, frozen-past equality, and variance bound remain explicit contracts; this does not construct the ambient partialTraj/history-to-condExpKernel law, arbitrary adaptive policy predictability, or final adaptive tail/regret theorem.",
        "mathlib_routes": [
...
def cmd_unfinished(args: argparse.Namespace) -> int:
    ...
    print("- COND-EXPECT-REWARD-HISTORYSTEP-CONDEXPKERNEL-COND-MGF-CONSUMER is compiled locally as the conditional sub-Gaussian analogue: a condExpKernel pushforward map law plus frozen-past centered-variable equality, explicit exponential integrability, and a deterministic variance-proxy upper bound yield Mathlib HasCondSubgaussianMGF; source-level policy wrappers and trajectory-law construction remain open.")
