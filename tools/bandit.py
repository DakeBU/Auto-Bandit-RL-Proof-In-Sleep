...
LOCAL_LEAF_CARDS = [
    ...
    },
    {
        ...
        "role": "Compiled count-facing UCB score-argmax bridge: sums the single-time concrete large-gap selected-arm probability bounds over an explicit finite time set, lifts the result through the existing finite-sum selected-action indicator lower-integral API, yielding a selected-time count budget `|times| * delta`, specializes `times = Finset.range T` through the existing recursive `pullCount` lower-integral identity to bound the selected arm's horizon pull count by `T * delta` when the large-gap condition holds throughout the horizon, exposes a threshold/suffix-shaped pull-count split charging explicit `freeTimes` by `1` and all other horizon times by `delta`, consumes an abstract free-time budget to produce `freeBudget + T * delta`, discharges that budget with `freeTimes.card` through a generic free-time indicator cardinality wrapper, instantiates the split with concrete textbook-radius large-gap/free-time Finsets, adds a threshold consumer yielding `B + T * delta` once all horizon times `t >= B` satisfy the large-gap radius condition, exposes the standard half-gap threshold adapter, exposes square/eight-proxy-log threshold consumers, and adds a proxy-small threshold consumer under positive log scale. Empirical-mean construction, history-recursive UCB action trace, concrete sample-count proof of the proxy-small condition for `B`, and final regret remain separate.",
