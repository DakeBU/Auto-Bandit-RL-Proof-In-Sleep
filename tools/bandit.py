...
LOCAL_LEAF_CARDS = [
    ...
    },
    {
        ...
        "role": "Compiled count-facing UCB score-argmax bridge: sums the single-time concrete large-gap selected-arm probability bounds over an explicit finite time set, lifts the result through the existing finite-sum selected-action indicator lower-integral API, yielding a selected-time count budget `|times| * delta`, and specializes `times = Finset.range T` through the existing recursive `pullCount` lower-integral identity to bound the selected arm's horizon pull count by `T * delta` when the large-gap condition holds throughout the horizon. Empirical-mean construction, history-recursive UCB action trace, threshold/suffix decomposition, and final regret remain separate.",
...
def cmd_unfinished(args: argparse.Namespace) -> int:
    ...
    print("- UCB-CONCRETE-SCORE-ARGMAX-COUNT-BUDGET is compiled locally as a count-facing bridge: finite-time-set selected large-gap probabilities for concrete UCB score argmax sum to at most `|times| * delta`, the same budget controls the lower integral of the selected-time indicator count, and the `Finset.range T` specialization bounds recursive selected-arm pullCount by `T * delta` under an all-horizon large-gap contract; threshold/suffix decomposition and final regret remain separate.")
