...
LOCAL_LEAF_CARDS = [
    ...
    },
    {
        ...
        "role": "Compiled definitional generated-action map source contract for the COND-EXPECT-REWARD route: it removes the explicit action-trace equality field from GeneratedActionRandomPairMapSource by defining the action trace as the shifted policy-generated trace over finite reward histories, derives timewise action measurability from measurable reward-history state extractors and reward traces, converts to the existing random-pair map source, and exposes full finite-pair-trace partialTraj law plus succ-indexed conditional mean-zero consumers. This still assumes the random next-pair law, ambient trajectory-to-condExpKernel identification, integrability, and final adaptive theorem.",
        "mathlib_routes": [
            "MLIB-CONDITIONAL-EXPECTATION",
            "LOCAL-LEAF-COND-EXPECT-REWARD-GENERATED-RANDOM-PAIR-SOURCE-CONTRACT",
            "LOCAL-LEAF-COND-EXPECT-REWARD-ACTION-FREEZE-GENERATED-TRACE-SOURCE",
...
def cmd_unfinished(args: argparse.Namespace) -> int:
    ...
    print("- COND-EXPECT-REWARD-GENERATED-DEFINITIONAL-MAP-SOURCE-CONTRACT is compiled locally as the variant that defines the action trace as the shifted policy-generated trace over finite reward histories and derives its timewise measurability from measurable state extractors plus reward traces, then reuses the generated random-pair map source consumers; it still assumes the random next-pair law and ambient trajectory-to-condExpKernel identification.")
