...
METHOD_AMENDMENT_SHA256 = (
    "888b5cbdc4c8165789d5967ef1e034bc532a125530a483476100b9ba89ea2ab5"
)
METHOD_AMENDMENT_ID = (
    "ABRL-TARGET-DRIFT-FIXED-BENCHMARK-ITT-METHOD-AMENDMENT-2026-08-26"
)
PLACEHOLDERS = (
    "{{CASE_ID}}",
    "{{SOURCE_ID}}",
...
def validate_method_amendment(
    ...
    require(
        ...
        == fixed_method.get("exact_sign_flip_sensitivity")
        and analysis.get("secondary_multiplicity")
        == fixed_method.get("secondary_multiplicity")
        and analysis.get("fresh_invocation_semantics")
        == fixed_method.get("fresh_invocation_semantics")
        and analysis.get("target_weighted_rate_role")
        == fixed_method.get("target_weighted_rate_role")
        and analysis.get("raw_run_weighted_rate_role")
        == fixed_method.get("raw_run_weighted_rate_role"),
        "execution analysis text differs from the fixed-benchmark amendment",
    )
    bindings = amendment.get("bindings", {})
    protocol_binding = bindings.get("unchanged_primary_protocol", {})
    script_binding = bindings.get("analysis_script_after_amendment", {})
    test_binding = bindings.get("analysis_tests_after_amendment", {})
    require(
