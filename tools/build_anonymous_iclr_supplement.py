...
SGB_THEOREM_TWO_NATIVE_LAW_FILE = (
    "BanditRLProof/Algorithms/"
    "StochasticGradientBanditTheoremTwoNativePrefix.lean"
)
SGB_THEOREM_TWO_SELECTED_BLOCK_FILE = (
    "BanditRLProof/Algorithms/"
...
SGB_THEOREM_TWO_NATIVE_LAW_SOURCE_DECLARATION_COUNT = 10
SGB_THEOREM_TWO_NATIVE_LAW_INDEXED_DECLARATIONS = frozenset({
...
SGB_THEOREM_TWO_SELECTED_BLOCK_SOURCE_DECLARATION_COUNT = 8
SGB_THEOREM_TWO_SELECTED_BLOCK_INDEXED_DECLARATIONS = frozenset({
...
TARGET_DRIFT_TOOLS = (
    ...
    "tools/test_target_drift_agent_production_action.py",
...
TARGET_DRIFT_PROTOCOL_FILES = (
    ...
    "evaluation/target-drift-v2/agent-production-action-fixture/prompt.md",
    "evaluation/target-drift-v2/agent-production-action-fixture/request.json",
    ...
    "evaluation/target-drift-v2/human-source-contract-external-trust-anchor-contract.json",
    "evaluation/target-drift-v2/human-source-contract-external-verification-protocol.json",
    "evaluation/target-drift-v2/human-source-contract-review-protocol.json",
    "evaluation/target-drift-v2/method-amendment-fixed-target-paired-bootstrap-2026-08-31.json",
    "evaluation/target-drift-v2/external-comparator-plan.json",
    "evaluation/target-drift-v2/external-comparator-plan.seal.json",
    LEANFLOW_ADAPTER_CONTRACT,
...
def validate_sgb_count(records, index):
    ...
    if (
        ...
        raise ValueError(
            ...
            "freshness compile in the frozen ledger. A separate ten-declaration "
            "module compiles native-prefix identification and the full "
            "visible/native trajectory law; stopped or pull-ordered selected "
            "IID, the stopped-prefix future-cylinder, conditional no-return, "
            "ballot phase, and the Theorem-2 terminal must remain blocked"
        )
    return {
    ...
    for row_id, spec in specs.items():
        ...
        if row_id == "stochastic-gradient-bandit-source-frozen-audit":
            native_law_names = {
                index_row["full_name"]
                for index_row in index.get("declarations", [])
                if index_row.get("file") == SGB_THEOREM_TWO_NATIVE_LAW_FILE
            }
            native_law_source = read_regular(
                SGB_THEOREM_TWO_NATIVE_LAW_FILE
            ).decode("utf-8")
            native_law_source_count = len(re.findall(
                r"(?m)^(?:def|theorem|instance)\s+",
                native_law_source,
            ))
            if (
                native_law_names != SGB_THEOREM_TWO_NATIVE_LAW_INDEXED_DECLARATIONS
                or native_law_source_count !=
                    SGB_THEOREM_TWO_NATIVE_LAW_SOURCE_DECLARATION_COUNT
            ):
                raise ValueError(
                    "separate SGB native-law module declaration drift"
                )
            selected_block_names = {
                index_row["full_name"]
                for index_row in index.get("declarations", [])
                if index_row.get("file") == SGB_THEOREM_TWO_SELECTED_BLOCK_FILE
            }
            selected_block_source = read_regular(
                SGB_THEOREM_TWO_SELECTED_BLOCK_FILE
            ).decode("utf-8")
            selected_block_source_count = len(re.findall(
                r"(?m)^(?:def|theorem|instance)\s+",
...
def build_claim_ledger(proof_report):
    ...
    sgb_comparison = next(
        row for row in theorem_audit_comparison["rows"]
        if row["id"] == "stochastic-gradient-bandit-source-frozen-audit"
    )
