...
-- Same noisy stream and unchanged estimator, with the strictly stronger bound.
theorem upper_sharp :
    (UCB.armStreamMeasure kernel).real {stream |
      4*(2 : ℝ)^(1/(1+1 : ℝ))*(Real.log (1/(1/20 : ℝ))/1000)^(1/(1+1 : ℝ)) ≤
        (∑ i ∈ Finset.range 1000,
          truncate (sourceTruncationThreshold 1 2 (Real.log (1/(1/20 : ℝ))) i) (stream i 0))/1000 - 1}
      ≤ Real.exp (-(5/4 : ℝ)*Real.log (1/(1/20 : ℝ))) := by
  apply source_truncated_mean_upper_tail_log_sharp (UCB.armStreamMeasure kernel)
    (fun i stream => stream i 0) 1 2 (Real.log (1/(1/20 : ℝ))) 1 1000 (by decide)
    (fun i => (measurable_pi_apply 0).comp (measurable_pi_apply i))
  · simpa only [sub_zero] using UCB.iIndepFun_armStreamMeasure_coord_sub kernel 0 0
  · norm_num
  · norm_num
...
#print axioms BanditRLProof.HeavyTail.source_truncated_mean_upper_tail_log_sharp
#print axioms BanditRLProof.HeavyTail.source_truncated_mean_lower_tail_log_sharp
#print axioms HeavyTailSourceConfidenceCanary.upper_sharp
